...
describe('buildInitialGameState — shape', () => {
  it('G has all required top-level keys', () => {
    ...
    assert.ok(
      gameState.transformTargets !== null
        && typeof gameState.transformTargets === 'object'
        && !Array.isArray(gameState.transformTargets),
      'G must have transformTargets object map (D-24469 / WP-658)',
    );
  });
  ...
  it('well-known cardStats entries carry tabletop values (Agent, Trooper, Officer, Sidekick)', () => {
    ...
    assert.deepStrictEqual(
      ...
      { attack: 0, recruit: 1, cost: 0, fightCost: 0, fightCostMode: 'static', fightCostBase: 0, hasAttackIcon: false, hasRecruitIcon: true, isShieldOrHydra: true },
      'S.H.I.E.L.D. Agent entry must give 1 recruit at cost 0 (shows a recruit icon; counts for S.H.I.E.L.D. Level)',
    );
    assert.deepStrictEqual(
      gameState.cardStats[SHIELD_TROOPER_EXT_ID],
      { attack: 1, recruit: 0, cost: 0, fightCost: 0, fightCostMode: 'static', fightCostBase: 0, hasAttackIcon: true, hasRecruitIcon: false, isShieldOrHydra: true },
      'S.H.I.E.L.D. Trooper entry must give 1 attack at cost 0 (shows an attack icon; counts for S.H.I.E.L.D. Level)',
    );
    assert.deepStrictEqual(
      gameState.cardStats[SHIELD_OFFICER_EXT_ID],
      { attack: 0, recruit: 2, cost: 3, fightCost: 0, fightCostMode: 'static', fightCostBase: 0, hasAttackIcon: false, hasRecruitIcon: true, isShieldOrHydra: true },
      'S.H.I.E.L.D. Officer entry must give 2 recruit at recruit cost 3 (shows a recruit icon; counts for S.H.I.E.L.D. Level)',
    );
    assert.deepStrictEqual(
      gameState.cardStats[SIDEKICK_EXT_ID],
      { attack: 0, recruit: 0, cost: 2, fightCost: 0, fightCostMode: 'static', fightCostBase: 0, hasAttackIcon: false, hasRecruitIcon: false, isShieldOrHydra: false },
      'Sidekick entry must carry recruit cost 2 with no face stats/icons (its value is a printed draw-two ability; NOT a S.H.I.E.L.D. card)',
    );
  });
