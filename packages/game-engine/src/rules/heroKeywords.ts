...
export type HeroKeyword =
  ...
  | 'do-over' // why: WP-681 / D-24498 — Deadpool's "Hey, Can I Get a Do-Over?" ("If this is the first Hero you played this turn, you may discard the rest of your hand and draw four cards."). An interactive OPTIONAL accept/decline pending choice gated on the new first-hero-played-this-turn heroCondition. Handler heroEffectDoOver parks a PendingDoOver { playerID } onto the FIFO G.pendingDoOverChoices queue; resolveDoOver({ accept: true }) discards the entire current hand through the discardFromHand chokepoint then draws a FIXED 4 (not one-per-discard), { decline: true } pops with no discard/draw. Carries NO magnitude (the draw-4 is fixed) → in NO_MAGNITUDE_KEYWORDS.
  | 'optional-ko-shield-officer' // why: WP-681 / D-24498 — Nick Fury's "Battlefield Promotion" ("You may KO a [team:shield] Hero from your hand or discard pile. If you do, you may gain a S.H.I.E.L.D. Officer to your hand."). Reuses the optional-ko-reward pending queue (G.pendingOptionalKoRewards): handler heroEffectOptionalKoShieldOfficer parks a PendingOptionalKoReward with koZones ['hand','discard'], koTeamFilter 'shield' (KO target restricted to S.H.I.E.L.D. Heroes; the Officer token itself counts), and rewardType 'gain-officer-hand' (a NEW reward variant that gains a S.H.I.E.L.D. Officer from supply to the acting player's HAND — the discard-bound gain-officer-current is unchanged). The reward is pure upside (a free Officer in hand), so it AUTO-resolves on KO (the reveal-from-hand precedent) rather than parking a second choice; empty Officer supply no-ops the reward. Carries NO magnitude → in NO_MAGNITUDE_KEYWORDS.
  | 'diving-block' // why: WP-682 / D-24499 — Captain America's "Diving Block" ("If you would gain a Wound, you may reveal this card and draw a card instead."). The FIRST reactive WOUND-time keyword — the wound-side sibling of return-on-discard. Fires at the gainWoundForPlayer chokepoint (checkDivingBlock): the Wound lands normally, then a reveal/decline seat choice is offered to the WOUND RECIPIENT (which may be a NON-ACTIVE seat → uses the WP-684 pending-seat-choice capability). On reveal the just-gained Wound is returned to the supply, a card is drawn, and Diving Block STAYS in hand (per-Wound; one copy per Wound). Executes at wound time, so — like return-on-discard — it has NO onPlay HERO_EFFECT_HANDLERS entry and is NOT in NO_MAGNITUDE_KEYWORDS; it joins WOUND_TIME_EXECUTED_KEYWORDS → MVP_KEYWORDS so the play-time hook visit does not emit a no-handler hollow.
  | 'pure-fury' // why: WP-682 / D-24499 — Nick Fury's "Pure Fury" ("Defeat any Villain or Mastermind whose Attack is less than the number of S.H.I.E.L.D. Heroes in the KO pile."). A conditional FREE defeat (no attack paid) that reuses the defeat-with-bystander shared free-defeat path (dispatchDefeatWithBystanderTarget) incl. Masterminds; the predicate is printed attack STRICTLY LESS THAN countShieldHeroesInKo(G) (global G.ko, S.H.I.E.L.D.-team Heroes only, NOT HYDRA). onPlay handler heroEffectPureFury: 0 eligible → no-op, 1 → auto-defeat, ≥2 → parks a PendingDefeatChoice { choiceType:'pure-fury' } resolved by resolveDefeatChoice. Carries NO magnitude → in NO_MAGNITUDE_KEYWORDS.
  | 'here-hold-this' // why: WP-683 / D-24500 — Deadpool's "Here, Hold This for a Second" ("A Villain of your choice captures a Bystander."). Handler heroEffectHereHoldThis: the ACTIVE player picks a City Villain that captures the top-supply Bystander via attachBystanderToVillain; 0 City Villains → the Mastermind captures (universal-rules-v23 §capture), empty Bystander supply → no-op, 1 → auto, ≥2 → parks an ACTIVE-scoped single-seat PendingSeatChoice (kind 'here-hold-this') resolved by resolveSeatChoice. Carries NO magnitude → in NO_MAGNITUDE_KEYWORDS.
  | 'random-acts' // why: WP-683 / D-24500 — Deadpool's "Random Acts of Unkindness" ("You may gain a Wound to your hand. Then each player passes a card from their hand to the player on their left."). Handler heroEffectRandomActs: parks an ACTIVE-scoped single-seat wound choice (PendingSeatChoice kind 'random-acts-wound', gain-Wound-to-HAND vs decline — a new destination variant of the discard-default gain); on resolve it gains the Wound then chains a SIMULTANEOUS MULTI-SEAT pass-left (PendingSeatChoice kind 'random-acts-pass-left', ctx.playOrder adjacency) via WP-684's multi-seat capability, applied ATOMICALLY. Solo / empty-supply degenerate faithfully. Carries NO magnitude → in NO_MAGNITUDE_KEYWORDS.
  | 'put-hand-on-deck-top' // why: WP-700 / D-24519 — printed "Draw N cards. Then put a card from your hand on top of your deck." (Gambit's Stack the Deck, Brainstorm's Time Loop Experiments, + dstr/wpnx/wtif siblings). A compound onPlay handler heroEffectPutHandOnDeckTop draws `magnitude` cards via drawFromPlayerDeck (magnitude = printed draw count) then parks a MANDATORY block-all PendingPutHandOnDeckTop for the ACTIVE player; resolvePutHandOnDeckTop({ cardId }) moves the chosen hand card to the TOP (index 0) of the player's own deck (deck = [cardId, ...deck]), no decline arm. Carries a magnitude (the draw count) → relies on the executeSingleEffect magnitude pre-gate, so it is NOT in NO_MAGNITUDE_KEYWORDS. The new queue enrolls in the D-24518 vanquish-drop set.
  | 'reveal-top-dispose' // why: WP-702 / D-24521 — printed "Reveal the top card of your deck. Discard it or put it back." (Gambit's Hypnotic Charm entry 1 + the rvlt/dstr/cvwr/gotg/shld/xmen/wwhk/wpnx standalone family). onPlay handler heroEffectRevealTopDispose SNAPSHOTS the active player's deck top (reshuffle-on-empty via ctx.random; does NOT remove it) and parks a block-all PendingRevealTopDispose for the ACTIVE player; resolveRevealTopDispose({ ownerPlayerID, cardId, disposition }) either discards that exact deck-top card ('discard') or keeps it ('top', a no-op). Empty deck + discard → logged no-op (nothing to reveal). Carries NO magnitude (per-card disposition, no count) → in NO_MAGNITUDE_KEYWORDS. The new queue enrolls in the D-24518 vanquish-drop set.
  | 'reveal-top-dispose-others' // why: WP-702 / D-24521 — printed "Do the same thing to each other player's deck." (Gambit's Hypnotic Charm entry 2, [hc:instinct]-gated). onPlay handler heroEffectRevealTopDisposeOthers reveals EACH OTHER seat's deck top (Object.keys(G.playerZones).sort(), skip the active player; reshuffle-on-empty via ctx.random; SNAPSHOT, no removal) and parks ONE PendingRevealTopDispose carrying every revealed { ownerPlayerID, cardId } for the ACTIVE player; the shared resolveRevealTopDispose dispositions each discard-or-keep by owner+ext_id. No other seats / all empty → logged no-op (no park). Carries NO magnitude → in NO_MAGNITUDE_KEYWORDS.
...
export const HERO_KEYWORDS: readonly HeroKeyword[] = [
  ...
  'do-over', // why: WP-681 / D-24498 — Deadpool's "Hey, Can I Get a Do-Over?"; gated on the first-hero-played-this-turn condition. Handler heroEffectDoOver parks a PendingDoOver; resolveDoOver({ accept }) discards the whole hand (discardFromHand chokepoint) then draws a FIXED 4, { decline } no-ops. No magnitude → in NO_MAGNITUDE_KEYWORDS
  'optional-ko-shield-officer', // why: WP-681 / D-24498 — Nick Fury's "Battlefield Promotion"; reuses the optional-ko-reward pending queue with koZones ['hand','discard'], koTeamFilter 'shield', rewardType 'gain-officer-hand'. Handler heroEffectOptionalKoShieldOfficer parks the entry; resolveOptionalKoReward KOs the chosen S.H.I.E.L.D. Hero and auto-gains a S.H.I.E.L.D. Officer to hand (pure upside; empty supply no-ops). No magnitude → in NO_MAGNITUDE_KEYWORDS
  'diving-block', // why: WP-682 / D-24499 — Captain America's "Diving Block" reactive wound interception at the gainWoundForPlayer chokepoint; offers a reveal/decline seat choice (WP-684 non-active-seat capability) to the wound recipient → return the Wound + draw + keep in hand. NO onPlay handler; joins WOUND_TIME_EXECUTED_KEYWORDS → MVP_KEYWORDS (mirrors return-on-discard); NOT in NO_MAGNITUDE_KEYWORDS
  'pure-fury', // why: WP-682 / D-24499 — Nick Fury's "Pure Fury" free defeat of a Villain/Mastermind whose printed attack < S.H.I.E.L.D.-Hero count in the global KO pile; onPlay handler heroEffectPureFury reuses the defeat-with-bystander shared free-defeat path (0/1/≥2 → no-op/auto/park PendingDefeatChoice choiceType 'pure-fury'). No magnitude → in NO_MAGNITUDE_KEYWORDS
  'here-hold-this', // why: WP-683 / D-24500 — Deadpool's "Here, Hold This for a Second" ("A Villain of your choice captures a Bystander."); handler heroEffectHereHoldThis attaches via attachBystanderToVillain (0 Villains → Mastermind captures, empty supply → no-op, 1 → auto, ≥2 → active-scoped PendingSeatChoice kind 'here-hold-this'). No magnitude → in NO_MAGNITUDE_KEYWORDS
  'random-acts', // why: WP-683 / D-24500 — Deadpool's "Random Acts of Unkindness" ("You may gain a Wound to your hand. Then each player passes a card from their hand to the player on their left."); handler heroEffectRandomActs parks an active-scoped wound choice (kind 'random-acts-wound', gain-to-HAND) then chains a simultaneous multi-seat pass-left (kind 'random-acts-pass-left', ctx.playOrder adjacency, atomic apply) via WP-684. No magnitude → in NO_MAGNITUDE_KEYWORDS
  'put-hand-on-deck-top', // why: WP-700 / D-24519 — "Draw N cards. Then put a card from your hand on top of your deck." (Gambit Stack the Deck + siblings). Compound onPlay handler heroEffectPutHandOnDeckTop draws `magnitude` cards then parks a MANDATORY block-all PendingPutHandOnDeckTop; resolvePutHandOnDeckTop places the chosen hand card on the deck TOP (index 0). Carries a magnitude (the draw count) → NOT in NO_MAGNITUDE_KEYWORDS
  'reveal-top-dispose', // why: WP-702 / D-24521 — "Reveal the top card of your deck. Discard it or put it back." (Gambit Hypnotic Charm entry 1 + standalone family). onPlay handler heroEffectRevealTopDispose snapshots the active player's deck top (reshuffle-on-empty; no removal) then parks a block-all PendingRevealTopDispose; resolveRevealTopDispose discards ('discard') or keeps ('top') that exact deck-top card. NO magnitude → in NO_MAGNITUDE_KEYWORDS
  'reveal-top-dispose-others', // why: WP-702 / D-24521 — "Do the same thing to each other player's deck." (Hypnotic Charm entry 2, [hc:instinct]-gated). onPlay handler heroEffectRevealTopDisposeOthers reveals each OTHER seat's deck top (sorted seat order, skip active; snapshot) then parks ONE shared PendingRevealTopDispose; resolveRevealTopDispose dispositions each by owner+ext_id. NO magnitude → in NO_MAGNITUDE_KEYWORDS
] as const;
