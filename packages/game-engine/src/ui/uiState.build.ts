...
export function buildUIState(
  ...
  const game = {
    ...
    // numbers its lines by (G.logMeta.turn, the PLAY-RELATIVE turn stamped in the
    // play-phase onBegin), NOT live ctx.turn. logMeta.turn is offset so the first
    // play turn reads 1 (the lobby phase consumes framework turn 1, so ctx.turn is
    // one ahead throughout play); reading it here keeps the header and the log in
    // lock-step. It also fixes the game-over display: when the match ends,
    // boardgame.io transitions play -> end and a phase change bumps ctx.turn one
    // further past the last play turn, but the end phase has no onBegin, so
    // logMeta.turn stays frozen at the last play-relative turn and matches the log.
    // Fall back to ctx.turn before the first play-phase onBegin has stamped logMeta
    // (lobby/setup), where the framework turn is the only number available.
  ...
  // snapshot; the block-all guard freezes the hand while pending) resolved to display
  // data, in hand order, plus the limit. The client requires exactly
  // `hand.length - limit` selections. resolveDisplay is spread fresh per entry so the
  // projection holds no reference into G.cardDisplayData (aliasing defense, WP-111
  // D-11105). Redaction to the chooser-only audience is enforced by
  // filterUIStateForAudience (D-24011 — the hand carries the chooser's private card
  // identities and must not leak to opponents/spectators).
  let pendingDiscardChoice: UIPendingDiscardChoice | undefined;
  if (gameState.pendingDiscardChoices !== undefined && gameState.pendingDiscardChoices.length > 0) {
    const frontChoice = gameState.pendingDiscardChoices[0]!;
    const chooserZones = gameState.playerZones[frontChoice.playerID];
    if (chooserZones !== undefined) {
  ...
  // --- 13c.6 Project pending return-zero-cost-discard choice (front of queue) ---
  // why: D-24139 — project the FRONT entry of G.pendingReturnZeroCostDiscard with the
  // eligible 0-cost discard cards recomputed fresh from the chooser's current discard
  // pile via getEligibleZeroCostDiscardCards — the SAME predicate the resolve move
  // validates with, so the client's { cardId } selection always round-trips. Each
  // display is spread fresh so the projection holds no reference into
  // G.cardDisplayData (aliasing defense, WP-111 D-11105). Redaction to the
  // chooser-only audience is enforced by filterUIStateForAudience (keyed on
  // .playerID), mirroring pendingOptionalPutBottomHQ.
  let pendingReturnZeroCostDiscard: UIPendingReturnZeroCostDiscard | undefined;
  if (
    gameState.pendingReturnZeroCostDiscard !== undefined &&
    gameState.pendingReturnZeroCostDiscard.length > 0
  ) {
    const frontChoice = gameState.pendingReturnZeroCostDiscard[0]!;
    const eligibleDiscardCards: UIEligibleKoHeroCard[] = [];
    for (const cardId of getEligibleZeroCostDiscardCards(gameState, frontChoice.playerID)) {
      eligibleDiscardCards.push({
        zone: 'discard',
        cardId,
        display: { ...resolveDisplay(cardId, gameState) },
      });
  ...
  // --- 13c.7 Project pending discard-to-play cost (front of queue) ---
  // why: WP-383 / D-24184 — project the FRONT entry of G.pendingDiscardToPlay with the
  // eligible hand cards recomputed fresh via getEligibleDiscardToPlayCards — the SAME
  // predicate the resolve move validates with, so the client's { cardId } selection
  // always round-trips. Each display is spread fresh (aliasing defense, WP-111 D-11105).
  // Chooser-only redaction is enforced by filterUIStateForAudience (keyed on .playerID),
  // mirroring pendingReturnZeroCostDiscard.
  let pendingDiscardToPlay: UIPendingDiscardToPlay | undefined;
  if (
    gameState.pendingDiscardToPlay !== undefined &&
    gameState.pendingDiscardToPlay.length > 0
  ) {
    const frontChoice = gameState.pendingDiscardToPlay[0]!;
    const eligibleDiscardCards: UIEligibleKoHeroCard[] = [];
    for (const cardId of getEligibleDiscardToPlayCards(gameState, frontChoice.playerID)) {
      eligibleDiscardCards.push({
        zone: 'hand',
        cardId,
        display: { ...resolveDisplay(cardId, gameState) },
    ...
    pendingDiscardToPlay = {
      playerID: frontChoice.playerID,
      remaining: frontChoice.remaining,
      eligibleDiscardCards,
    };
  }
  ...
  return {
    ...
    // why: D-24139 — omit-when-absent (never an `undefined` literal) under exactOptionalPropertyTypes.
