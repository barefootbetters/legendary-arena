...
describe('revealVillainCard', () => {
  ...
  it('does NOT attach a bystander on villain City entry — canonical (WP-432)', () => {
    // why: WP-432 removed the non-canonical D-1701 entry-attach. A villain
    // entering the City captures nothing from the supply pile (bystanders enter
    // only via a bystander CARD reveal or a specific effect); the supply pile is
    // untouched and no "on entering the City" line is emitted.
    const gameState = createMockGameState({
      deck: ['card-a', 'card-b'],
      discard: [],
      cardTypes: { 'card-a': 'villain', 'card-b': 'villain' },
    ...
    assert.ok(
      !('card-a' in moveContext.G.attachedBystanders) ||
        moveContext.G.attachedBystanders['card-a']!.length === 0,
