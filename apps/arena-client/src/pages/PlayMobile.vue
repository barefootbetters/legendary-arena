...
export default defineComponent({
  ...
  setup() {
    ...
    // blocks end-turn / pass-priority at EVERY stage while a Shadowed Thoughts
    // play-top-Villain-Deck choice is pending (board frozen, mirrors hasPendingDrawOrEmpowered).
    ...
    // Passed to TurnActionBar to block end-turn / pass-priority while a reveal-three assignment is pending.
      ...
      >
        ...
        <template v-if="viewer !== null">
          ...
          <!-- why: WP-603 / D-24413 — the Melter Fight KO/keep prompt renders above
               TurnActionBar; appears only for the fighting player when
               pendingMelterKoChoice is set. The block-all guard guarantees at most one
               pending-choice type is set. -->
          <PendingMelterKoChoicePrompt
          ...
          <PendingRevealTopDisposePrompt
            ...
            :submit-move="submitMove"
          />
          <!-- why: WP-753 / D-24580 — the reveal-three draw / discard / KO prompt; choosing (active) player only. -->
          <PendingRevealThreeAssignPrompt
            :pending-reveal-three-assign="snapshot.pendingRevealThreeAssign"
            :viewer-player-id="viewer.playerId"
            :submit-move="submitMove"
          />
          <!-- why: WP-695 / D-24512 — the Ruthless Dictator scry-3 prompt; choosing player only. -->
          <PendingRuthlessDictatorChoicePrompt
            :pending-ruthless-dictator-choice="snapshot.pendingRuthlessDictatorChoice"
            :viewer-player-id="viewer.playerId"
            :submit-move="submitMove"
          />
          <!-- why: WP-695 / D-24512 — the Electromagnetic Bubble X-Men pick prompt; choosing player only. -->
          <PendingElectromagneticBubbleChoicePrompt
          ...
          <PendingDiscardChoicePrompt
            ...
            :submit-move="submitMove"
          />
          <!-- why: WP-538 / D-24347 — the core Dr. Doom put-cards-on-deck prompt renders above
               TurnActionBar; appears only for the choosing player when
               pendingPutCardsOnDeckChoice is set. The block-all guard guarantees at most one
               pending-choice type is set. -->
          <PendingPutCardsOnDeckChoicePrompt
          ...
          <DrawOrEmpoweredPrompt
            ...
            :submit-move="submitMove"
          />
          <!-- why: WP-719 / D-24541 — the Covering Fire choose-one prompt (Hawkeye); appears only
               for the choosing (active) player when pendingCoveringFireChoice is set. Same
               block-all posture as the draw-or-empowered prompt above. -->
          <CoveringFireChoicePrompt
          ...
          <!-- why: WP-678/679 / D-24494/D-24495 — the Undercover target pick (send which
               [team:shield] Hero), incl. the nested pick from the shld mixed choose-one. -->
          <UndercoverChoicePrompt
          ...
          <!-- why: WP-684 / D-24501 — the non-active/multi-seat pending-choice prompt
               (foundational scaffold; concrete card renderers ship with WP-682 / WP-683).
               Appears for an addressed, still-outstanding seat (incl. a NON-ACTIVE seat) via
               the engine per-seat audience redaction. Same block-all posture as above. -->
          <PendingSeatChoicePrompt
          ...
          <!-- why: WP-663 / D-24474 — the Shadowed Thoughts play-top-Villain-Deck prompt;
               appears only for the choosing player when pendingPlayVillainTop is set. The
