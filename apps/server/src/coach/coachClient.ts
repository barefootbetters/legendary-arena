...
const COACH_SYSTEM_PROMPT = [
  ...
  'so treat them as deliberate purchase choices, never as starter cards. Each entry',
  'reads "Card Name ×N (Hero)": the count appears only when more than one copy was',
  'bought, and the hero in parentheses is the hero deck the card belongs to. A card',
  'with no hero (e.g. S.H.I.E.L.D. Officer) is a supply-pile card. `heroes` is the',
  'pool of hero decks that were available to buy this match.',
  '',
  ...
  '`conditionalClausesAssembled` are CLAUSE counts covering ONLY synergy clauses:',
  'abilities gated on what else was in play when the card was played (e.g. another',
  'Hero of a class or team). Other conditions, such as Ambush or Master Strike',
  'triggers, Digest, or Excessive Violence, are NOT in these counts. So never',
  'conclude from them that a particular card or hero "never triggered"; they only',
  'say how many synergy clauses were played and how many had their condition met.',
  'The summary does not say WHICH cards those clauses belong to, so never name a',
  'specific card as an unassembled synergy clause.',
