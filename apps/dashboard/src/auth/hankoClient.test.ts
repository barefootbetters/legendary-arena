...
// why: the wrapper's `__hankoFactory` test seam keeps `@teamhanko/*` out of the
// test bundle entirely — the real package registers browser-only custom
// elements at module load, which crashes under `node --import tsx --test`. Only
// `import type` declarations from the wrapper are used here; no runtime broker
// code is loaded. Ported from the arena-client wrapper's test (WP-241 §H).

/**
 * Build a fake broker handle. Tests pass overrides per case; any unspecified
 * method throws if invoked unexpectedly — a stronger guarantee than silently
 * returning `undefined`.
 */
function makeFakeHanko(overrides: Partial<HankoLike>): HankoLike {
  return {
    ...
    logout:
      overrides.logout ??
      (async () => {
        throw new Error('Fake logout called unexpectedly — the test did not stub this method.');
      }),
    onSessionCreated:
    ...
    async () => {
      await initializeHankoClient({
        ...
        __hankoFactory: async () => {
          throw new Error('network down: tenant unreachable at https://example.test');
        },
      });
    },
    (err: unknown) => {
      assert.ok(err instanceof HankoInitializationFailed, 'expected HankoInitializationFailed');
...
test('getCurrentTokenFromHandle normalizes a null SDK return to null', async () => {
  const fake = makeFakeHanko({ getSessionToken: () => null as unknown as string });
  const handle = await initializeHankoClient({
    tenantBaseUrl: 'https://example.test',
    __hankoFactory: async () => fake,
  });
...
test('getCurrentTokenFromHandle normalizes an empty-string SDK return to null', async () => {
  // why: explicitly verify the empty-string case — a future SDK behavior change
  // that returned '' instead of null would otherwise leak an empty bearer header
  // to API clients and surface as a confusing 401.
...
test('onSessionCreated reads getSessionToken at fire time, ignoring the event payload', async () => {
  ...
  // why: invoke the broker-side listener with an arbitrary fake event payload.
  // The wrapper MUST ignore the payload and re-read `getSessionToken()` at fire
  // time — verified by the consumer receiving the getter's return value.
  registeredCallback!({ payload: 'unrelated-event-string', claims: { sub: 'irrelevant' } });
  assert.equal(consumerReceivedTokens.length, 1);
  assert.equal(consumerReceivedTokens[0], 'jwt-from-getter');
  assert.ok(getterReads >= 1, 'wrapper must call getSessionToken on every session-created fire');
});
