 ...
 * subdomains — a single sign-in on `dashboard.` is visible to `play.` and
 * `www.` (cross-subdomain SSO; WP-347 amends D-16002's default-cookie
 * stance). On any other host it returns `undefined` so the SDK keeps its
 * default host-scoped cookie — a browser silently DROPS a `Set-Cookie`
 * whose `Domain` the current page is not under, so forcing
 * `.legendary-arena.com` on `localhost` (dev) or `*.pages.dev` (preview)
 * would break sign-in there.
 *
...
async function defaultProductionFactory(
  ...
  // production hosts so play / dashboard / www share one sign-in; omit it on
  // localhost / *.pages.dev (setting a domain the page is not under makes the
  // browser drop the cookie and breaks sign-in). Production-only branch (tests
  // inject `__hankoFactory`), so reading `window.location.hostname` is safe.
  const cookieDomain =
