...
test('useInPlayCoverage reads the real committed seed + ledger and computes the in-play coverage snapshot', () => {
  ...
  // 2026-09-07 (WP-665 / D-24476, re-pin): this snapshot reads the committed seed +
  // hero ledger + runtime-observed hollows, all regenerated by WP-665. Amadeus Cho's
  // `gamma-draining-nanites` transform is now EXECUTABLE (gated on cardsDrawnThisTurnAtLeast),
  // so its observations move OUT of the unresolved `transform` bucket into resolved — and
  // the regen also caught up prior stale derived feeds (WP-660/661 recruit-threshold gating,
