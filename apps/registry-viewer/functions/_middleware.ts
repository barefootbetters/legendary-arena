/**
 * Asset-shell guard — Registry Viewer edge subsurface.
 *
 ...
 *
 * Why this exists: Cloudflare Pages' built-in single-page-application fallback
 * answers any unmatched path — including a missing `/assets/<hash>.js` — with
 * `200` + `index.html` (`text/html`). `public/_headers` marks `/assets/*`
 ...
 * strict MIME checking rejects it, and the viewer never mounts — a blank screen
 ...
 * again 2026-08-23, and that `legends.legendary-arena.com` was exposed to;
 * `cards.legendary-arena.com` carried the same preconditions with no guard. See
 * ewiki `operational-health-checks.md §SPA asset delivery`.
 *
 ...
 *
 * IMPORTANT: this guard only runs if the `legendary-arena` Pages project is
 * configured so Cloudflare finds this `functions/` directory — its **Root
 * directory must be `apps/registry-viewer`** (with Build output `dist`). With Root
 ...
 *   curl -sSI https://cards.legendary-arena.com/assets/does-not-exist-probe.js
 ...
 *
 * Edge subsurface of the client-only Registry Viewer SPA: imports no game
 * framework, no engine runtime, and no database client; computes no game outcomes.
 */
