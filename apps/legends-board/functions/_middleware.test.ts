/**
 * Tests for the legends-board `_middleware` asset-shell guard.
 *
 ...
 * Function in production. That wiring depends on the `legendary-arena-legends`
 * Pages project Root directory being `apps/legends-board`; verify it live with a
...
test('missing hashed asset (HTML shell at an /assets/ URL) returns an uncacheable 404', async () => {
  const context = makeContext(
    'https://legends.legendary-arena.com/assets/index-DHXnxNt7.js',
    htmlShellResponse(),
  );
...
test('an ordinary SPA navigation (HTML at a non-asset path) passes through unchanged', async () => {
  const context = makeContext(
    'https://legends.legendary-arena.com/some/deep/spa-route',
    htmlShellResponse(),
  );
...
test('a CSS asset that fell back to the HTML shell also returns a 404', async () => {
  const context = makeContext(
    'https://legends.legendary-arena.com/assets/index-Bc8_E8DQ.css',
    htmlShellResponse(),
  );
