...
**Vector lockup and wordmark (SVG).** The horizontal lockup and the
`LEGENDARY / ARENA` wordmark as scalable vectors. Unlike the emblem, these
are a **true vector extraction**, not an autotrace: the master
`Legendary Arena-400x200.ai` is PDF-compatible, so its real paths, gradients,
and already-outlined letterforms import directly — only small soft-glow
effects remain as embedded rasters. Use the lockup for site headers, press,
and marketing heroes; use the wordmark alone where the emblem is shown
separately or a text-only lockup is needed. Both are transparent masters;
the versions shown here sit on a dark plate for the light wiki page.

![Legendary Arena horizontal lockup as a vector on a dark rounded plate — the hero/villain emblem left of LEGENDARY in a gold gradient with ARENA below in white between gold flourishes.](/branding/logo-la-lockup-on-dark.svg "width=72%")

![Legendary Arena wordmark as a vector on a dark rounded plate — LEGENDARY in a gold gradient above ARENA in white between two gold flourishes, no emblem.](/branding/logo-la-wordmark-on-dark.svg "width=56%")

*Extracted from the `.ai` with Inkscape, then split into lockup and wordmark
masters. Generator:
[build-lockup-svg.py](../ewiki/branding/build-lockup-svg.py) — regenerate all
four SVGs with `python build-lockup-svg.py` (needs the `.ai` in pCloud and
Inkscape on PATH). The transparent masters are `logo-la-lockup.svg` and
`logo-la-wordmark.svg`.*

### Which file, and where the masters live

| Mark | On-wiki artifact (`ewiki/branding/`) | Approved master (source of truth) |
|---|---|---|
| Horizontal lockup (raster) | `logo-la-lockup-black.png` (1600 px wide) | `C:\pcloud\LA\brand\Legendary Arena-400x200.ai` |
| Horizontal lockup (vector master) | `logo-la-lockup.svg` (transparent) | extracted from the `.ai` |
| Horizontal lockup (vector, dark plate) | `logo-la-lockup-on-dark.svg` | same extraction, for light-background display |
| Wordmark (vector master) | `logo-la-wordmark.svg` (transparent) | extracted from the `.ai` |
| Wordmark (vector, dark plate) | `logo-la-wordmark-on-dark.svg` | same extraction, for light-background display |
...
The emblem SVGs (plus their trace input `emblem-trace-source.png`) come from
`build-emblem-svg.py`; the four lockup/wordmark SVGs come from
`build-lockup-svg.py`. Each generator emits its whole set in one run, so the
transparent and dark-plate variants never drift, and both traces are
deterministic — re-running on the same input is byte-identical.
...
- **The `.ai` is the editable master; the SVGs are derived.** The `.ai`
  itself does not render on the JavaScript-free wiki, so the page serves a
  PNG plus vector SVGs extracted from it. To change the lockup or wordmark
  you edit the `.ai`, then re-run `build-lockup-svg.py` to regenerate the
  SVGs and re-export the PNG — you do not edit the SVGs or PNG by hand.
...
- **The emblem SVG is an autotrace, not a hand-built master.** `logo-la-emblem.svg`
  ...
  production master if one is needed. (The lockup and wordmark SVGs are the
  higher-fidelity case — a *true* vector extraction from the `.ai`, not a
  trace.)
- **The lockup/wordmark SVGs carry a few embedded raster glows.** The `.ai`
  imports as real paths and gradients, but Illustrator's soft-glow effects
  come across as small embedded rasters (~27 KB of the file). The letterforms
  and emblem shapes are true vector and scale cleanly; only those decorative
  glows would pixelate at extreme magnification. Stripping them (for a purely
  vector file) would drop the glow and is deferred until a use actually needs
  it.
...
- `C:\pcloud\LA\brand\` — the approved masters: `Legendary Arena-400x200.ai`
  (lockup + wordmark, the editable vector master the SVGs are extracted from),
  `logo-la-black.jpg` (emblem), `Logo-LA-800x800.png` (gold hero half), and
  `segments.zip` (the emblem's raster layers, the trace input for
  `logo-la-emblem.svg`).
