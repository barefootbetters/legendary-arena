...
| Square emblem | `logo-la-emblem-black.jpg` (1408 × 1408) | `C:\pcloud\LA\brand\logo-la-black.jpg` |
...
(the emblem broken into its component paths) — stay in `C:\pcloud\LA\brand\`
and are **not** copied into this repo. The brand folder's own rule is that
...
- **No SVG masters yet.** The brand folder's naming convention anticipates
  `logo-la-*.svg` vector masters; today the approved files are the `.ai`
  plus PNG/JPG exports. A web-native SVG of the emblem would let it scale
  crisply in the app HUD and favicon without a raster step.
...
- `C:\pcloud\LA\brand\` — the approved masters: `Legendary Arena-400x200.ai`
  ...
  half), and `segments.zip` (emblem component paths).
