...
source:
  ...
  - ../docs/ai/work-packets/WP-500-auto-assign-handles.md
...
> **Handle claiming — auto-assigned at sign-in as of WP-500 (was unwired; discovered 2026-08-04, fixed 2026-08-05).**
>
> **History (the bug).** WP-101 shipped the handle columns + `claimHandle`, but the
> *claim path was never wired*: `claimHandle` (`apps/server/src/identity/handle.logic.ts`)
> had **zero non-test call sites** — no HTTP route exposed it, and `MyProfilePage.vue`
> only *rendered* `@{handleCanonical}` (no claim/change input). First-sign-in provisioning
> (`accountProvisioning.logic.ts`, WP-174) never set a handle, so `handle_canonical` was
> **NULL for every account**, and every `@handle` affordance — the Friends "add by
> `@handle`" (WP-351, `friendships.routes.ts`) and the match-invite-by-`@handle`
> (WP-358 / WP-366, `matchInvites.routes.ts`), both via `findAccountByHandle` — returned
> `handle_not_found` (**HTTP 404**) for every target. Surfaced 2026-08-04 when an operator
> tried to invite a newly-registered friend by username; the only working path was the
> WP-369 copy-join-link (identity-agnostic — it resolves a match by ID, never a handle).
>
> **Fix — two tracks (both shipped 2026-08).**
> - *Track 1 — shipped.* **[WP-499](../docs/ai/work-packets/WP-499-join-by-match-id.md)** /
>   D-24302 productized the copy-link as a lobby **"Join by match ID or link"** input
>   (paste a raw ID or invite link → join the first open seat via `POST /api/match/join`).
>   Identity-agnostic — two players can play together with **no handle at all**.
> - *Track 2 core — shipped.* **[WP-500](../docs/ai/work-packets/WP-500-auto-assign-handles.md)** /
>   D-24303 wires handle assignment itself. `deriveHandle` slugs `display_name` to a valid
>   handle (`^[a-z][a-z0-9_]{2,23}$`; prefix `u` for a leading non-letter, `player` fallback),
>   and `assignAutoHandle` sets it **fail-open at first-sign-in provisioning** — writing
>   `handle_canonical` + `display_handle` while leaving `handle_locked_at` **NULL**, so the
>   auto-handle stays **changeable** (a later explicit `claimHandle` locks it). This amends
>   the WP-101 mutual-presence invariant: `handle_locked_at` is now independent (NULL =
>   auto-assigned/changeable, `now()` = explicit claim), and `assignAutoHandle` is a
>   sanctioned **second, disjoint** writer (`handle_canonical` + `display_handle` only);
>   `claimHandle` stays the sole writer of `handle_locked_at`.
>
> **⚠ One operational step still outstanding — the prod backfill.** Auto-assignment fires
> only for *newly-provisioned* accounts, so the entire pre-WP-500 user base remains
> NULL-handle — and thus unreachable by `@handle` — until an operator runs
> `node --env-file=.env scripts/backfill-handles.mjs` against **prod** (the D-24026
...
5. Ranked eligibility gate — **executed as
   ...
   The charter's *lobby-invite-flow* half is **executed as
   [WP-358](../docs/ai/work-packets/WP-358-match-friend-invite-server.md)**
   (executed 2026-07-11, D-24150 Active): the create/join lobby exists,
   ...
   **executed as [WP-369](../docs/ai/work-packets/WP-369-waiting-room-seat-aware-invite-panel.md)**
   ...
   lobby "Join by match ID or link" input) — the reliable, identity-agnostic path
   to play together regardless of handle state (it predates and complements WP-500
   handle auto-assignment; see the callout in *Owner-page identity fields* above):
   it also adds a read-only `GET /games/legendary-arena/:id` fetch
...
6. Abuse controls — **executed as
   ...
   spam-vector risk) is **executed as
...
- **Display label — now resolves the real name (WP-305 + WP-330).** Per
  ...
  normally wins.
- **`@handle` lookups — 404-for-everyone until WP-500; now auto-assigned.** Historically
  every feature resolving a typed name through `findAccountByHandle` (add-a-friend, invite
  by `@handle`) returned `handle_not_found` for **every** target, because `claimHandle` had
  no production caller and provisioning left `handle_canonical` NULL for all — the whole
  `@handle` surface was dormant. **WP-500 (2026-08-05)** fixed the root cause: new accounts
  get a changeable auto-derived handle at sign-in, and `scripts/backfill-handles.mjs` heals
  existing rows. **Caveat:** existing-account reachability is **pending the prod backfill
  run** (auto-assign only covers new accounts). Full diagnosis + the shipped two-track fix +
  the outstanding prod backfill are in the callout under *Owner-page identity fields*.
