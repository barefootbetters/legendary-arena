...
_Generated 2026-09-14 by `scripts/architecture-inventory.mjs`._
...
- **`apps/registry-viewer`** — Client-only Registry Viewer for Legendary Arena (Vite + Vue 3)
  - Stack: Vue 3 SFCs (`vue@^3.4.27`) + Pinia stores (`pinia@^2.1.7`) + Vite bundler (`vite@^5.3.1`).
...
- **`apps/wiki-viewer`** — Engineering wiki build pipeline. Build-time, read-only Hugo projection of `wiki/` (no `package.json` — Hugo is a Go binary, not a Node dep). Layer-boundary clean: zero runtime imports of `@legendary-arena/game-engine`, `@legendary-arena/registry`, or `apps/server`. Build pipeline is `pnpm wiki-viewer:project` (copy `wiki/*.md` → `apps/wiki-viewer/content/`) → `pnpm wiki-viewer:check-links` (case-sensitive internal-link gate) → `hugo --minify`.
  - Stack: Hugo Extended (`hugo@0.161.1`, pinned in `apps/wiki-viewer/.hugo-version`) + 82 source pages projected from `wiki/` + deployed as Render Static Site `legendary-arena-wiki`.
...
|---|---|---|
| Cloudflare Access (zero-trust gate) | zero-trust gate | `dashboard.legendary-arena.com`, `ewiki.legendary-arena.com`, `legendary-arena-dashboard.pages.dev` |
...
|---|---|---|---:|---:|---:|
| `apps/arena-client/package.json` | @legendary-arena/arena-client | Gameplay client SPA for Legendary Arena (Vue 3 + Vite + Pinia, TypeScript) | 7 | 15 | 0 |
| `apps/dashboard/package.json` | @legendary-arena/dashboard | Internal admin dashboard SPA for Legendary Arena (Vue 3 + PrimeVue 4 + Vite) | 10 | 13 | 0 |
...
| `apps/legends-board/package.json` | @legendary-arena/legends-board | Public Legends Attract Board — read-only scoreboard SPA for legends.legendary-arena.com | 1 | 10 | 0 |
| `apps/registry-viewer/package.json` | registry-viewer | Client-only Registry Viewer for Legendary Arena (Vite + Vue 3) | 5 | 14 | 0 |
...
| `package.json` | legendary-arena | Legendary Arena monorepo — card registry, viewer, and tooling | 0 | 5 | 0 |
...
| `pinia` | ^2.1.7 | 66 _(comprehensive)_ | `apps/arena-client/package.json` (dep); `apps/dashboard/package.json` (dep); `apps/registry-viewer/package.json` (dep) |
...
| `vue` | ^3.4.27 | 257 _(comprehensive)_ | `apps/arena-client/package.json` (dep); `apps/dashboard/package.json` (dep); `apps/legends-board/package.json` (dep); `apps/registry-viewer/package.json` (dep); `packages/vue-sfc-loader/package.json` (dev); `packages/vue-sfc-loader/package.json` (peer) |
...
| Package | Version(s) | Files importing | Declared in |
|---|---|---:|---|
| `boardgame.io` | ^0.50.0 | 68 _(comprehensive)_ | `apps/arena-client/package.json` (dep); `apps/server/package.json` (dep); `packages/game-engine/package.json` (dep) |
...
| `zod` | ^3.23.8, ^3.22.4 | 19 _(comprehensive)_ | `apps/registry-viewer/package.json` (dep); `packages/lagn-spec/package.json` (dep); `packages/registry/package.json` (dep) |
...
| Package | Version(s) | Files importing | Declared in |
|---|---|---:|---|
| `echarts` | ^5.5.0 | 13 _(partial)_ | `apps/dashboard/package.json` (dep) |
...
| Package | Version(s) | Files importing | Declared in |
|---|---|---:|---|
| `pg` | ^8.13.0 | 54 _(comprehensive)_ | `apps/server/package.json` (dep) |
...
| Package | Version(s) | Files importing | Declared in |
|---|---|---:|---|
| `@vue/test-utils` | ^2.4.6 | 94 _(comprehensive)_ | `apps/arena-client/package.json` (dev); `packages/vue-sfc-loader/package.json` (dev) |
...
_Other candidates in this category not currently installed:_ `vitest`, `happy-dom`, `@playwright/test`, `cypress`, `msw`, `sinon`, `fast-check`
...
| `@legendary-arena/game-engine` | workspace:* | 220 _(comprehensive)_ | `apps/arena-client/package.json` (dev); `apps/engine-runner/package.json` (dep); `apps/replay-producer/package.json` (dep); `apps/server/package.json` (dep); `package.json` (dev); `packages/preplan/package.json` (peer) |
...
| `@types/node` | ^22.19.17, ^20.0.0, ^25.6.0 | 9 _(partial)_ | `apps/arena-client/package.json` (dev); `apps/dashboard/package.json` (dev); `apps/legends-board/package.json` (dev); `apps/registry-viewer/package.json` (dev); `apps/replay-producer/package.json` (dev); `packages/lagn-spec/package.json` (dev); `packages/registry/package.json` (dev); `packages/vue-sfc-loader/package.json` (dev) |
...
| `canvas-confetti` | ^1.9.3 | 1 _(minimal)_ | `apps/arena-client/package.json` (dep) |
...
| `howler` | ^2.2.4 | 2 _(minimal)_ | `apps/arena-client/package.json` (dep) |
| `koa-body` | ^5.0.0 | 21 _(comprehensive)_ | `apps/server/package.json` (dep) |
| `pagefind` | 1.5.2 | 0 ⚠ | `package.json` (dev) |
...
|---|---|---:|---|
| `brevo` | marketing / email | 21 files | Transactional + marketing email, newsletter forms, SMTP relay. |
...
- `[legendary-arena-website] docs/ai/work-packets/WP-018-brevo-automation.md`
...
- `[legendary-arena-website] assets/js/snipcart-lazy.js`
- `[legendary-arena-website] docs/01-VISION.md`
- `[legendary-arena-website] docs/ai/work-packets/WP-019-snipcart-commerce.md`
- `[legendary-arena-website] docs/ai/work-packets/WP-044-site-performance-recovery.md`
...
|---|---|---|---:|
| `boardgame.io` | ^0.50.0 | direct dep — `apps/arena-client/package.json`, `apps/server/package.json`, `packages/game-engine/package.json` | 68 _(comprehensive)_ |
| `pg` | ^8.13.0 | direct dep — `apps/server/package.json` | 54 _(comprehensive)_ |
...
| `pinia` | ^2.1.7 | direct dep — `apps/arena-client/package.json`, `apps/dashboard/package.json`, `apps/registry-viewer/package.json` | 66 _(comprehensive)_ |
...
| `vue` | ^3.4.27 | direct dep — `apps/arena-client/package.json`, `apps/dashboard/package.json`, `apps/legends-board/package.json`, `apps/registry-viewer/package.json`, `packages/vue-sfc-loader/package.json` | 257 _(comprehensive)_ |
...
| Package | Version(s) | Adoption | Files importing |
|---|---|---|---:|
| `@cloudflare/workers-types` | ^4.20260701.1, ^4.20240620.0 | direct dep — `apps/arena-client/package.json`, `apps/legends-board/package.json`, `apps/registry-viewer/package.json`, `packages/registry/package.json` | 3 _(partial)_ |
| `@types/jsdom` | ^21.1.7 | direct dep — `apps/arena-client/package.json` | 0 _(tooling)_ |
| `@types/node` | ^22.19.17, ^20.0.0, ^25.6.0 | direct dep — `apps/arena-client/package.json`, `apps/dashboard/package.json`, `apps/legends-board/package.json`, `apps/registry-viewer/package.json`, `apps/replay-producer/package.json`, `packages/lagn-spec/package.json`, `packages/registry/package.json`, `packages/vue-sfc-loader/package.json` | 9 _(partial)_ |
...
| `@vue/test-utils` | ^2.4.6 | direct dep — `apps/arena-client/package.json`, `packages/vue-sfc-loader/package.json` | 94 _(comprehensive)_ |
...
|---|---|---|
| `@cloudflare/workers-types` | ^4.20260701.1, ^4.20240620.0 | `apps/arena-client/package.json` ^4.20260701.1; `apps/legends-board/package.json` ^4.20260701.1; `apps/registry-viewer/package.json` ^4.20260701.1; `packages/registry/package.json` ^4.20240620.0 |
...
| `apps/legends-board/functions/tsconfig.json` | `@cloudflare/workers-types`, `@types/node` |
| `apps/legends-board/tsconfig.json` | `@types/node`, `@vue/tsconfig` |
| `apps/registry-viewer/functions/tsconfig.json` | `@cloudflare/workers-types`, `@types/node` |
...
Lockfile resolves **630** packages: **48** are direct dependencies declared in some `package.json`, **582** are transitive.
...
- Installed but never mentioned in doc: **26**
