...
_Generated 2026-08-31 by `scripts/architecture-inventory.mjs`._
...
- **`apps/wiki-viewer`** — Engineering wiki build pipeline. Build-time, read-only Hugo projection of `wiki/` (no `package.json` — Hugo is a Go binary, not a Node dep). Layer-boundary clean: zero runtime imports of `@legendary-arena/game-engine`, `@legendary-arena/registry`, or `apps/server`. Build pipeline is `pnpm wiki-viewer:project` (copy `wiki/*.md` → `apps/wiki-viewer/content/`) → `pnpm wiki-viewer:check-links` (case-sensitive internal-link gate) → `hugo --minify`.
  - Stack: Hugo Extended (`hugo@0.161.1`, pinned in `apps/wiki-viewer/.hugo-version`) + 69 source pages projected from `wiki/` + deployed as Render Static Site `legendary-arena-wiki`.
...
|---|---|---|---:|---:|---:|
| `apps/arena-client/package.json` | @legendary-arena/arena-client | Gameplay client SPA for Legendary Arena (Vue 3 + Vite + Pinia, TypeScript) | 7 | 14 | 0 |
...
| `pinia` | ^2.1.7 | 61 _(comprehensive)_ | `apps/arena-client/package.json` (dep); `apps/dashboard/package.json` (dep); `apps/registry-viewer/package.json` (dep) |
...
| `vue` | ^3.4.27 | 226 _(comprehensive)_ | `apps/arena-client/package.json` (dep); `apps/dashboard/package.json` (dep); `apps/legends-board/package.json` (dep); `apps/registry-viewer/package.json` (dep); `packages/vue-sfc-loader/package.json` (dev); `packages/vue-sfc-loader/package.json` (peer) |
...
| Package | Version(s) | Files importing | Declared in |
|---|---|---:|---|
| `boardgame.io` | ^0.50.0 | 59 _(comprehensive)_ | `apps/arena-client/package.json` (dep); `apps/server/package.json` (dep); `packages/game-engine/package.json` (dep) |
...
| Package | Version(s) | Files importing | Declared in |
|---|---|---:|---|
| `pg` | ^8.13.0 | 53 _(comprehensive)_ | `apps/server/package.json` (dep) |
...
| Package | Version(s) | Files importing | Declared in |
|---|---|---:|---|
| `@vue/test-utils` | ^2.4.6 | 75 _(comprehensive)_ | `apps/arena-client/package.json` (dev); `packages/vue-sfc-loader/package.json` (dev) |
...
| `@legendary-arena/game-engine` | workspace:* | 181 _(comprehensive)_ | `apps/arena-client/package.json` (dev); `apps/engine-runner/package.json` (dep); `apps/replay-producer/package.json` (dep); `apps/server/package.json` (dep); `package.json` (dev); `packages/preplan/package.json` (peer) |
...
| `koa-body` | ^5.0.0 | 18 _(comprehensive)_ | `apps/server/package.json` (dep) |
...
|---|---|---|---:|
| `boardgame.io` | ^0.50.0 | direct dep — `apps/arena-client/package.json`, `apps/server/package.json`, `packages/game-engine/package.json` | 59 _(comprehensive)_ |
| `pg` | ^8.13.0 | direct dep — `apps/server/package.json` | 53 _(comprehensive)_ |
...
| `pinia` | ^2.1.7 | direct dep — `apps/arena-client/package.json`, `apps/dashboard/package.json`, `apps/registry-viewer/package.json` | 61 _(comprehensive)_ |
...
| `vue` | ^3.4.27 | direct dep — `apps/arena-client/package.json`, `apps/dashboard/package.json`, `apps/legends-board/package.json`, `apps/registry-viewer/package.json`, `packages/vue-sfc-loader/package.json` | 226 _(comprehensive)_ |
...
| `@vue/test-utils` | ^2.4.6 | direct dep — `apps/arena-client/package.json`, `packages/vue-sfc-loader/package.json` | 75 _(comprehensive)_ |
...
Lockfile resolves **628** packages: **47** are direct dependencies declared in some `package.json`, **581** are transitive.
