...
**Giving an HQ Hero away (D-24335).** The `give-hq-hero-by-trait-to-current`
primitive — co2e Ultron's Fight *"Choose a [hc:tech] Hero from the HQ. Either KO
that Hero or choose a player to gain it."* — is the eighteenth primitive: an
auto-resolve, predicate-parameterized (`{requireKind, requireValue}`, e.g.
`hc:tech`) effect that removes the **highest-cost** HQ Hero matching the trait
(ties → rightmost index, the `captureHeroFromHq` selection rule), refills the
vacated slot from the hero deck, and gives the Hero to the current (fighting)
player's **discard** (the "gain" routing, D-24327 — never the victory pile). The
printed **KO-or-gift** double-choice auto-resolves to the gift: in co-op a gift
gives a player a free Hero *and* refills the HQ, strictly dominating a KO (which
only refills), so a rational cooperative chooser never KOs — the KO branch is not
...
- WP-656 (D-24467) — Emma Frost's Diamond Form *"Whenever you defeat a Villain or Mastermind this turn, you get +3[icon:recruit]"* — a fidelity **over-fire** fix. The setup parser read the `[icon:recruit]` magnitude but not the "Whenever you defeat …" trigger prose, so it emitted a flat unconditional `onPlay recruit:3` (a free +3 Recruit on play, an over-powered card). Modeled the printed trigger through the existing D-24377 deferred-conditional-grant ("wait-and-see") path plus a new `defeatedVillainOrMastermindThisTurn` condition (a case in `evaluateCondition` / `describeFailedCondition`, listed in `WAIT_AND_SEE_CONDITION_TYPES`, runtime-drift-pinned). The shape is **edge-triggered per defeat**, not a sticky boolean: the `fightVillain` / `fightMastermind` success tails set a per-turn edge flag (gated on a pending grant existing), and `resolveDeferredHeroGrants` (already run from the play-phase `onMove` hook) **consumes** the flag each resolution **and re-arms** the fired grant — so it pays +3 *per* defeat and never leaks a +3 onto a subsequent non-defeat move. Mastermind "defeat" counts **per tactic** (each successful tactic fight, not only the final vanquish); Silent Sniper's *free* defeat is deliberately out of scope. Oracle-safe: the signal is consumed within the same move, so it never survives to a move-boundary hash point — both hash oracles byte-unchanged, **no re-pin**. Card data: a new `[keyword:defeated-villain-or-mastermind]` marker on `core.json` only (co2e's Diamond Form is a different Master-Strike-KO card; nmut's `assume-diamond-form` is a Mastermind tactic). The first of the operator-reported Emma Frost "cards not triggering" fixes.
- WP-659 (D-24470) — Emma Frost's Psychic Link *"Each player may reveal another X-Men Hero. Each player who does draws a card"* — a **parse-gate + unimplemented-body** fix, and the new `reveal-from-hand` hero keyword. The upstream text carries a **mid-sentence** `{team: 4}` describing the reveal target, but Step 1b of `heroAbility.setup.ts` extracted **every** `[team:X]` token as a `requiresTeam` play-gate, so the card wrongly blocked on "another X-Men Hero played this turn" *and* the reveal→draw mechanic was unimplemented — the card did nothing (live-observed: magneto-Midtown-Bank-Robbery 1p). Fixed via the Investigate marker→criterion precedent (D-24373), **not** a general parser rewrite: a new `[keyword:reveal-from-hand]` marker tells the parser the co-located `[team:X]` is the reveal **criterion**, not a play-gate (suppressed from Step 1a/1b exactly as size-changing / copy-powers / investigate lines suppress theirs). The new `reveal-from-hand` `HeroKeyword` + handler draws one card for each player (`Object.keys(playerZones).sort()`, seat order) whose hand holds a criterion-matching Hero — "another" needs no self-exclusion since the played Psychic Link has already left the hand. **Auto-reveal, no pending-choice park:** revealing a hand card is pure upside (co-op posture, leaks nothing), so declining is strictly dominated. No new `G` field (a stateless hand-read + draw), so both hash oracles are byte-unchanged. `core.json` marker only. The second Emma Frost fix.
