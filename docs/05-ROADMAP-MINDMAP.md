  ...
  root((Legendary Arena))
    ["Multiplayer Deck-Builder\nboardgame.io + TypeScript + R2"]
      ...
      Hero Ability Coverage & Markup Pipeline
        ...
        ["WP-356 📝 shuffle-discard-empty-reward hero keyword: Jocasta Reprocess + Electromagnetic Eyebeams (both antm Jocasta cards hollow — no effect marker; live-game diagnosis 2026-07-11 proved the shuffle never fires via the turn-13→14 deck cycle; one mandatory immediate keyword on the D-24029 substrate: D-24019-style 3-segment token [keyword:shuffle-discard-empty-reward:<recruit|attack>:<n>], seeded rewards exactly recruit/attack, descriptor reuses magnitude+rewardType so heroAbility.types.ts untouched; executor = empty discard → addResources grant, else combined deterministic discard→deck shuffle via ShuffleProvider + G.messages both branches; no pending choice/new move/projection; 2 marker rows + VALID_TOKEN_PATTERN + regenerated antm.json + hero ledger; locked 1877/438→1888/441; optional-shuffle variants dead/ssw2 + Flying Steed deferred; drafted 2026-07-11, READY; EC-386 at execution-prep; D-24148)"]
      ...
      Friends & Ranked Trust (2026-07)
        ...
        ["WP-358 📝 Match friend-invite (server) — a seated player invites an accepted friend by @handle → legendary.match_invites record + fail-open notifyMatchInvite email; accept returns matchId and the client joins via the existing POST /api/match/join (no server-side bgio join); friends-only by design (getFriendshipStatus==='accepted') so anti-spam + block-respecting by construction; 4 authenticated endpoints, MatchInviteView never exposes accountId; lobby-invite half of packet #5; drafted 2026-07-11 READY; EC-388; D-24150"]
        ["WP-359 📝 Friend-email opt-out toggle (arena client) — a ?route=me checkbox reading/writing WP-357's friendRequestEmails via the existing owner-profile fetch/save (one ref + one PATCH field + one checkbox); client OwnerProfileView/Patch mirror gains the field; no new endpoint/composable; client follow-on to WP-357; drafted 2026-07-11; EC-389; D-24151"]
...
| Hero Ability Coverage & Markup Pipeline | 45/46 | 1 open |
...
| Friends & Ranked Trust (2026-07) | 7/10 | 3 open |
| Next Horizons | 0/2 | 2 📦 queued |
| Phase 10 — Debugging, Testing & Troubleshooting | 0/8 | 8 📝 placeholders |
| Governance Drafts | 2/3 | 1 ⏸ |
| **Total** | **347/353 WP ✅** (+ 4/4 Foundation Prompts) | 1 ⏸, 5 open |

**Open / blocked WPs (derived from WORK_INDEX, 6):** WP-042.1 ⏸ blocked; WP-345 open; WP-358 open; WP-359 open; WP-360 open; WP-356 open.
<!-- ROADMAP-COUNTS:END -->
