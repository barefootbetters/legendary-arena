  ...
  root((Legendary Arena))
    ["Multiplayer Deck-Builder\nboardgame.io + TypeScript + R2"]
      ...
      Friends & Ranked Trust (2026-07)
        ...
        ["WP-371 📝 Draft — Match-create player-count composition gate (server) + lobby surface (arena client) — POST /api/match/create validates villain-group/henchman/hero lengths vs numPlayers via the WP-370 PLAYER_COUNT_SETUP table (server imports @legendary-arena/registry), returns 400 PLAYER_COUNT_COMPOSITION_MISMATCH + creates no match on mismatch (parity with the engine block, drift-tested); the lobby (LobbyView submitCreate/submitFromJson) surfaces it inline + optional Create-disable; arena-client imports no registry table (boundary — server error only); api-endpoints create row replaced whole (D-11804); unblocked now WP-370 ✅ done; reserves D-24167"]
        ["WP-372 📝 Draft — Loadout builder: player-count required counts + warn/export-gate (registry viewer) — the cards.legendary-arena.com Loadout tab reads PLAYER_COUNT_SETUP (imported from @legendary-arena/registry; no re-typed literals), shows a required heroes/villain-groups/henchmen/villain-deck-bystanders readout for the player count, warns per mismatch (mirrors the useLoadoutDraft missingRequiredVillainGroupIds warn/block pattern), and disables LAGN export while any mismatch is present (the warn-in-builder half of D-24165); authoring stays free (heroes chosen, never auto-filled); no game-engine import; unblocked now WP-370 ✅ done; consumes D-24165 (no new D-entry)"]
...
| Friends & Ranked Trust (2026-07) | 13/15 | 2 open |
| Next Horizons | 0/2 | 2 📦 queued |
| Phase 10 — Debugging, Testing & Troubleshooting | 0/8 | 8 📝 placeholders |
| Governance Drafts | 2/3 | 1 ⏸ |
| **Total** | **359/364 WP ✅** (+ 4/4 Foundation Prompts) | 1 ⏸, 4 open |

**Open / blocked WPs (derived from WORK_INDEX, 5):** WP-042.1 ⏸ blocked; WP-349 open; WP-345 open; WP-371 open; WP-372 open.
<!-- ROADMAP-COUNTS:END -->
