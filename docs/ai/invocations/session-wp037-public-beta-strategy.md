...
4. [docs/ai/ARCHITECTURE.md](../ARCHITECTURE.md) §Section 3 — The Three Data Classes (beta feedback is transport metadata; `G` is Class 1 runtime and is never deployed; beta release artifacts are Class 2 Configuration — same as production)
5. [docs/ai/ARCHITECTURE.md](../ARCHITECTURE.md) §Layer Boundary (Authoritative) — server layer owns feedback collection wiring; engine is never aware of beta users
6. [docs/ai/ARCHITECTURE.md](../ARCHITECTURE.md) §MVP Gameplay Invariants — beta games run the same deterministic engine; "Moves & Determinism" applies verbatim
7. [docs/ai/REFERENCE/02-CODE-CATEGORIES.md](../REFERENCE/02-CODE-CATEGORIES.md) §`engine` — new `src/beta/` subdirectory must be in the engine list (D-3701, landed at `a4f5574`)
8. [docs/ai/execution-checklists/EC-037-beta-strategy.checklist.md](../execution-checklists/EC-037-beta-strategy.checklist.md) — primary execution authority (Locked Values + Guardrails + Required `// why:` Comments + Files to Produce)
9. [docs/ai/work-packets/WP-037-public-beta-strategy.md](../work-packets/WP-037-public-beta-strategy.md) — authoritative WP specification
...
11. [docs/ops/RELEASE_CHECKLIST.md](../../ops/RELEASE_CHECKLIST.md) — beta uses the same release gates as production (WP-035). No beta-specific shortcuts.
12. [packages/game-engine/src/index.ts](../../../packages/game-engine/src/index.ts) — the export surface WP-037 appends to (after the WP-036 simulation block)
13. [packages/game-engine/src/ops/ops.types.ts](../../../packages/game-engine/src/ops/ops.types.ts) — WP-035 reference for engine-category pure-type files (WP-037's `beta.types.ts` mirrors the module-header discipline and re-export pattern)
14. [packages/game-engine/src/simulation/ai.types.ts](../../../packages/game-engine/src/simulation/ai.types.ts) — WP-036 reference for engine-category pure-type files with narrow unions
