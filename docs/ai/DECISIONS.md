...
### D-12001 — Loadout Draft State Ownership: App.vue Hoist + Props (Option A)

**Decision:** `apps/registry-viewer/src/App.vue` is the single owner of the page-level `useLoadoutDraft(registry)` instance. The composable is invoked exactly once inside `App.vue`'s `onMounted` block, alongside the existing `useSetupFromUrl(reg)` invocation. The resulting `UseLoadoutDraftApi` is held in a top-level `shallowRef<UseLoadoutDraftApi | null>(null)` and passed to `<LoadoutBuilder>` as a `:draftApi` prop. `<LoadoutPreview>` does not call `useLoadoutDraft` at all; its "Edit this loadout" button emits a `request-edit` event carrying the synthesized preview JSON, which `App.vue`'s `onPreviewRequestEdit` handler routes through `loadFromJson` on the hoisted draft. The result is surfaced via a `previewEditAcknowledgement: Ref<"idle" | "loaded" | "rejected">` passed back as a prop to `<LoadoutPreview>` for the existing success/error display.

**Rationale:** This decision restores a single-owner mental model for draft state, eliminating the dual-draft divergence shipped by WP-114 (each consuming component instantiated its own non-singleton draft, so "Edit this loadout" mutated an invisible second instance instead of the visible editor below). Single ownership matches user expectation: clicking "edit" populates the editor the user is looking at. The Vue auto-unwrap subtlety is documented inline (`shallowRef`, not `ref` — see Implementation Notes below).

This pattern mirrors the existing `useSetupFromUrl(reg)` hoist that WP-114 already established under EC-116 §Locked Values "Composable ownership". Mirroring it preserves architectural consistency, requires zero changes to `useLoadoutDraft.ts` (preserving the WP-091 PS-1 immutable signature lock), uses only plain Vue props + emits (no router, satisfying WP-117 D-11702), and keeps the prop surface narrow (one prop into `<LoadoutBuilder>`, one event + one acknowledgement prop with `<LoadoutPreview>`). Two-level prop drilling is acceptable at this depth and avoids the global-state pitfalls of provide/inject for this small surface.

**Implementation Notes:**
- **Use `shallowRef`, not `ref`, for the hoisted API container.** `useLoadoutDraft` returns an object whose properties are themselves Refs (`draft: Ref<MatchSetupDocument>`, `errors: ComputedRef<...>`, `isValid: ComputedRef<boolean>`). A regular `ref()` deep-reactifies its value via `reactive()`, which auto-unwraps nested refs on property access — so `loadoutDraftApi.value.draft` would silently become a plain `MatchSetupDocument` instead of `Ref<MatchSetupDocument>`. `<LoadoutBuilder>`'s destructure (`const { draft } = props.draftApi`) would then bind `draft` to a plain object, and the template's `draft.value.composition.…` would throw at render time. Vue swallows the error and the entire Loadout tab subtree renders blank. **Build + automated tests do NOT catch this; only the manual smoke does.** This trap is recorded as the first Common Failure Smell in `EC-121-loadout-preview-roundtrip-fix.checklist.md` so future composable-hoist work can avoid it.
- The `request-edit` event fires at most once per click. No auto-promotion on URL arrival, no debounce, no implicit fire on validation-state change — the user clicks the button, the event emits once, the parent calls `loadFromJson` once.
- `<LoadoutPreview>` continues to import zero mutators from `useLoadoutDraft` (read-only-by-default posture from EC-116 §Guardrails #4/#5 preserved); the only mutation surface is the user-initiated emit.

**Rejected:**
- **Option B — Module-level singleton inside `useLoadoutDraft.ts`.** Would require modifying `useLoadoutDraft.ts` to store its returned API in module scope. This unlocks WP-091 PS-1 (immutable signature lock) and would force a re-evaluation of every existing call site. Singleton state in a Vue composable is also an anti-pattern that interferes with future testability and test isolation. The lock exists precisely to prevent this kind of drift; reversing it would require a separate governance WP plus a DECISIONS supersession of WP-091 PS-1 — a much larger lift than this fix warrants.
- **Option C — Vue provide/inject.** Would hide the dependency relationship between `App.vue` and the two children behind a string injection key, making the data flow harder to trace at the call site. Provide/inject pays off when the consumer tree is deep (3+ levels) or branchy (many distinct consumers); here the tree is two levels with two consumers, both already direct children of `App.vue`. Props are more explicit, more boring, and a junior developer can follow them without learning an additional Vue mechanism.

**Downstream Surface Note (WP-120 Smoke 1).** The round-trip mechanism this decision establishes is mechanically verified, but the success path of `loadFromJson` from a URL-derived preview cannot currently complete because `setupContract.schema.ts:12`'s `EXT_ID_PATTERN = /^[a-z0-9-]+$/` rejects the slash-containing set-qualified IDs that WP-113 (D-10014) locked as the canonical format. WP-114's `setupUrlParams.ts` deliberately round-trips set-qualified IDs ("URLSearchParams handles forward slashes in the WP-113 set-qualified ID format cleanly through standard URL encoding"). This pre-existing validator drift was latent because WP-114's "Edit this loadout" populated an invisible second draft and never visibly succeeded or failed; WP-120 made the path actually run end-to-end and exposed the drift. A follow-up task chip was spawned 2026-04-30 to align `EXT_ID_PATTERN` with the set-qualified format. Once that lands, WP-120's Smoke 1 success path becomes user-observable.

**Introduced:** WP-120 (Commit A `EC-121:` at `05d5ded` 2026-04-30; Commit B `SPEC:` governance close 2026-04-30)
**Reinforces:** WP-091 PS-1 (`useLoadoutDraft.ts` immutable signature — preserved verbatim by hoist approach); WP-114 EC-116 §Locked Values "Composable ownership" (the precedent pattern for `useSetupFromUrl`); WP-117 D-11702 (no router for `apps/registry-viewer` — fix uses plain Vue props + emits); D-10014 (set-qualified `<setAbbr>/<slug>` ID format — preserved as the validator-alignment target for the follow-up task)
**Status:** Active
