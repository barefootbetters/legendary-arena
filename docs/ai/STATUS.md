...
### WP-054 / EC-054 Executed — Public Leaderboards & Read-Only Web Access (2026-04-26, EC-054)

📊 **WP-054 complete — verified competitive results are now publicly readable through a library surface; HTTP wiring deferred.** Three exported async functions in a brand-new `apps/server/src/leaderboards/` directory project read-only views over `legendary.competitive_scores`: `getScenarioLeaderboard(options, database, deps?)` returns a paginated scenario-scoped leaderboard with PAR-gate fail-closed via injected `LeaderboardDependencies.checkParPublished`; `getPublicScoreByReplayHash(replayHash, database)` returns a single permalink entry keyed on cryptographic SHA-256 (D-5401); `listScenarioKeys(database)` returns the alphabetically sorted set of scenario keys with at least one publicly visible (`'link'` or `'public'`) verified score. Every read uses `INNER JOIN legendary.players` for the display name and `INNER JOIN legendary.replay_ownership` for the visibility filter — no `LEFT JOIN ... COALESCE` fallback (D-5403, defense-in-depth: the schema's `display_name text NOT NULL` is not the load-bearing safety guard; the `INNER JOIN` is). Sort order is `cs.final_score ASC, cs.created_at ASC` verbatim (deterministic; ties broken by earliest submission). `totalEligibleEntries` is computed via a parallel `COUNT(*)` with the same `WHERE` + `INNER JOIN`s — never an unfiltered table count. `rank` is computed in TypeScript as `options.offset + i + 1` per row so it reflects global position within eligible results, not page-local index.

**Scope.** Three files in Commit A `e50e93e` (1285 insertions): `apps/server/src/leaderboards/leaderboard.types.ts` (interfaces only — `PublicLeaderboardEntry` 9 readonly fields per the locked key set, `ScenarioLeaderboard` `{ scenarioKey, entries: readonly PublicLeaderboardEntry[], totalEligibleEntries }`, `LeaderboardQueryOptions` `{ scenarioKey, limit, offset }`, `LeaderboardDependencies` DI seam mirroring WP-053's `SubmissionDependencies` precedent, internal `ParGateHit` re-statement matching the post-WP-053a JSDoc shape verbatim, type-only `DatabaseClient` re-export from `../identity/identity.types.js`); `apps/server/src/leaderboards/leaderboard.logic.ts` (the three exported async functions plus `PRODUCTION_DEPENDENCIES.checkParPublished = () => null` fail-closed default, private `mapRowToEntry(row, rank)` helper with explicit fresh-literal construction per the WP-051 aliasing-guard precedent, all SQL inline-concatenated with the locked column lists / `INNER JOIN`s / visibility filters / sort order / LIMIT-OFFSET pagination); `apps/server/src/leaderboards/leaderboard.logic.test.ts` (9 tests in one `describe('leaderboard logic (WP-054)', …)` block — 8 DB-dependent (#1-#8) using the locked WP-052 §3.1 inline `hasTestDatabase ? {} : { skip: 'requires test database' }` skip pattern + 1 logic-pure drift-detection test #9 that always runs and asserts `Object.keys(entry).sort()` equals the locked 9-key set verbatim plus `!(forbiddenKey in entry)` for every never-expose key, with forbidden keys constructed via array-join to keep the sensitive-field grep clean against the test file; fixture seeding via `submitCompetitiveScoreImpl` from WP-053 — no synthetic write-side fixtures against `legendary.competitive_scores`; `escapedVillains` counter varies per state to produce distinct hashes AND distinct engine-derived `finalScore`s for ordering / tie-break / pagination assertions; `TRUNCATE TABLE … RESTART IDENTITY CASCADE` for `beforeEach` cleanup, replacing the WP-053 `DELETE FROM` precedent because the WP-054 no-write grep gate is run against `*.ts` including test files). Zero new database tables, zero new migration files, zero modifications to any file outside the new directory.

**Test baseline shift.** apps/server `47 / 7 / 0` → **`56 / 8 / 0`** (+9 tests / +1 suite; with 24 skipped if no test DB: 16 inherited from WP-052 + WP-053 + WP-103 + 8 of WP-054's 8 DB-dependent tests; test #9 always runs). Engine baseline `522 / 116 / 0` (post-WP-053a) unchanged; zero existing tests modified. Note: the EC's `+8 / 55` arithmetic was off by one against the explicit 9-test plan; the implementation honors the explicit plan and yields `+9 / 56` per post-mortem §Arithmetic note.

**Library surface only — HTTP wiring deferred.** Per the locked Lifecycle Prohibition: none of the three leaderboard helpers are called from `game.ts` / phase hooks / `server.mjs` / `index.mjs` / `apps/arena-client/` / `apps/replay-producer/` / `apps/registry-viewer/` / any `packages/**` package, or any sibling server domain (`competition`, `identity`, `par`, `replay`). The future request-handler WP that publishes these helpers as HTTP endpoints will own: stateless IP-based rate limiting (D-5402 — explicitly deferred from WP-054 per pre-flight SR-1 disposition); bound `parGate.checkParPublished` injection from server startup (will spread-extend `PRODUCTION_DEPENDENCIES` rather than mutate it); HTTP serialization, content-type negotiation, and CORS; cache-control headers (the helpers themselves never cache). `PRODUCTION_DEPENDENCIES.checkParPublished` defaults to `() => null` (fail-closed) — every `getScenarioLeaderboard` call against this default returns an empty leaderboard until that wiring lands; the lifecycle prohibition makes this safe (no production caller exists today; the only consumer is the colocated test file which passes its own stub `deps`).

**Trust surface.** `PublicLeaderboardEntry` exposes exactly the 9 public-safe fields (`createdAt`, `finalScore`, `parVersion`, `playerDisplayName`, `rank`, `rawScore`, `replayHash`, `scenarioKey`, `scoringConfigVersion`) and excludes the 7 never-expose fields from WP-053's `CompetitiveScoreRecord` 11-key set by construction (the sequential bigserial primary key, the D-5201 server-identity correlation handle, the three identity-binding fields the account record carries, the redundant audit-provenance hash that equals `replayHash` by construction, and the owner-only score-breakdown audit detail). Drift-detection test #9 pins this contract via `Object.keys(entry).sort()` against a literal — adding a future field without updating the test fails loudly. The permalink lookup (`getPublicScoreByReplayHash`) is keyed exclusively on `replayHash` (cryptographic SHA-256, unguessable, deterministic), never `submissionId` (sequential `bigserial`, enables enumeration) — D-5401 cites the WP-052 UUID-v4 enumeration-attack rationale at `apps/server/src/identity/identity.types.ts:40` as precedent.

**Layer boundary held.** Production code (`leaderboard.types.ts`, `leaderboard.logic.ts`) imports nothing from `boardgame.io`, `@legendary-arena/registry`, `@legendary-arena/preplan`, `@legendary-arena/vue-sfc-loader`, or `apps/server/src/competition/competition.logic.ts`. The single permitted engine reference is `import type { ScenarioScoringConfig }` at `leaderboard.types.ts:26` (type-only — mirrors the WP-053 `competition.types.ts` precedent). The test file additionally imports the engine functions `computeStateHash` and `computeParScore` plus engine types for fixture computation, mirroring the WP-053 test-file precedent at `competition.logic.test.ts:33-56`. No scoring math anywhere in production code: `final_score` / `raw_score` / `par_version` / `scoring_config_version` values returned to a caller are the values WP-053 stored at submission time; nothing in this packet recomputes scoring or weights (server-as-projection, per D-5301-equivalent rationale).

**01.5 NOT INVOKED.** Zero `LegendaryGameState` field, zero `buildInitialGameState` shape change, zero new moves, zero new phase hooks; engine package entirely untouched. **01.6 post-mortem MANDATORY** per three triggers: new long-lived abstractions (`PublicLeaderboardEntry` / `ScenarioLeaderboard` / `LeaderboardQueryOptions` / `LeaderboardDependencies` consumed by future WPs); new contract consumed by future WPs (the public read API for the future HTTP wrapper); new DI seam pattern (extending WP-053's `SubmissionDependencies` precedent into a read-only PAR-gate consumption context). Delivered at `docs/ai/post-mortems/01.6-WP-054-public-leaderboards-read-only.md` covering all 14 mandatory audits (layer-boundary, engine immutability, AccountId-not-PlayerId per D-5201, sensitive-field exclusion, replayHash permalink, aliasing, determinism, server-as-projection (no scoring math), PAR-gate DI seam, no-write, no-transport, test-suite delta proof, drift-detection test #9, INNER JOIN defense-in-depth) plus the pre-commit review verdict embedded at §6 ("Safe to commit as-is"). Three-commit topology: A0 SPEC `f56a955` + `cbd4f7d` (WP-054 + EC-054 v1.3 + session-context bridge with post-pre-flight + post-copilot reconciliation); A `EC-054:` `e50e93e` (3 files; 1285 insertions); B `SPEC:` (this block + WORK_INDEX.md WP-054 `[ ]` → `[x]` + EC_INDEX.md EC-054 row added as Done + DECISIONS.md `D-5401` / `D-5402` / `D-5403` inline + 01.6 post-mortem). Staging by exact filename only — never `git add .` / `-A` / `-u` (P6-27). Unrelated `DESIGN-RANKING.md` working-tree edit + untracked `data/cards-combined.*` + `WP-100-*.md` + `WP-111-*.md` + `Combine-CardData.ps1` scratch were stashed/excluded from every WP-054 commit.

**Unblocks.** The future request-handler WP that publishes these helpers as HTTP endpoints (will own transport, content-type negotiation, CORS, cache-control, and stateless IP-based rate limiting per D-5402); future profile-page composition WPs (e.g., WP-102) that compose `listScenarioKeys` + `getScenarioLeaderboard` + `getPublicScoreByReplayHash` into per-account or per-scenario views; future tournament-aggregation WPs that consume the leaderboard projection without re-deriving SQL or cracking the storage layer open.
