...
### WP-100 / EC-100 Executed — Interactive Gameplay Surface (Click-to-Play UI Scaffold) (2026-04-26, EC-100)

🎮 **WP-100 complete — the arena client is now playable end-to-end through the browser.** Six new interactive components under `apps/arena-client/src/components/play/` (`HandRow`, `CityRow`, `HQRow`, `MastermindTile`, `TurnActionBar`, `PlayView`) wire the existing `UIState` projection (WP-089) and `submitMove` seam (WP-090) into a click-to-play surface. The active player can now click `Draw` (refills the hand to 6 cards), click any hand card to play it, click any City villain to fight, click any HQ hero to recruit, click the mastermind to defeat its top tactic, and click `End Turn` to pass control. Spectators and waiting players see the resulting state via the same `UIState` projection. Until WP-100, arena-client was a spectator: it could observe a server-driven match but had no way to drive a turn.

**Scope.** 14 files in Commit A `378729a`: 13 new files under `apps/arena-client/src/components/play/` (six `.vue` + six `.test.ts` + one shared `uiMoveName.types.ts` per Option B from the session prompt) plus a one-seam edit to `apps/arena-client/src/App.vue` that imports `PlayView`, exposes a stable `submitMove` closure bound to `liveClient.value`, and replaces `<ArenaHud />` with `<PlayView :submit-move="submitMove" />` in the live route only. The `LiveMatchView.vue` referenced in WP-100 §Scope G does not exist in the codebase; `App.vue` is the equivalent route holder, and the WP §Scope G "or the equivalent file introduced by WP-090; the WP author must locate and read the actual file before editing" clause explicitly anticipates this. Fixture and lobby routes are unchanged.

**Test baseline shift.** apps/arena-client `143 / 10 / 0` → **`169 / 16 / 0`** (+26 tests / +6 suites; within the WP §Test Expectations estimate range +18..+30). Each new test file uses a `describe('<Component> (WP-100)', () => …)` wrapper so the suite count rises as expected. All 26 tests assert the click → `submitMove(name, args)` tuple plus disabled-state derivation purely from `currentStage`. Engine, server, and registry baselines unchanged.

**Single runtime engine-import discipline preserved.** The arena-client's only runtime import of `@legendary-arena/game-engine` remains `apps/arena-client/src/client/bgioClient.ts:16` (`import { LegendaryGame }`). All six new components consume engine types via `import type` only. Verified by the §Verification Step 3 grep (no matches for non-`type` engine imports under `apps/arena-client/src/components/play/`).

**`UiMoveName` typed union.** A locally-defined six-member union at `apps/arena-client/src/components/play/uiMoveName.types.ts` mirrors the engine's six-name UI move vocabulary (`'drawCards' | 'playCard' | 'endTurn' | 'fightVillain' | 'recruitHero' | 'fightMastermind'`). Every component declares `submitMove: SubmitMove` where `SubmitMove = (name: UiMoveName, args: unknown) => void`. Move-name typos are caught at compile time. The `args` parameter remains `unknown` by design — the engine validates payload shape on receipt.

**Stage-only gating.** Cost data is not yet projected into `UIState` (`UICityCard`, `UIHQState.slots`, `UIMastermindState` carry no `fightCost` / `cost` field), so all six components apply stage-only gating: disabled state derives purely from `uiState.game.currentStage`. When a follow-up WP after WP-111 projects costs, the disabled expression extends to also disable when `economy.availableAttack` / `economy.availableRecruit` is below the projected cost. The intermediate state — interactive surface that ignores affordability — is correct for this scaffold because the engine itself silently no-ops on insufficient resources.

**Scaffold artifact: the `Draw` button.** The `Draw` button in `TurnActionBar.vue` emits `submitMove('drawCards', { count: 6 })` to compensate for the engine's MVP gap (no `turn.onBegin` auto-draw, no `HAND_SIZE` constant; `playerInit.ts` initializes hand to `[]`). Without it the game is unplayable in the browser. The button is decision-logged (D-10003) and labeled in a `// why:` comment as a deletion target — when a follow-up engine WP adds auto-draw, the button is REMOVED, not refactored.

**01.5 NOT INVOKED.** All four trigger criteria absent: zero `LegendaryGameState` field, zero `buildInitialGameState` shape change, zero new `LegendaryGame.moves` entry, zero new phase hooks. Engine package entirely untouched. **01.6 post-mortem MANDATORY** per three triggers: new long-lived abstraction (six interactive components are the canonical click-to-play surface that future arena-client gameplay WPs extend); new code subdirectory (`apps/arena-client/src/components/play/`); first interactive intent-emitting surface in arena-client beyond the lobby. Delivered at `docs/ai/post-mortems/01.6-WP-100-interactive-gameplay-surface.md`.

**Out of scope (deferred).** Card display fidelity (names, images, costs sourced from the registry) is queued as WP-111 (UIState card display projection — engine-side); a trivial follow-up UI WP after WP-111 binds the WP-100 components to `UICardDisplay` data. A11y polish (ARIA labels, keyboard focus management, screen-reader state announcements per Vision §17) is acknowledged-deferred to a follow-up WP before public-beta gating. Engine auto-draw + `HAND_SIZE` constant is a separate engine WP (not yet drafted) that retires the scaffold `Draw` button. Pre-plan UI integration (EC-059 contract surface) was not modified — the gameplay-time wiring is queued as WP-070 (Live Mutation Middleware — Pre-Plan ↔ Engine Disruption Wiring). Manual smoke test (two browsers, full turn end-to-end through a running dev server) is deferred to user verification — the executing session has no live server + browser environment available; flagged in the post-mortem §10 Notes for the user to run before promoting closure.
