...
### WP-120 Executed — Loadout Preview Round-Trip Fix (2026-04-30, EC-121)

🧰 **WP-120 complete — `<LoadoutPreview>`'s "Edit this loadout" button now populates the visible `<LoadoutBuilder>` instead of an invisible second draft instance.** Closes the WP-114 deferred UX caveat by hoisting the single `useLoadoutDraft(registry)` invocation from each component's local setup up to `App.vue`'s `onMounted` block, mirroring the existing `useSetupFromUrl` hoist pattern. `<LoadoutBuilder>` now accepts `draftApi: UseLoadoutDraftApi` as a prop and sources its destructured 22-name surface from the prop. `<LoadoutPreview>` emits a `request-edit` event carrying the synthesized preview JSON; `App.vue`'s `onPreviewRequestEdit` handler calls `loadFromJson` on the hoisted draft and surfaces the result via a `previewEditAcknowledgement` ref passed back as a prop. **Single owner of draft state**, single render, no more divergence.

**Decision landed.** One new entry in `docs/ai/DECISIONS.md`: **D-12001** (draft state ownership = Option A, hoist `useLoadoutDraft` to `App.vue` + pass API as props; rejected B singleton — would unlock WP-091 PS-1 immutable lock; rejected C provide/inject — less explicit than props at this depth).

**Vue auto-unwrap fix folded into Commit A** (mid-execution, surfaced by Smoke 1). `loadoutDraftApi` uses `shallowRef`, not `ref`. Reason: `ref()` deep-reactifies its value via `reactive()`, which auto-unwraps nested refs on property access — so `loadoutDraftApi.value.draft` would silently become a plain `MatchSetupDocument` instead of `Ref<MatchSetupDocument>`, breaking `<LoadoutBuilder>`'s destructure and rendering the entire Loadout tab blank. Build + automated tests do NOT catch this; only manual smoke does. Recorded inline as a `// why:` comment + as the first Common Failure Smell in EC-121 + as a sub-bullet under WP-120 §Scope (A).

**No engine, server, registry, preplan, or arena-client touch.** `git diff --name-only -- 'packages/**' 'apps/server/**' 'apps/arena-client/**' 'data/**'` empty. `useLoadoutDraft.ts` byte-identical (WP-091 PS-1 lock preserved). `setupUrlParams.ts` + `useSetupFromUrl.ts` byte-identical (WP-114 URL contract preserved). No `vue-router` added (WP-117 D-11702 preserved). No new dependency. No persistence layer. Test baseline `22 / 4 / 0` preserved.

**Manual smokes both PASS** (operator-recorded 2026-04-30). Smoke 1 (URL round-trip): the round-trip mechanism is mechanically verified — click "Edit this loadout" → `request-edit` event → `App.vue` handler → `loadFromJson` on the hoisted draft → validator → `previewEditAcknowledgement` updated → preview pane displays the result. Smoke 1 surfaced a separate, pre-existing validator drift between WP-113's locked set-qualified ID format (`<setAbbr>/<slug>`, D-10014) and `setupContract.schema.ts:12`'s `EXT_ID_PATTERN = /^[a-z0-9-]+$/` which rejects slashes; the success path of `loadFromJson` cannot complete with WP-114's URL contract until that validator is aligned. **Out-of-scope for WP-120** — flagged as a follow-up task chip 2026-04-30. Smoke 2 (no-URL non-stickiness): blank Builder on fresh load, manual edits do not persist across refresh, confirming the no-persistence contract from WP-091 + WP-114 is preserved by the hoist.

**Files modified.** Two-commit topology. Commit A (`EC-121:` at `05d5ded`): `apps/registry-viewer/src/App.vue` + `apps/registry-viewer/src/components/LoadoutBuilder.vue` + `apps/registry-viewer/src/components/LoadoutPreview.vue` + `docs/ai/work-packets/WP-120-loadout-preview-roundtrip-fix.md` (new) + `docs/ai/execution-checklists/EC-121-loadout-preview-roundtrip-fix.checklist.md` (new) + `docs/ai/work-packets/WORK_INDEX.md` (slot reservation) + `docs/ai/execution-checklists/EC_INDEX.md` (EC-121 row). Commit B (`SPEC:`): `docs/ai/STATUS.md` (this entry) + `docs/ai/DECISIONS.md` (D-12001) + `docs/ai/work-packets/WORK_INDEX.md` (`[ ]` → `[x]`) + `docs/ai/execution-checklists/EC_INDEX.md` (`Draft` → `Done`).

**Vision alignment.** §10a (Registry Viewer public surface) — preserved. The fix restores user-mental-model parity with runtime behavior on a public read-only surface; no monetization, no PvP framing, no scoring or leaderboards added. Conflict assertion: "No conflict". Determinism preservation: N/A (no engine / replay / RNG / PAR / simulation surface touched). Non-Goal proximity: NG-1..NG-7 not crossed. §20 Funding Surface Gate **N/A** with explicit justification: pure UX bug fix on the public registry viewer; no UI surfaces referencing donations, supporter tiers, or tournament funding are added or modified per WP-097 §A/§B/§C. §21 API Catalog Update **N/A** with explicit justification: no HTTP endpoints touched, no `apps/server/src/**` library functions added or modified.
...
**01.6 post-mortem OPTIONAL** per WP-066 / WP-094 / WP-096 / EC-103 viewer-side precedent (no new long-lived abstraction in the engine sense; the hoisted-instance pattern is a new instance of the existing `useSetupFromUrl` ownership pattern, not a new abstraction). Not authored.

**Downstream impact.** The follow-up task spawned 2026-04-30 will align `setupContract.schema.ts:12` `EXT_ID_PATTERN` with WP-113's set-qualified `<setAbbr>/<slug>` ID format (D-10014) so WP-114's URL contract can complete the round-trip end-to-end. Once that lands, WP-120 Smoke 1 success-path can be re-run and the `loadFromJson` success path through to a populated `<LoadoutBuilder>` becomes user-observable. The hoist pattern WP-120 establishes is the precedent for any future Vue 3 SPA composable that needs to be shared across sibling components when the composable's own signature is locked.
