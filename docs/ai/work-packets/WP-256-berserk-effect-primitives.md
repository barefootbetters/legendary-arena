...
The engine gains a reusable primitive substrate — node primitives (`sequence`, `move-card`, `gain-resource`), one value expression (`card-printed-stat`), and the transient `bind`/`ref` execution-context mechanism. **Berserk itself is data.** The mechanically-identical cousin — discard the top card, gain **Recruit** equal to its printed Recruit — is then `resource: 'recruit'` + `stat: 'recruit'`: data, **no engine change** (this WP proves it with a test, §Acceptance Criteria D).
...
- Dispatch MUST be **runtime-guarded before indexing** the ImplementationMaps. Even though `EffectNode`/`ValueExpression` are closed TypeScript unions, the AST is *data* and a regenerated/loaded artifact may be malformed. An unknown `node.type` or `amount.type` is handled by: (1) append a warning to `G.messages`; (2) skip the node / return `0`; (3) **never throw**. Mirror `executeSingleEffect`'s `if (handler === undefined) return;` guard — never call `EFFECT_NODE_HANDLERS[node.type](...)` / `VALUE_EXPRESSION_EVALUATORS[amount.type](...)` without first confirming the key resolves.
- **Warning emission is best-effort and non-throwing.** If `G.messages` is absent or not an array (a minimal test fixture), the interpreter still skips/defaults deterministically and MUST NOT throw while attempting to warn (a small local guard `if (Array.isArray(G.messages)) G.messages.push(...)`).
...
### I) Tests
- New `rules/effectPrimitive.test.ts` — drift (all closed arrays match their unions; the two registries match their ImplementationMap keys bidirectionally) + interpreter behavior (Berserk end-to-end: top card → discard + `+Attack` = its printed attack; the **Recruit-cousin AST** → `+Recruit`, proving the cousin is data; empty-deck no-op; missing-`cardStats` → +0; unknown node/value → warn-not-throw, including a malformed `type` that would crash a direct map index; **the context is never written to `G`**; **context isolation** — two separate `interpretHeroPrimitiveEffect` calls do not share bindings: a later top-level `ref` to an earlier `bind` resolves to `0` and warns, proving the context is per top-level evaluation, D-24029 §9).
- Modified `rules/heroAbility.setup.test.ts` — a `[keyword:Berserk]` ability → `hook.primitiveEffects` carries the Berserk composition, `hook.keywords` does NOT contain `berserk`, `HERO_KEYWORDS` still 17; **registry-clone** — mutating the parsed `hook.primitiveEffects[0]` does NOT mutate `HERO_COMPOSITION_MARKERS['berserk']` (deep-copy proof).
...
## Out of Scope
- **No new condition primitive and no bespoke X-Force logic.** This WP does not introduce a primitive-level `conditional`, `if-team`, selector, or count-scaling model. If an existing hero ability line already parses a non-Berserk condition through the current hook-condition system, the Berserk composition is executed only when that existing hook condition passes, because `primitiveEffects` run inside the same conditions gate as legacy effects. That is **reuse of the existing hook seam, not a new Berserk variant implementation** (and not a violation — the executor design intends it; see §Execution Clarifications Locked During Review).
- **No bespoke "+Recruit too" compound support beyond existing legacy effects.** If a card line already has a legacy recruit effect plus `[keyword:Berserk]`, the fixed execution order is legacy effects first, then primitive effects. This WP does not add a new compound composition marker or a second registry row.
...
- **No new primitive surface beyond Berserk's required substrate.** This WP seeds only three node types (`sequence`, `move-card`, `gain-resource`), one value-expression type (`card-printed-stat`), and the transient `bind`/`ref` execution-context mechanism. Future mechanics that need `draw-card`/`ko-card`/selectors/`conditional`/`for-each` earn their own primitive ceremony then, not now.
...
- [ ] Bindings are isolated per top-level primitive effect: a `ref` in a later top-level effect cannot read a binding created by an earlier top-level effect (resolves to `0` + warns).
- [ ] Parsed primitive effects are deep copies of registry compositions: a test proves the shared `HERO_COMPOSITION_MARKERS['berserk']` object is not mutated through a parsed hook.
- [ ] Runtime dispatch is guarded: a malformed `node.type`/`amount.type` warns + skips/defaults and does NOT throw (a direct ImplementationMap index would crash — the guard prevents it).
...
- [ ] `pnpm sim:coverage --check` OK after intentional baseline regeneration. The baseline diff is **manually inspected** and shows only the intended hero coverage movement: `berserk` leaves `unsupportedMechanics`; `wpnx` and `xmen` hero `noEffect` decrease; the corpus executable count rises accordingly; **no unrelated set/side/mechanic moves**; `[keyword:Berserks]` (plural) remains unsupported.
...
- **Operator review re-confirmation (2026-06-16):** Jeff's review tightened wording + criteria (the §Execution Clarifications below) — no new files, scope, or contract surface, only narrower/more-testable locks. Pre-flight READY + copilot PASS hold (the edits resolve the same RISKs the gates surfaced and add binary tests for the highest-risk invariants); no gate re-run required per `01.4 §Authorized Next Step` (updates do not change scope).

---

## Execution Clarifications Locked During Review

These clarifications are part of the execution contract and close the remaining ambiguity in the draft (folded in from operator review, 2026-06-16).

1. **Existing hook conditions may gate Berserk.** This WP does not add a primitive-level condition system, X-Force-specific logic, or a compound Berserk parser. If an existing hero ability line already parses non-Berserk conditions into the current hook-condition system, the Berserk composition runs only when those existing conditions pass — `primitiveEffects` execute inside the same conditions-passed block as legacy `effects`. That is reuse of the existing seam, **not** a new Berserk variant.
2. **Runtime dispatch is guarded.** Unknown node/value types are handled by a key-existence check *before* indexing `EFFECT_NODE_HANDLERS` / `VALUE_EXPRESSION_EVALUATORS`. Malformed AST data warns and skips/defaults; it never throws (closed TypeScript unions are compile-time only).
3. **Warning emission is non-throwing.** Warnings to `G.messages` are best-effort. A missing/unwritable `G.messages` (a minimal test fixture) must not turn a safe default into a thrown execution error.
4. **Bindings are per top-level interpretation only.** A binding created during one `interpretHeroPrimitiveEffect` call is not visible to a later call. Tests prove both: (a) the binding is never serialized into `G`; and (b) a later top-level `ref` cannot read an earlier top-level `bind`.
5. **Composition registry rows are never aliased into hooks.** The parser attaches a deep copy of `HERO_COMPOSITION_MARKERS['berserk']`; a test proves mutating the parsed hook effect does not mutate the shared registry object.
6. **Coverage regeneration is inspected, not rubber-stamped.** After `pnpm sim:coverage --update-baseline`, the baseline diff must show only the intended Berserk movement: `berserk` leaves unsupported mechanics, `wpnx`/`xmen` hero `noEffect` decrease, the executable count rises accordingly, no unrelated set/side/mechanic moves, and `[keyword:Berserks]` remains unsupported.
