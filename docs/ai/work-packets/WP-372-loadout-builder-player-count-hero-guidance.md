...
**Status:** Draft 2026-07-13 · **BLOCKED on WP-370** (needs `PLAYER_COUNT_SETUP` + the `setupContract` coupling) · **Standard two-session lane** (D-24028 — consumes a new registry contract surface; NOT lightweight only because it depends on the cross-layer WP-370 contract). Pairs with **EC** (authored at execution-prep). Reserves **no new D-entry** (consumes **D-24165** — the enforcement model "warn in builder, block at engine/server").
