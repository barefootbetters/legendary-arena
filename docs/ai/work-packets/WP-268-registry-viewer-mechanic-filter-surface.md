# WP-268 — Registry Viewer Hero Mechanic Filter Surface

**Status:** Draft (candidate — not yet reviewed, not ready to execute)
**Primary Layer:** Registry Viewer (`apps/registry-viewer`)
**Dependencies:** WP-267 (publishes `data/metadata/card-mechanics.json` to R2 and exports `CardMechanicsIndexSchema` from `@legendary-arena/registry/schema` — this WP consumes both; WP-267 must land first)

---

## Session Context

WP-267 (producer) lands a viewer-safe, normalized hero-mechanic feed at `/metadata/card-mechanics.json` (staged `data/metadata/card-mechanics.json`, published to R2) plus a `CardMechanicsIndexSchema` in the registry package. This WP (consumer) wires that feed into the registry-viewer so a user can **filter cards by hero mechanic** — the immediate user value the operator asked for ("query published hero mechanics in registry-viewer").
...
```bash
# A. WP-267 landed: the schema is exported from the viewer-safe subpath
node -e "const s=require('./packages/registry/dist/schema.js'); if(!s.CardMechanicsIndexSchema) process.exit(1); console.log('A_OK');"
# Expected: A_OK  (run `pnpm -r build` first; STOP if absent — WP-267 not landed)

# B. WP-267 landed: the staged feed exists and validates
...
If precondition A or B fails, **WP-267 has not landed** — this WP is BLOCKED; STOP and report.
...
- `packages/registry/src/schema.ts` `CardMechanicsIndexSchema` (from WP-267) — the validation contract; import via `@legendary-arena/registry/schema` (the barrel-avoidance subpath, per `cardTypesClient.ts:22–31`).
...
- Add `apps/registry-viewer/src/components/MechanicFilter.vue` — a multi-select chip ribbon listing only non-`hidden` mechanics (label + cardCount), emitting the selected slug set. Hidden when the mechanic list is empty (missing feed).
...
- Add an `applyFilters` (or equivalent App-level) test asserting mechanic filtering returns only matching cards and composes with a name query.

## Out of Scope

- Any producer-side change (transform, schema, `data/metadata/`, CI gate) — that is WP-267.
- Deriving mechanics from ability text at runtime — the per-card mapping in the feed is the sole source.
- Showing `hidden` mechanics in the default ribbon (they may exist in the feed for diagnostics but must not render as primary chips — realizes AC-7 at the UI layer).
- Villain/mastermind/scheme/henchman mechanics (the feed is hero-scoped; all-types is WP-269).
...
7 files (4 viewer + 3 governance). No new DECISIONS entry — this WP consumes the D-24044 contract WP-267 locks; it creates no new decision (the filter semantics mirror the existing ability-effect filter).
...
- `hidden: true` mechanics are excluded from the ribbon's default chip list (realizes **AC-7**).
...
**Session protocol:**
- If precondition A/B fails (schema or feed absent): STOP — WP-267 has not landed; this WP is BLOCKED.
- If the feed validates but `cards{}` is empty while `mechanics[]` is non-empty: STOP and report (producer join gap — a WP-267 defect, not something the consumer works around).
...
- Empty fallback: `{ version, scope, mechanics: [], cards: {} }` (non-blocking)
- Filter composition: OR-within-mechanics, AND-across-filters
- Hidden mechanics: excluded from the default ribbon
...
6. The ribbon renders only non-`hidden` mechanics; `hidden: true` entries never appear as default chips (**AC-7**).
...
```bash
# 1. No forbidden imports anywhere in the viewer's new/changed code
grep -rE "from\s+'[^']*(game-engine|apps/server|apps/dashboard|\.\./\.\./\.\./scripts)" apps/registry-viewer/src/lib/cardMechanicsClient.ts apps/registry-viewer/src/components/MechanicFilter.vue
# Expected: NO MATCH (exit 1)

# 2. Client mirrors the singleton pattern + uses the schema subpath
grep -F "@legendary-arena/registry/schema" apps/registry-viewer/src/lib/cardMechanicsClient.ts   # Expected: >=1
grep -F "card-mechanics.json" apps/registry-viewer/src/lib/cardMechanicsClient.ts                # Expected: >=1

# 3. Filtering uses the per-card mapping, not ability parsing
grep -F "parseAbilityText" apps/registry-viewer/src/components/MechanicFilter.vue apps/registry-viewer/src/App.vue | grep -i mechanic
# Expected: NO MATCH (mechanic filtering never parses ability text)
...
# 5. No producer-side files touched
git diff --name-only | grep -E '^(scripts/|packages/registry/|data/metadata/|\.github/)' ; echo "producer hits above (expect none)"

# 6. Working-tree scope
git diff --name-only | sort
# Expected: the 4 viewer files + 3 governance files only
```
...
- [ ] `docs/ai/STATUS.md` Done entry names WP-268 + the filter surface
- [ ] `docs/ai/DECISIONS.md` NOT updated — consumes D-24044; no new decision
- [ ] WORK_INDEX + EC_INDEX rows flipped to Done
- [ ] Commit prefix `EC-299:` for code, `SPEC:` for governance close

---

## Lint Gate Self-Review (00.3 — 21 sections)

Run 2026-06-20 against this WP + EC-299. Result: **PASS** (all sections PASS or justified N/A).
...
- **§3 Assumes** — PASS (preconditions A–F; A/B gate the WP-267 hard-dep with exact expected output).
- **§4 Context** — PASS (cardTypesClient template, AbilityEffectFilter template, applyQuery/applyFilters, the WP-267 schema, viewer CLAUDE.md + import rules, 00.6). 00.2 N/A — mechanic slugs are coverage tokens, not 00.2 fields.
