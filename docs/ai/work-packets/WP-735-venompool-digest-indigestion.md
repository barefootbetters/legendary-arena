...
8. `cards:check` / `effect-index:check` / `mechanics:metadata:check` / `ledger:heroes:check` / `sim:runtime-observed:check` / `sim:coverage --check` all exit 0; the Core-4 drop out of `runtime-observed-hollows.json` for the digest/indigestion mechanic (only the deferred `play-to-the-crowd` indigestion remains), and `sim:coverage` recognizes `digest-indigestion` as supported (no coverage regression); the branch effects (`carnivore`/`cauldron` draw, `digest-that-chimichanga` rescue) read `executable` in the hero mechanic ledger. **Amendment (execution):** the raw `digest` / `indigestion` mechanic rows stay `unsupported` in the hero mechanic ledger — the ledger aggregates status **per-hero**, and `venom` / `venompool` each carry BOTH a fused Core-4 Digest card AND a deferred one (`insatiable-hunger` / `play-to-the-crowd`), so a by-hook "executable" label would over-claim the deferred sibling (a reward-integrity violation). Under-claiming the raw token is the honest-conservative reading; the un-hollow is verified by the runtime-observed drop + `sim:coverage` recognition, not by the aggregated raw-token row. (Original AC text claimed the four cards read `digest-indigestion`/`executable`; corrected here — see govern-close.)
