# WP-267 — Hero Mechanic Metadata Feed (Contract + Transform + CI Gate)
...
- Reserve and land D-24044 (the published-contract shape + normalization + hidden-by-default lock).

## Out of Scope

- Any `apps/registry-viewer` file — the client, filter UI, and `applyFilters` wiring are **WP-268** (hard-dep'd on this WP).
- Any `apps/dashboard` file — the dashboard keeps its own build-time ledger copy; this feed does not replace it.
- Villain / mastermind / scheme / henchman mechanics — the ledger is hero-only; extending it is **WP-269** (queued). This feed's `scope` is the literal string `"hero"`.
...
- `docs/ai/DECISIONS.md` — **modified** (new D-24044)
...
- `scope` is the literal `"hero"`; do not emit other scopes (those are WP-269).
...
- [ ] `docs/ai/STATUS.md` Done entry names WP-267 + the feed
- [ ] `docs/ai/DECISIONS.md` D-24044 landed (contract + normalization + hidden-by-default lock)
- [ ] WORK_INDEX + EC_INDEX rows flipped to Done
- [ ] Commit prefix `EC-298:` for code, `SPEC:` for governance close

---

## Lint Gate Self-Review (00.3 — 21 sections)

Run 2026-06-20 against this WP + EC-298. Result: **PASS** (all sections PASS or justified N/A).
...
- **§15 Definition of Done** — PASS (STATUS + DECISIONS D-24044 + WORK_INDEX + EC_INDEX + scope-boundary check).
