...
- [x] **D-24026 live-verify — fire-payoff DEFERRED to the client-affordance WP (2026-09-22):** the fire path can only be triggered by a `useExcessiveViolence: true` fight, which has **no live surface today** — the arg is unwired anywhere in `apps/` (client affordance is the deferred sibling client WP) and autoplay bots never opt into the +1 overspend. The over-optimistic original note ("drive via the diagnostic / autoplay `setupData` path") was wrong: `setupData` configures the match, it cannot submit a move with EV on. So per D-24557 §A the fire-payoff live-verify is discharged to the "Fight using Excessive Violence" **client** WP, where the button is verified end-to-end. WP-736's obligation is met by: (a) the fire mechanic's exhaustive merged unit coverage (enroll-not-fire / fire-at-fight / once-per-turn / unaffordable-decline / debit-ordering, all green in #2258), and (b) the enroll-at-play + no-regression path being post-deploy smoke-verifiable (a bot vnom match completes without regression against the deployed `/api/version` gitSha).
