...
- Pure helper `deriveSweepTrendPoints(recentRuns)` → `readonly SweepTrendPoint[]` ordered **oldest → newest** via a **stable ascending sort on `submittedAtMs`** — NOT a bare `.reverse()` of the server's most-recent-first order (a raw reverse would invert tie order); the stable sort keeps equal-`submittedAtMs` runs in their `recentRuns` (most-recent-first) input order. `SweepTrendPoint = { runId, submittedAt, submittedAtMs, cadence, windowIndex, cellCount, healthRate }` where `submittedAtMs = Date.parse(submittedAt)` — `submittedAt` is the server's `toISOString()` ISO-8601 string (`sweep.logic.ts` `mapRowToSummary`), so the parse is spec-deterministic and NOT a wall-clock read — and `healthRate = computeSweepHealthRate(run)`.
...
- **Ordering (locked):** `deriveSweepTrendPoints` returns points oldest → newest via a **stable ascending sort on `submittedAtMs`** (NOT a bare `.reverse()`, which would invert tie order); identical `submittedAtMs` ties preserve the `recentRuns` most-recent-first input order.
- **Time axis (locked):** `submittedAtMs = Date.parse(submittedAt)` (pure, deterministic — `submittedAt` is an ISO-8601 `toISOString()` string); the chart consumes a consistent monotonic time axis; no reformatting beyond display labels.
