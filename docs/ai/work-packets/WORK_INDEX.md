...
- [ ] WP-163 — Autoplay Playback Controls (Server). **Drafted** 2026-05-19; **pending lint gate / human review**. Adds six REST endpoints `POST /api/match/:matchId/autoplay/{pause,resume,step-forward,step-back,restart,go-to-end}` driving media-player-style playback over the existing autoplay bot loop in `apps/server/src/autoplay/autoplay.mjs`. Introduces a per-match `PlaybackController` (module-level `Map<string, PlaybackController>` keyed by `matchId`) that gates the bot loop via `await waitIfPaused()` between moves and maintains a cursor-based history of up to 100 `structuredClone`'d `{ G, ctx }` snapshots (classified Class 1 Runtime State per `MOVE_LOG_FORMAT.md`; NOT a `MatchSnapshot`, `ReplayInput`, or `LogEntry`). Standardized `AutoplayControlResponse` envelope across all six endpoints; closed Endpoint Behavior Matrix locks exactly which endpoints return `uiState` (step-back and restart always; step-forward only on cursor move; pause/resume/go-to-end never). REST-only rewind delivery — no `transport.pubSub` for rewind. API Catalog whole-row replace per D-11804 (six new rows, `Status: Wired`, `Auth: guest`). Controller lifecycle bound to `runBotMatch` (created at match start, destroyed on every exit path — success, max-turn, error catch). New endpoints registered without `koaBody()` (bodyless POSTs). New `playbackController.mjs` is a pure helper — no `boardgame.io` import, no `Math.random()`, no persistence path. Hard-deps: none. Parallel-safe with WP-164 until WP-164 wires the endpoints. D-16301..D-16308. See [WP-163](WP-163-autoplay-playback-server.md) + [EC-180](../execution-checklists/EC-180-autoplay-playback-server.checklist.md).
- [ ] WP-164 — Autoplay Playback Controls (Client). **Drafted** 2026-05-19; **blocked: depends on WP-163**. Renders a media-player-style control bar (⏮ ⏪ ⏸/▶ ⏩ ⏭) on `apps/arena-client/src/pages/PlayDesktop.vue` when the URL carries an autoplay `match` query param. New playback service at `src/services/autoplayPlayback.ts` issues POSTs to the six WP-163 endpoints and, when a response contains `uiState`, injects it into `useUiStateStore` via the existing `setSnapshot` action (verified at `apps/arena-client/src/stores/uiState.ts:35`). Disabled-when state machine locked verbatim: step-back disabled when `cursor === 0` OR `!isPaused`; step-forward disabled when `!isPaused`; restart disabled when `!isPaused` OR `historyLength === 0`; go-to-end disabled when game is over; pause shown when `!isPaused`, resume otherwise. Single ingestion path for `uiState` — Socket.IO continues through the existing transport unchanged; any live broadcast overwrites the injected state unconditionally (no merge, no reconciliation). `uiState.ts` MUST NOT appear in `git diff` — reuse only. No new Pinia store. No `game-engine/setup` import (D-14401). Hard-dep: WP-163 (server endpoints). No new D-entries. See [WP-164](WP-164-autoplay-playback-client.md) + [EC-181](../execution-checklists/EC-181-autoplay-playback-client.checklist.md).
