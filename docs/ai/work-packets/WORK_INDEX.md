...
- [ ] WP-120 — Loadout Preview Round-Trip Fix (Hoist `useLoadoutDraft` to App.vue) — **DRAFT 2026-04-30** (lint-gate self-review PASS; pre-execution). Closes WP-114's deferred UX caveat where `<LoadoutPreview>`'s "Edit this loadout" button populates an invisible second draft instance instead of the visible `<LoadoutBuilder>`. Single decision **D-12001 = A** (hoist `useLoadoutDraft(registry)` invocation to `App.vue` + pass API as props; rejected B singleton — would unlock WP-091 PS-1 immutable lock; rejected C provide/inject — less explicit than props at this depth). Three Vue files modified: `App.vue` (single hoist + `request-edit` handler + `previewEditAcknowledgement` ref), `LoadoutBuilder.vue` (accept `:draft-api` prop, remove internal `useLoadoutDraft` call), `LoadoutPreview.vue` (emit `request-edit`, remove internal `useLoadoutDraft` call). `useLoadoutDraft.ts` byte-identical (PS-1 lock preserved); `setupUrlParams.ts` + `useSetupFromUrl.ts` byte-identical (WP-114 URL contract preserved). No router added (D-11702 preserved). No new dependency. No persistence. No engine, server, registry, or arena-client touch. §17 cited per §10a (Registry Viewer public surface) — preserved (NG-1..NG-7 not crossed). §20 N/A (no funding surface). §21 N/A (no HTTP endpoints touched). Test baseline `22 / 4 / 0` preserved (no Vue component testing infrastructure exists in repo; manual operator smokes per WP-114 §14 precedent cover the round-trip + non-stickiness invariants). Dependencies: WP-091, WP-114, WP-117. See [WP-120-loadout-preview-roundtrip-fix.md](WP-120-loadout-preview-roundtrip-fix.md).
