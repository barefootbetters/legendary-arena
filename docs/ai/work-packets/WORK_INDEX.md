...
- [ ] WP-114 — Registry Viewer URL-Parameterized Setup Preview ("Game of the Week") — Drafted 2026-04-28; lint-gate self-review **PASS**; EC-116 reserved (not yet drafted); pre-flight pending. **BLOCKED on WP-086** (hard sequencing — WP-086 touches the same component tree under `apps/registry-viewer/src/components/` plus `App.vue`; WP-114 lands after WP-086 to avoid merge churn on `LoadoutBuilder.vue` and `App.vue`). WP-086 is now Ready (promoted 2026-04-29; WP-084 dependency cleared 2026-04-21 at `b250bf1`).
