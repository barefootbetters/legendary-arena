...
- [ ] WP-114 — Registry Viewer URL-Parameterized Setup Preview ("Game of the Week") — Drafted 2026-04-28; lint-gate self-review **PASS**; EC-116 drafted; pre-flight + 01.7 copilot check at `docs/ai/invocations/preflight-wp114.md` (2026-04-29) — verdict DO NOT EXECUTE YET → **READY TO EXECUTE post-PS-1/2/3** (PS-1 BLOCKING export amendment to `useLoadoutDraft.ts`, PS-2 RISK extended forbidden-mutator gate, PS-3 BLOCKING `pnpm --filter` package-name fix; all three applied to WP + EC + source 2026-04-29). **WP-086 hard-sequencing prerequisite satisfied 2026-04-29 at `ccc6d0e` (Commit A `EC-086:`)** — same component tree (`LoadoutBuilder.vue`, `App.vue`) is now stable on `main`.
  ...
  Notes: Adds a read-only URL-driven setup-preview surface to the public registry viewer so a curated MATCH-SETUP can be shared as a single link. Five composition fields are URL-bound using the canonical 9-field names verbatim (`schemeId`, `mastermindId`, `villainGroupIds`, `henchmanGroupIds`, `heroDeckIds`); the four count fields and all envelope fields are deliberately default-only (sourced from `useLoadoutDraft.ts` constants — no re-declaration). Reuses `validateMatchSetupDocument()` against the loaded `CardRegistry`; unknown ext_ids surface as non-fatal "Unknown ext_id: <value>" lines. New component `LoadoutPreview.vue` is read-only (must not import any `useLoadoutDraft` mutator other than `loadFromJson`, which is invoked exactly once on user-initiated "Edit this loadout" click). New "Copy Setup Link" button on `LoadoutBuilder.vue` serializes `draft.value.composition` back into the same URL shape with a clipboard-fallback `<input readonly>` reveal. App.vue auto-switches to the Loadout tab on first mount when URL params are present (one-shot — does not re-fire on subsequent user tab navigation). Eight files projected (at the ≤8 cap; PS-1 added `useLoadoutDraft.ts` for the additive `export` amendment): `apps/registry-viewer/src/lib/setupUrlParams.ts` (+ `.test.ts`), `apps/registry-viewer/src/composables/useSetupFromUrl.ts` (+ `.test.ts`), `apps/registry-viewer/src/components/LoadoutPreview.vue`, `apps/registry-viewer/src/components/LoadoutBuilder.vue` (modified), `apps/registry-viewer/src/App.vue` (modified), `apps/registry-viewer/src/composables/useLoadoutDraft.ts` (modified — additive `export` keyword on six `DEFAULT_*` constants only, no logic change, per PS-1). No persistence (no `localStorage`, no `sessionStorage`, no IndexedDB, no cookies), no engine handoff, no server contact, no router library, no new npm dependency. §17 Vision Alignment: §10a (Registry Viewer public surface) — no conflict; NG-1..NG-7 proximity check confirms no monetization, no PvP framing, no scoring/leaderboards. §20 Funding Surface Gate: N/A — no funding affordances added or modified; preview surface is non-funding registry-viewer content per WP-097 §B (existing affordances unchanged). Four D-114XX entries projected at execution: canonical-name URL parameters; count/envelope-not-URL-bound rationale; one-shot auto-switch pattern; PS-1 additive `export` of six `DEFAULT_*` constants in `useLoadoutDraft.ts` (scope-neutral amendment to WP-091 immutable file — drift-test imports them). See [WP-114-registry-viewer-url-parameterized-setup-preview.md](WP-114-registry-viewer-url-parameterized-setup-preview.md).
...
- [ ] WP-117 — Client Routing Strategy (Architecture) — **STUB DRAFT 2026-04-29** (pre-lint, pre-pre-flight; governance-only WP). Locks per-app router posture for `apps/arena-client` and `apps/registry-viewer` (vue-router@4 vs no-router vs in-house) before further URL-bearing feature WPs relitigate. Three-to-four `[DECISION REQUIRED]` blocks unresolved (D-NNN01 arena-client, D-NNN02 registry-viewer, D-NNN03 history mode if router adopted, optional D-NNN04 replay-URL format). §17 N/A unless D-NNN04 opted in; §20 N/A. Forbidden alternatives: any router other than vue-router. Dependencies: WP-061. See [WP-117-client-routing-strategy.md](WP-117-client-routing-strategy.md).

- [ ] WP-118 — HTTP API Surface Catalog (Architecture) — **STUB DRAFT 2026-04-29** (pre-lint, pre-pre-flight; governance-only WP). Creates `docs/ai/REFERENCE/api-endpoints.md` as authoritative catalog; backfills shipped endpoints (lobby create/join, profile, handle, replay) and registers WP-115 leaderboard as Pending. Locks catalog format, error-response shape, versioning policy, and the update-obligation rule that future endpoint-bearing WPs must satisfy. Four `[DECISION REQUIRED]` blocks unresolved (D-NNN01..D-NNN04). Catalog is descriptive — code-vs-WP drift is recorded, not reconciled here. §17 triggered via #3 (player identity); §20 N/A. Dependencies: WP-011, WP-012, WP-101, WP-102, WP-103, WP-115. See [WP-118-http-api-surface-catalog.md](WP-118-http-api-surface-catalog.md).

- [ ] WP-119 — Architecture Doc Hygiene — **STUB DRAFT 2026-04-29** (pre-lint, pre-pre-flight; governance-only WP). Three small drift fixes bundled: (1) add `apps/replay-producer` (D-6301 / WP-063 shipped) to the System Layers diagram in `docs/02-ARCHITECTURE.md` and add a Package Boundaries row; (2) align preplan import-rule wording across `docs/ai/ARCHITECTURE.md`, `docs/02-ARCHITECTURE.md`, and `.claude/rules/architecture.md` (canonical phrasing TBD at lint time); (3) add `## Internationalization` section with deferred-i18n posture (English-only MVP, no library, cite Vision §17). One `D-NNN01` entry for i18n posture; diagram + wording fixes are pure cleanups. §17 triggered via #9 (i18n surface); §20 N/A. No code touched. No dependencies. See [WP-119-architecture-doc-hygiene.md](WP-119-architecture-doc-hygiene.md).
