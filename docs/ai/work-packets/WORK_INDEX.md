...
- [ ] WP-116 — Disconnect & Reconnect Semantics (Architecture) — **READY FOR EXECUTION (DRAFT v1.1, 2026-05-02).** Originally drafted 2026-04-29 as STUB; pre-flight 2026-04-30 surfaced 2 BLOCKING + 3 RECOMMENDED PS items, all 5 resolved in the same prep commit; all five (six) `[DECISION REQUIRED]` blocks resolved 2026-04-30 to **D-11601 = B (phase-aware grace window), D-11602 = B (pause match during `play.main` mid-turn drops), D-11603 = B (ready cleared on disconnect), D-11604 = A (hard timeout → match ends; replay emitted with `endReason: 'abandoned'`), D-11605 = A (replay always emitted with `endReason` discriminator), D-11606 = A (default-defer spectator behavior — preamble marker only, no DECISIONS entry)**. Lint self-review filled in pre-flight PS-4: 14 PASS / 6 N/A / 0 FAIL across §1-§20 of `00.3-prompt-lint-checklist.md`. Pre-flight verdict flipped to **READY 2026-04-30** on re-verdict; copilot check disposition flipped HOLD → CONFIRM (5 RISK findings resolved). Locks application-layer policy on top of WP-090's Socket.IO transport: rejoin grace window, turn-handover-on-drop during `play.main`, lobby ready-state on rejoin, mid-match abandonment threshold, replay-on-abort behavior. Concrete timer magnitudes intentionally NOT locked at this WP — class of policy only; future implementation WP gathers production telemetry. Governance-only WP (no code; produces architectural policy + DECISIONS entries). **No EC required** per D-10001 + 2026-04-26 Amendment (no `apps/` / `packages/` / `data/` files staged → `.githooks/commit-msg` Rule 5 not triggered → no EC stub required); commits use `SPEC:` prefix (never `EC-###:`). Six governance-core files in scope: `docs/ai/ARCHITECTURE.md` (new `## Disconnect & Reconnect Semantics` section with phase × event matrix — 4 phases × 3 events = 12 cells, no TBD placeholders), `docs/02-ARCHITECTURE.md` (mirror section after `## Transport`), `docs/ai/DECISIONS.md` (append D-11601..D-11605 + one-line D-11606 deferred-default preamble marker), `.claude/rules/architecture.md` (one-line pointer to new policy section), `docs/ai/STATUS.md` (one-line capability statement), `docs/ai/work-packets/WORK_INDEX.md` (this row checked off). **v1.1 refresh (2026-05-02)** folds knowledge from WP-115 / EC-119 closeout (commit `ab9bcce` 2026-05-01): D-11501..D-11506 now occupy the slot range immediately preceding D-11601..D-11606 (no collision risk); the D-11604 closed-set discriminator pattern has a stronger precedent (D-11804 replace-whole-row semantics applied at scale during WP-115 row transitions); D-11505 reinforces the "deferral despite available infrastructure" pattern WP-116's future implementation WP will inherit; the §Session Context multiplayer-feature roster is corrected (WP-115 ships read-only HTTP, not a live-match surface — does not need disconnect policy); the §Out of Scope unrelated-untracked-files example is refreshed (EC-119 has shipped, no longer "stub work"); the §Context (Read First) anchor at `apps/server/src/server.mjs` is refreshed to reflect WP-115's pool / route-register / SIGTERM closePool additions (none of which touch reconnect semantics). §17 Vision Alignment triggered (§17.1 #4 multiplayer reconnection — Vision §4 covenant; §17.1 #2 replays — Vision §22 / §24 from D-11605); §20 Funding Surface Gate N/A (governance-doc carve-out). Dependencies: WP-090 (Socket.IO transport shipped 2026-04-24). See [WP-116-disconnect-reconnect-semantics.md](WP-116-disconnect-reconnect-semantics.md).
