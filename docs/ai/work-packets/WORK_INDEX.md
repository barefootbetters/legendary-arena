...
- [x] WP-120 — Loadout Preview Round-Trip Fix (Hoist `useLoadoutDraft` to App.vue) — **Done 2026-04-30** (Commit A `05d5ded` `EC-121:` execution; Commit B `SPEC:` governance close). Closes the WP-114 deferred UX caveat where `<LoadoutPreview>`'s "Edit this loadout" button populated an invisible second draft instance instead of the visible `<LoadoutBuilder>`. Single decision **D-12001 = A** (hoist `useLoadoutDraft(registry)` invocation to `App.vue` + pass API as props; rejected B singleton — would unlock WP-091 PS-1 immutable lock; rejected C provide/inject — less explicit than props at this depth). Three Vue files modified: `App.vue` (single `shallowRef`-held hoist + `request-edit` handler + `previewEditAcknowledgement` ref), `LoadoutBuilder.vue` (accept `:draft-api` prop, remove internal `useLoadoutDraft` call), `LoadoutPreview.vue` (emit `request-edit`, remove internal `useLoadoutDraft` call). `useLoadoutDraft.ts` byte-identical (PS-1 lock preserved); `setupUrlParams.ts` + `useSetupFromUrl.ts` byte-identical (WP-114 URL contract preserved). No router added (D-11702 preserved). No new dependency. No persistence. No engine, server, registry, or arena-client touch. **Mid-execution finding folded into Commit A:** `loadoutDraftApi` uses `shallowRef`, not `ref`, because `ref()` deep-reactifies object values via `reactive()` which auto-unwraps nested refs — surfaced by Smoke 1 (Loadout tab rendered blank because `loadoutDraftApi.value.draft` became a plain object instead of a Ref, breaking `<LoadoutBuilder>`'s destructure). Build + automated tests do NOT catch this; only manual smoke does. Recorded inline as a `// why:` comment + first Common Failure Smell in EC-121 + sub-bullet under WP-120 §Scope (A). §17 cited per §10a (Registry Viewer public surface) — preserved (NG-1..NG-7 not crossed). §20 N/A (no funding surface). §21 N/A (no HTTP endpoints touched). Test baseline `22 / 4 / 0` preserved. Manual smokes both **PASS — operator-recorded 2026-04-30**: Smoke 1 round-trip mechanism mechanically verified (event → handler → `loadFromJson` → validator → acknowledgement display all wired correctly); Smoke 2 non-stickiness PASS (blank Builder on fresh load, no carryover after refresh). **Smoke 1 surfaced separate pre-existing validator drift** between WP-113 D-10014 set-qualified `<setAbbr>/<slug>` ID format and `setupContract.schema.ts:12`'s `EXT_ID_PATTERN = /^[a-z0-9-]+$/` (rejects slashes); the success path of `loadFromJson` cannot complete with WP-114's URL contract until that validator is aligned. Out-of-scope for WP-120; flagged as a follow-up task chip 2026-04-30. 01.5 NOT INVOKED (zero `LegendaryGameState` field, zero `buildInitialGameState` shape change, zero new `LegendaryGame.moves` entry, zero new phase hooks). 01.6 post-mortem OPTIONAL per WP-066 / WP-094 / WP-096 / EC-103 viewer-side precedent — not authored. Dependencies: WP-091 (`useLoadoutDraft.ts` 22-name API + PS-1 immutable lock), WP-114 (`useSetupFromUrl` precedent + `<LoadoutPreview>` "Edit this loadout" call site), WP-117 (D-11702 no-router posture). See [WP-120-loadout-preview-roundtrip-fix.md](WP-120-loadout-preview-roundtrip-fix.md) + [EC-121-loadout-preview-roundtrip-fix.checklist.md](../execution-checklists/EC-121-loadout-preview-roundtrip-fix.checklist.md) + D-12001.
