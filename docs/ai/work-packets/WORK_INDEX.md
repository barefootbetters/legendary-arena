...
- [ ] WP-102 — Public Player Profile Page (Read-Only) — Drafted 2026-04-25; lint-gate self-review PASS; pre-flight pending; staleness sweep 2026-04-28 (post-WP-101 baselines + migration slot `007` → `008` for the WP-101 handle migration + WP-053 contract-immutability rule added; EC slot retarget guidance from `EC-102` → **`EC-117`** per the viewer-series collision precedent).
