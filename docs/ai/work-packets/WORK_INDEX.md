...
- [ ] WP-196 — Dashboard Net Revenue + Paid-Action Errors Widgets. **Draft** 2026-05-31 + **harden round 1** 2026-05-31. Client-only (`apps/dashboard/`) follow-up to the WP-157 scaffold + WP-162 polish. Implements **WP-A** of the operator-dashboard pre-mortem grouping (drafted 2026-05-31; B = acquisition funnel, C = retention cohorts, D = public-surface health + general error monitor + cost watchdog, E = TAM-saturation + content-breadth — all not yet drafted). Adds two widgets on `/monetization`: **NetRevenueChartWidget** — stacked-bar 4-bucket decomposition (gross − royalty − Stripe fees − infra COGS) with `revenueDeductions.ts` static config (royalty/fees/COGS percentages ship as flagged-mock placeholders pending finance review); integer-cents arithmetic throughout (no floating-point dollars); per-day rounded values are the sole source of truth for aggregates (no recompute-from-aggregate, no ratio-into-money — D-19604); negative-net days preserved AND surfaced via tooltip label `"Negative net day"` + range-negative footer `(net loss)` qualifier (D-19606); operator interpretation footer renders single `"Net margin: X.X%"` label, informational only with no RYG (D-19606). **PaidActionErrorsWidget** — surfaces two billing-side health signals: Stripe webhook fulfillment failure rate (`legendary.stripe_events.process_error IS NOT NULL`) and Checkout intent abandonment rate (`legendary.stripe_checkout_sessions.intent_status IN ('expired', 'canceled')`); future server endpoint path `/metrics/billing/health` locked here AND its forward contract — response shape byte-compatible with `BillingHealth`, `0.0 ≤ rate ≤ 1.0`, `count = round(total × rate)`, `authenticated-session-required` + `finance`/`admin` role gate — so the follow-up server WP has zero schema ambiguity (D-19603 extension). Mock determinism contract: `seed = hash(range.start + '\|' + range.end)`, byte-identical output for identical input across calls / reloads / widgets, no bare `Math.random()` in services or composables (D-19605). Both widgets follow the WP-157 Widget Contract enforced via Widget State Semantics (loading = skeleton, error = full-sentence + no chart, empty = message + no chart, data = chart) and the WP-162 design-token / theme-toggle discipline. Required tests expanded from 7 → 9 (added aggregation-consistency + referential-stability). All data mock (`VITE_USE_MOCKS=true`); no server endpoints added; no schema changes; no `apps/server/` files touched; zero `@legendary-arena/*` imports. §17 PASS (Financial Sustainability + NG-1..NG-8 — internal admin/finance operator surface, no player-facing copy). §20 N/A with justification. §21 N/A with justification. **D-19601..D-19606 reserved**: D-19601 (4-bucket model + integer-cents), D-19602 (placeholder royalty %), D-19603 + extension (paid-action union + endpoint path + forward server contract), D-19604 (aggregation derivation + numerical integrity guard), D-19605 (mock determinism contract), D-19606 (operator interpretation hook + negative-net first-class signal). Hard-deps: WP-157 ✅, WP-162 ✅. ~5 new + ~4 modified files (single-layer Client work). EC-225 drafted 2026-06-01 (PS-1..PS-3 verbatim D-entry transcription gates; 9-test lock; comprehensive grep gates). See [WP-196](WP-196-dashboard-net-revenue-and-paid-action-errors.md) + [EC-225](../execution-checklists/EC-225-dashboard-net-revenue-and-paid-action-errors.checklist.md).

- [ ] WP-197 — Dashboard Live Deploy (Cloudflare Pages + Access Gate). **Draft** 2026-06-01 (EC-223). Server / Infra deploy + governance only — zero code edits to `apps/server/src/**` or `apps/dashboard/src/**`. Stands up `https://dashboard.legendary-arena.com` on a new CF Pages project `legendary-arena-dashboard` behind a Cloudflare Access Self-hosted Application (Email OTP, single-operator allow = `jeff@barefootbetters.com`), mirroring the `ewiki.` precedent (DOMAINS.md §ewiki) with CF Pages as the origin instead of Render. Ships in mock mode (`VITE_USE_MOCKS=true` on CF Pages Production env) so widgets render their 4-state shell without real data wiring; real-data wiring deferred to follow-up WPs (pre-mortem grouping WP-B/C/D/E or successors per D-19702). Flips the `dashboard` row in `docs/ops/domains.json` from `state: "planned"` to `"live"` only after `pnpm check:domains` reports `READY`. Locks the canonical **Gate-before-expose** rule (Sub-task C [CF Access app] strictly precedes Sub-task D [custom domain attach]; the window between attach and Access intercept is a public mock-login exposure window — the in-app login accepts any email + any role). Security Tripwire as a live invariant: pre-auth `200` = SEV-0 + immediate Rollback Procedure trigger (7 steps + 3-command verification). Deterministic 3-state Probe Lifecycle (`PENDING` → `READY` → `OK`); `state` flip is forbidden before `READY`. 4-axis drift-reconciliation in `After Completing` (manifest vs CF Pages project + DNS + Access app + `domains.json` row). PS-2.1 artifact-based sequencing enforcement per sub-task (A → B → C → D → E → F → G → H). DNS posture locked: proxied (orange-cloud) — Cloudflare Access cannot intercept gray-cloud DNS-only traffic. Build command locked to `pnpm install --frozen-lockfile && pnpm --filter "@legendary-arena/dashboard..." build` (trailing `...` per WP-144 / D-14401). `expectedStatus` is closed three-value set `[302, 401, 403]`; `expectedLocation` prefix is `https://legendary-arena.cloudflareaccess.com/`. D-19701 (CF Pages + Access posture, mirroring `ewiki.` precedent) + D-19702 (initial deploy ships mock-mode; real-data wiring deferred) reserved. Hard-deps: WP-157 ✅ (`bef03a8`), WP-162 ✅ (`54007cc`, PR #119). Soft-dep: Cloudflare Zero Trust plan active (last verified 2026-05-08 under `ewiki.` deploy; PS-1 re-verifies at execution time). 6 files (all governance + ops; zero code): `docs/ops/{domains.json,DOMAINS.md}` + `docs/ai/{DECISIONS,STATUS}.md` + `docs/ai/work-packets/WORK_INDEX.md` + `docs/ai/execution-checklists/EC_INDEX.md`. Pre-flight READY 2026-06-01; lint gate self-review 21/21 PASS (with §20 and §21 N/A justified). Operator actions (CF Pages project creation, CF Access application creation, custom domain attachment, DNS proxy posture, env var entry) happen in the Cloudflare dashboard and produce no repo diff. See [WP-197](WP-197-dashboard-live-deploy.md) + [EC-223](../execution-checklists/EC-223-dashboard-live-deploy.checklist.md).
