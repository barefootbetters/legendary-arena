...
- [ ] WP-151 — Physical Card Phase 3 — `HeroCardSchema.imageUrl` Removal + R2 Image Rename. **Draft.** Hard-dep: WP-141 ✅. Removes `imageUrl` from `HeroCardSchema`, updates both registry `flattenSet()` to source hero imageUrl from `physicalCards[]`, regenerates all 40 `data/cards/*.json`, renames R2 split-pair files to combined-name pattern. Closes the 44-unmatched-files surface from v16 migration. D-15101, D-15102. See [WP-151](WP-151-physical-card-phase-3-imageurl-removal.md).
