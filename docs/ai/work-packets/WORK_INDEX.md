...
- [x] WP-100 — Interactive Gameplay Surface (Click-to-Play UI Scaffold) — ✅ Done 2026-04-26 at Commit A `378729a` (preceded by governance-prep `7ff4006` and followed by Commit B governance close). Drafted 2026-04-26; 00.3 lint-gate self-review PASS; engine-source pre-review 2026-04-26 (move payloads + UIState paths grounded against `coreMoves.types.ts`, `fightVillain.ts`, `recruitHero.ts`, `fightMastermind.ts`, `uiState.types.ts`); engine draw-mechanics pre-review 2026-04-26 (no auto-draw — Draw button locked at `count: 6`); Q1 resolved via sibling WP-111 draft; pre-flight + copilot check verdict READY TO EXECUTE / CONFIRM (PS-1 / PS-2 / PS-3 all applied to WP-100 in-place 2026-04-26). Test baseline `143 / 10 / 0` → **`169 / 16 / 0`** (+26 tests / +6 suites — within the +18..+30 estimate range from §Test Expectations). Engine, server, registry packages unchanged. Single runtime engine-import discipline preserved (`bgioClient.ts:16` remains the sole importer). Stage-only gating throughout (cost data not yet projected into UIState; affordability gating extends with WP-111 follow-up). `LiveMatchView.vue` does not exist in the codebase; the live-route holder is `apps/arena-client/src/App.vue` (per WP-100 §Scope G "or the equivalent file" clause). 01.6 post-mortem at `docs/ai/post-mortems/01.6-WP-100-interactive-gameplay-surface.md` (three triggers: new long-lived abstraction, new code subdirectory `apps/arena-client/src/components/play/`, first interactive surface in arena-client beyond the lobby). Three D-100xx entries landed at Commit B: D-10003 (drawCards `count: 6` scaffold-artifact rationale), D-10004 (CardExtId-as-label decision; registry-projection deferred to WP-111), D-10005 (prop-drilled submitMove vs provide / inject — testability rationale).
