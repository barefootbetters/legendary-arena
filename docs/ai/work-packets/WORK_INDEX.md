...
- [x] WP-104 — Owner Profile Data Model & `/me` Edit — **Done 2026-05-02 (`EC-128:`).** Three authenticated-write owner-only HTTP endpoints under `/api/me/` registered on the existing boardgame.io Koa router via `registerOwnerProfileRoutes(server.router, pool, { requireAuthenticatedSession })`: `GET /api/me/profile` (read), `PATCH /api/me/profile` (sparse partial per D-10406 RFC 7396 with `Object.hasOwn` three-state discrimination), `PUT /api/me/links` (replace-all-by-list per D-10407 with 10-entry cap, single `BEGIN/COMMIT` transaction). New `legendary.player_profiles` table (1:1 with `legendary.players`, `ON DELETE CASCADE`) with three per-section closed-set privacy toggles defaulting to `'private'` per D-10403 + Vision §3 fail-closed posture. New `legendary.player_links` table (many-to-1, also `ON DELETE CASCADE`) with closed-set 6-entry provider allowlist per D-10404 (twitter / github / twitch / discord / youtube / website) and HTTPS-only any-host URL CHECK per D-10405. Read-no-mutate invariant preserved: `getOwnerProfile` synthesizes the most-private default view when no row exists; first PATCH owns row creation via `INSERT ... ON CONFLICT (player_id) DO UPDATE` upsert. `'unknown_account'` returns HTTP **401, not 403**, per the account-existence-probe defense locked in WP-104. WP-126 broker-specific `SessionVerifier` deferred — every authenticated request returns 500 with `code: 'session_verifier_not_configured'` per D-11204 fail-closed posture; arena-client `MyProfilePage.vue` surfaces the locked verbatim banner copy. Public profile (WP-102) byte-identical post-WP-104; future surface-integration WP joins owner-edit fields onto `PublicProfileView`. **Eight DECISIONS entries** land at execution: D-10401 (module path = extend `apps/server/src/profile/`), D-10402 (migration slot 009 single file), D-10403 (per-section closed-set privacy enum), D-10404 (6-entry provider allowlist), D-10405 (HTTPS-only any-host URL validation), D-10406 (sparse partial PATCH RFC 7396), D-10407 (replace-all-by-list PUT 10-entry cap), D-10408 (same-commit wiring per WP-115 precedent). Server baseline `pass 73 / fail 0 / skipped 36` → **`pass 82 / fail 0 / skipped 42`** (+9 logic-pure / +6 DB-required-skipped / +1 suite). Engine baseline `pass 604 / fail 0` UNCHANGED. No new npm dependencies. Fourteen files: 1 migration + 4 new server profile files + 1 server.mjs modification + 1 new arena-client API wrapper + 1 new Vue page + 1 App.vue modification + 2 reference docs (`api-endpoints.md` 3 new `Wired` rows per D-11804, `00.2-data-requirements.md §4.1` 2 new table inventory rows) + 4 governance ledgers (this row + STATUS.md / DECISIONS.md / EC_INDEX.md). 01.5 NOT INVOKED. 01.6 post-mortem SKIPPED with documented rationale (the new contract surfaces duplicate-then-extend WP-052 / WP-101 / WP-102 patterns line-for-line; no fundamentally new abstraction worth a dedicated post-mortem). Vision: §3 / §11 / §14 / §15 cited. NG-1 / NG-3 / NG-6 not crossed. Determinism N/A (no engine touch). §20 Funding Surface Gate N/A (owner identity / presentation surface). §21 API Catalog TRIGGERED (3 new `Wired` endpoints per D-11804). Commit prefix `EC-128:` at execution (never `WP-104:` per `01.3-commit-hygiene-under-ec-mode.md`). See [WP-104-owner-profile-data-model-and-me-edit.md](WP-104-owner-profile-data-model-and-me-edit.md) + [EC-128-owner-profile-data-model-and-me-edit.checklist.md](../execution-checklists/EC-128-owner-profile-data-model-and-me-edit.checklist.md) + D-10401 + D-10402 + D-10403 + D-10404 + D-10405 + D-10406 + D-10407 + D-10408. **Original draft notes (DRAFT 2026-05-02) preserved below for traceability.** Drafted 2026-05-02; lint-gate self-review **PASS** (§1–§21); EC-128 drafted 2026-05-02 (next free EC slot after EC-127 reserved WP-125 on 2026-05-01; WP-keyed slot retargeted EC-104 → EC-128 per the locked precedent EC-101 → EC-114, EC-103 → EC-111, EC-109 → EC-115, EC-121 → EC-122, EC-123 → EC-127; EC-104 is occupied by viewer-series). Splits the owner-edit half of the original WP-PLAYER-PROFILE proposal into its own scoped packet. Authorizing contracts: WP-102 (profile module + page conventions, landed 2026-04-28 at `EC-117:`); WP-112 (`requireAuthenticatedSession` orchestrator + caller-injected provider pattern, landed 2026-05-02 at `EC-112:`); WP-052 (identity model + `legendary.players`); WP-115 (long-lived `pg.Pool` lifecycle anchor + `register*Routes(...)` precedent). Architectural choices locked at draft time under **D-10401** (module path = extend `apps/server/src/profile/`, NOT new `apps/server/src/account/`) + **D-10402** (migration slot 009 — single file `009_create_player_profiles_and_links.sql` covering both new tables idempotently). Six further decisions surfaced as `[DECISION REQUIRED]` blocks for the executor: D-DEC-1 (privacy-toggle granularity — recommended default: per-section closed-set enum `'private'|'public'`); D-DEC-2 (`player_links.provider` validation — recommended default: small closed-set allowlist); D-DEC-3 (URL validation posture — recommended default: HTTPS-only any host for both `avatar_url` and `links.url`); D-DEC-4 (PATCH semantics — recommended default: sparse partial per RFC 7396); D-DEC-5 (PUT links semantics — recommended default: replace-all-by-list with 10-entry cap); D-DEC-6 (route-wiring posture — recommended default: same-commit wiring per the WP-115 precedent, NOT deferred). Scope (10 production / reference files + 4 governance ledgers = 14 total at session close, in line with WP-115 / WP-125 precedent): `data/migrations/009_create_player_profiles_and_links.sql` + `apps/server/src/profile/{ownerProfile.types.ts, ownerProfile.logic.ts, ownerProfile.logic.test.ts, ownerProfile.routes.ts}` + `apps/server/src/server.mjs` (modified — wire `registerOwnerProfileRoutes` per D-DEC-6 = (a) default; if (b), unmodified) + `apps/arena-client/src/lib/api/ownerProfileApi.ts` + `apps/arena-client/src/pages/MyProfilePage.vue` + `apps/arena-client/src/App.vue` (modified — extend `AppRoute` with `'me'`) + `docs/ai/REFERENCE/api-endpoints.md` (modified — adds 3 new rows per D-11804). MUST NOT modify WP-102 contract files (`profile.{types,logic,routes}.ts` + `profile.logic.test.ts` + `PlayerProfilePage.vue` + `profileApi.ts`); MUST NOT modify WP-052 / WP-112 / WP-115 contract files (`identity/`, `auth/`, `db/`, migrations 004-008); MUST NOT modify `.claude/rules/*.md`. Out-of-scope (deferred to dedicated WPs): avatar **upload** (URL-only here — WP-106 owns R2 pipeline); public-profile integration (joining owner fields onto `PublicProfileView` — separate future WP-104B amendment); badges (WP-105); integrity admin (WP-107+); payments (WP-108+); tournament identity (WP-109 extends WP-104's `legendary.player_profiles` table column-additively); friend graphs (`'friends'` privacy value deliberately excluded from D-DEC-1 closed set until a friend-graph WP lands); CSRF / cookie carriers (per D-11202 — bearer header only); rate limiting (per D-11503 — deferred to future hardening WP). Owner-only writes are gated by the WP-112 `requireAuthenticatedSession` orchestrator with fail-closed unconfigured-default per D-11204 — until WP-126 (broker-specific verifier) lands, every authenticated request returns 500 with `code: 'session_verifier_not_configured'`; the `MyProfilePage.vue` surfaces this gracefully via a "Authentication is not yet configured" banner. D-11804 catalog-update obligation triggered (3 new `Wired` rows per D-DEC-6 default, or `Library-only` if (b)). Vision Alignment: §3 (Player Trust & Fairness — fail-closed auth gate; most-private privacy defaults), §11 (Stateless Client Philosophy — server-authoritative re-render after every write), §14 (Explicit Decisions, No Silent Drift — six decisions surfaced + locked), §15 (Built for Contributors — caller-injected auth provider pattern admits any verifier), NG-1 / NG-3 / NG-6 — not crossed; determinism N/A (no engine touch). §20 Funding Surface Gate **N/A** with explicit justification (owner identity / presentation surface; no funding-flow surface introduced). Server test baseline `pass 73 / fail 0 / skipped 36` → projected post-WP-104 `pass 73+N / fail 0 / skipped 36+M` where N + M = 10–14 new tests (10–14 case target per WP-104 §Scope (In) §D). Engine baseline `604/132/0` UNCHANGED. Hard-deps: WP-052, WP-101 (`legendary.players` + handle precedent), WP-102, WP-112, WP-115. Soft-consumers: WP-104B (future surface-integration WP joining owner fields onto `PublicProfileView`); WP-105 (badges — display on the same edit page); WP-106 (avatar upload — replaces the manual-URL workflow); WP-109 (team affiliation extends `legendary.player_profiles` column-additively). Commit prefix at execution: `EC-128:` (code under `apps/server/src/`, `apps/arena-client/src/`, and `data/migrations/` will be staged → SPEC: prefix forbidden per `01.3` Rule 5). See [WP-104-owner-profile-data-model-and-me-edit.md](WP-104-owner-profile-data-model-and-me-edit.md) + [EC-128-owner-profile-data-model-and-me-edit.checklist.md](../execution-checklists/EC-128-owner-profile-data-model-and-me-edit.checklist.md).
...
- [ ] **(deferred placeholder)** WP-108 — Profile Billing & Funding
  History UI Surface. **Reframed 2026-05-03** in light of WP-132 /
  WP-133 / WP-134 drafts: WP-108 no longer owns any monetization
  plumbing. The three monetization-stack WPs cover what the
  original WP-108 placeholder originally bundled together —
  - **WP-132** (drafted 2026-05-03) ships the entitlements
    substrate (`legendary.entitlements` + `EntitlementKey` closed
    union + `GET /api/me/entitlements` read endpoint).
  - **WP-133** (drafted 2026-05-03) ships Stripe Checkout creation
    + signature-verified webhook ingestion (`legendary.stripe_events`
    + `legendary.stripe_checkout_sessions`).
  - **WP-134** (drafted 2026-05-03) ships the webhook → entitlement
    fulfillment processor + recovery script.
  The original placeholder's `legendary.player_payments` table is
  **superseded** by `legendary.stripe_checkout_sessions` (WP-133)
  + `legendary.entitlements` (WP-132); WP-108 introduces no new
  monetization tables. WP-108's reframed scope is the
  **arena-client `/me` UI page** that surfaces three read-only
  history sections, each consuming a backend read endpoint:
  - **"Your benefits"** — consumes WP-132's existing
    `GET /api/me/entitlements`; renders the active entitlement
    list grouped by `entitlement_key` with friendly labels (e.g.,
    `'supporter_tier_basic_2026'` → "Legendary Supporter (2026)").
  - **"Your purchase history"** — consumes a new
    `GET /api/me/billing/history` endpoint over
    `legendary.stripe_checkout_sessions` (filtered by
    `account_id` + `intent_status = 'completed'`); WP-108 owns
    this endpoint and the matching `apps/server/src/billing/`
    library function. The endpoint returns the locked Stripe
    session ID + amount + currency + completed_at + the
    associated `entitlement_key` for receipt linking.
  - **"Your tournament funding history"** — consumes a future
    WP-097-derivative read endpoint (Open Collective / PayPal
    donation history per `docs/TOURNAMENT-FUNDING.md`). WP-108
    inherits this section as a placeholder until WP-097 lands a
    canonical donation-record schema; until then the section
    renders an empty state ("Coming soon — see WP-097").
  Dependencies: **WP-097 must land** (D-9701 anchors the
  tournament-funding policy and the §F Funding Surface Gate);
  **WP-098 must land** (the §20 Funding Surface Gate Trigger in
  `00.3` must exist for this WP to declare applicability without
  a §17 lint FAIL); **WP-132 must land** (consumed read endpoint
  for the "Your benefits" section); **WP-133 must land** (read
  source for the "Your purchase history" section);
  **WP-134 must land** (the entitlements that "Your benefits"
  reads are populated by WP-134's fulfillment processor — without
  it the entitlements list is permanently empty even after a
  successful Stripe checkout). Scope (target ≤10 files when
  ready): one new HTTP endpoint
  `GET /api/me/billing/history` (`authenticated-session-required`
  per WP-112) + matching `apps/server/src/billing/` library
  function; one new arena-client page section on `MyProfilePage.vue`
  with three regions (benefits / purchases / donations); one
  typed API wrapper in `apps/arena-client/src/lib/api/`; never
  gates gameplay (NG-1 / D-9905 invariant). User-visible copy MUST
  use Vision §784–794 paid-tier framing ("Become a Legendary
  Supporter", "Your benefits", "Purchase history") for the first
  two sections; donation framing (per `docs/TOURNAMENT-FUNDING.md
  §Public Blurb (Reusable)`) is reserved for the third section
  only. The §F gate F-1..F-7 MUST be applicable-declared in the
  WP body at draft time. **Will not draft until** WP-097 + WP-098
  + WP-132 + WP-133 + WP-134 land. (Original 2026-04-25
  placeholder framing — `legendary.player_payments` table +
  generic "owner-only view surface on /me" + open-ended
  "PayPal / Stripe / OpenCollective webhook ingestion" — is
  preserved here as historical context: that bundle was
  intentionally split into WP-097 [donation governance],
  WP-132/133/134 [Stripe paid-tier monetization], and this
  reframed WP-108 [UI consumer] across the 2026-04-30..2026-05-03
  drafting window.)

- [x] WP-109 — Team Affiliation (Profile-Level Cooperative Cohorts) — **Done 2026-05-03 (`EC-115:`).** Drafted 2026-04-26; review-pass amendment 2026-05-03 (captain invariant + default invalidity behavior + same-size cohort exclusivity + read-time visibility + timeline monotonicity); lint-gate self-review **PASS**; EC-115 drafted 2026-04-26 (originally EC-109; retargeted EC-109 → EC-115 on 2026-04-27 per filename collision with executed viewer-series `EC-109-delete-unused-auxiliary-metadata.checklist.md`); pre-flight + copilot-check completed 2026-05-03 in `docs/ai/invocations/preflight-wp109.md` (**CONFIRM** disposition after fourteen scope-neutral amendments PS-1..PS-14 landed); session prompt at `docs/ai/invocations/session-wp109-team-affiliation.md`. User pre-locked 2026-05-03 PS-3 = YES (extend `OwnerProfileView` 7 → 8 keys + modify `MyProfilePage.vue`) and OQ-4 = (a) (denormalize `team_size` into `legendary.team_member_events`); §17 OQ-1..OQ-3 also pre-locked at recommended defaults (D-10901..D-10903). **WP-104 hard dependency satisfied 2026-05-02 at `cea9108`** when WP-104 / EC-128 landed; `legendary.player_profiles` + `legendary.player_links` + migration 009 + the `OwnerProfileView` type contracts are now present in `main`. Pre-flight + copilot-check + user pre-lock 14 amendments landed in `SPEC: WP-109 / EC-115 — pre-flight PS-1..PS-14 + user pre-lock 2026-05-03` commit at `3508ad2` (2026-05-03); execution lands in `EC-115:` commit immediately following. Eight DECISIONS entries (D-10901..D-10908) inserted in numeric order at execution. Server test baseline `pass 82 / fail 0 / skipped 42` → `pass 99 / fail 0 / skipped 54` (+17 logic-pure tests / +12 DB-required tests / +1 suite); engine baseline `pass 604 / fail 0` UNCHANGED. 01.6 post-mortem authored at `docs/ai/post-mortems/01.6-WP-109-team-affiliation.md` recording the seal-via-UPDATE design tension, the `'friends'`-fallback semantic gap, the locked-DTO extension pattern, the `legendary.teams` name-collision resolution, and the two-event promotion model.
  Dependencies: WP-104 (landed — `legendary.player_profiles` + `apps/server/src/profile/` module are extension targets); WP-052 (identity model); WP-101 (handle claim flow); WP-102 (public profile page — `PublicProfileView` is the integration target for `teamAffiliations[]`, NOT WP-104's owner-edit surface per pre-flight PS-3); WP-115 (long-lived `pg.Pool` lifecycle anchor + `register*Routes(...)` precedent — soft-dep).
  Notes: Adds a Server-layer team-identity surface (Team entity + member event log + audit log) plus a `teamAffiliations[]` projection on WP-102's `PublicProfileView` (4 → 5 keys; drift test at `profile.logic.test.ts:168–173` updated in same commit) and (per PS-3 default = YES) on WP-104's `OwnerProfileView` (7 → 8 keys; drift test at `ownerProfile.logic.test.ts:146–155` updated likewise). Read-time visibility join through the new team tables, server-evaluated per WP-109 §11 (friends-of-graph fallback to `private` when no friend-graph surface exists). Variable team size declared at creation as `teamSize: 3 | 4 | 5` (immutable post-creation; maps to Legendary's three meaningful cooperative formats — 3-handed, 4-handed, 5-handed). Substitute cap = `min(2, teamSize − 2)` (1 / 2 / 2). Validity rule generalized: `liveMembers ≥ teamSize − 2 AND liveMembers + liveSubs ≥ teamSize − 1`; default behavior on violation = mutation fails with full-sentence error (WP-109 §8.2; AC #7). Captain invariant: exactly one `captainPlayerId` per team at all times; captain MUST be a current `member` (WP-109 §6; AC #10). Same-size cohort exclusivity: at most one active team per `teamSize` per player; cross-`teamSize` overlap permitted (WP-109 §8.5; AC #12). Monotonic timeline: `joinedAt ≤ leftAt`; `joinedAt` not rewritable post-`leftAt` (WP-109 AC #11). Vision-aligned per §3 / §4 / §23(b) / §25; D-0005 preserved trivially (no comparison surface introduced); DESIGN-RANKING.md §12 deferral honored (no team rankings, no inter-team comparison, no rewards, no MMR — identity and history only). Team-play attribution onto run records explicitly out of scope and deferred to a future WP (provisionally WP-110+). `cohortLabel` field name avoids collision with DESIGN-RANKING §2 Season. Operator-override paths exist in code but HTTP exposure is gated behind a future admin-auth WP. Twenty files staged at execution (PS-3 = YES + OQ-4 = (a) user pre-lock 2026-05-03; +1 if 01.6 post-mortem authored): `apps/server/src/teams/{team.types.ts, team.logic.ts, team.routes.ts, team.logic.test.ts}` (4 new), `data/migrations/010_create_teams_and_membership.sql` (new — migration slot 010 assigned per PS-4; `legendary.team_member_events` carries denormalized `team_size int NOT NULL CHECK (team_size IN (3, 4, 5))` column per OQ-4 = (a), enabling the simple `(player_id, team_size) WHERE left_at IS NULL` UNIQUE partial index for same-size exclusivity defense in depth), `apps/server/src/profile/{profile.types.ts, profile.logic.ts, profile.logic.test.ts}` (modified — `PublicProfileView` 4 → 5 keys; drift test at lines 168–173 extended per PS-6), `apps/server/src/profile/{ownerProfile.types.ts, ownerProfile.logic.ts, ownerProfile.logic.test.ts}` (modified per PS-3 = YES — `OwnerProfileView` 7 → 8 keys; drift test at lines 146–155 extended), `apps/server/src/server.mjs` (modified — `registerTeamRoutes(server.router, pool, deps)` per WP-104 D-10408 same-commit-wiring precedent), `apps/arena-client/src/pages/PlayerProfilePage.vue` (modified — corrected path per PS-5; `PublicProfile.vue` does not exist), `apps/arena-client/src/pages/MyProfilePage.vue` (modified per PS-3 = YES — read-only "your teams" block in a new region using existing `defineComponent` wrapper), `docs/ai/REFERENCE/api-endpoints.md` (modified per PS-7 — 8 new `Wired` rows per D-11804), `docs/ai/REFERENCE/00.2-data-requirements.md` (modified per PS-8 — 3 new §4.1 Table Inventory rows for `legendary.teams` / `legendary.team_member_events` / `legendary.team_audit_log`), plus 4 governance ledgers (STATUS.md / DECISIONS.md / WORK_INDEX.md / EC_INDEX.md). Eight DECISIONS entries land at execution: D-10901 (OQ-1 friend-graph fallback), D-10902 (OQ-2 explicit two-event promotion), D-10903 (OQ-3 explicit cohort rollover), D-10904 (PS-3 = YES OwnerProfileView extension), D-10905 (`apps/server/src/teams/` server-layer classification), D-10906 (migration slot 010 + idempotency + OQ-4 = (a) denormalization), D-10907 (single-transaction multi-row create-team), D-10908 (`TeamId` branded type). New D-entry required at execution to classify `apps/server/src/teams/` as a server-layer directory (mirrors D-5202 for `identity/`, D-10301 for `replay/`, D-10201 for `profile/`); D-NNNN number assigned at execution. Server test baseline `pass 82 / fail 0 / skipped 42` locked at pre-flight (post-WP-104). Engine test baseline `pass 604 / fail 0` UNCHANGED. Several §17 Open Questions remain pre-flight items (friend-graph existence at execution time, substitute auto-promotion ergonomics, cohort rollover); same-size cohort exclusivity + invalidity recovery state are now defaults locked in the WP per the 2026-05-03 amendment. See [WP-109-team-affiliation.md](WP-109-team-affiliation.md), [EC-115-team-affiliation.checklist.md](../execution-checklists/EC-115-team-affiliation.checklist.md), and the pre-flight artifact at `docs/ai/invocations/preflight-wp109.md`.

- [x] WP-112 — Session Token Validation Middleware. **Done 2026-05-02 (`EC-112:`).** Drafted 2026-05-02; lint-gate self-review **PASS** (§1–§21); EC-112 drafted 2026-05-02 (status `Draft` in `EC_INDEX.md`; the natural EC-112 slot is free — no Registry Viewer EC-101+ namespace collision per the rule documented at the top of `EC_INDEX.md`, confirmed against the EC-120 row's reference to "EC-112 would shadow WP-112 (deferred placeholder; Session Token Validation Middleware per D-10002)"). Renumbered from "WP-100" by D-10002. Authorizing contract: WP-099 §A "Session Validation Middleware" (F-1..F-7 Future-Auth Gates locked there); WP-099 §B "Hanko Wiring Module"; WP-099 D-9904 (Hanko code confined to `apps/server/src/auth/hanko/`). Architectural choice locked at draft time under **D-11201** (recorded in WP-112 §Decision Points; copied verbatim into `DECISIONS.md` at execution): **SIBLING WP** — WP-112 ships the broker-agnostic orchestrator + the `SessionVerifier` interface + the `findAccountByAuthProviderSub` lookup helper + tests; the Hanko-specific verifier (SDK wiring, JWKS fetch/cache, JWT validation, claim extraction) is deferred to a sibling implementation WP — **WP-126 — External Authentication Integration (Hanko Session Verifier)** — whose deferred-placeholder row is added below in the same SPEC drafting commit. Three further decisions surfaced as `[DECISION REQUIRED]` blocks for the executor (D-11202 token extraction source, D-11203 `findAccountByAuthProviderSub` signature, D-11204 unconfigured-default behavior); recommended defaults documented in WP-112 §Decision Points. Scope (≤8 production / reference files; six projected): `apps/server/src/auth/{sessionToken.types.ts, sessionToken.logic.ts, sessionToken.logic.test.ts, accountLookup.logic.ts, accountLookup.logic.test.ts}` + `docs/ai/REFERENCE/api-endpoints.md` (modified — adds two `Library-only` rows per D-11804). MUST NOT modify WP-052 contract files (`apps/server/src/identity/identity.{types,logic}.ts`, migrations 004 / 005); MUST NOT modify WP-099 governance artifacts; MUST NOT modify `.claude/rules/*.md`. F-1..F-7 disposition enumerated and PASS by construction in WP-112 §"WP-099 Future-Auth Gate (F-1..F-7) Disposition". D-11804 catalog-update obligation triggered (new `Library-only` rows for `requireAuthenticatedSession` + `findAccountByAuthProviderSub`). Vision Alignment: §3, §11, §14, §15, NG-1, NG-3, NG-6 — no conflict; determinism N/A. Dependencies: WP-099 (policy contract); WP-052 (identity model). Soft-consumers (none of which require WP-112 to land first because all use the caller-injected provider pattern): WP-101, WP-102, WP-104. Commit prefix at execution: `EC-112:` (code under `apps/server/src/` will be staged → SPEC: prefix forbidden per `01.3` Rule 5). See [WP-112-session-token-validation-middleware.md](WP-112-session-token-validation-middleware.md) + [EC-112-session-token-validation-middleware.checklist.md](../execution-checklists/EC-112-session-token-validation-middleware.checklist.md).

- [x] WP-126 — External Authentication Integration (Hanko Session Verifier) — **Done 2026-05-03 (`EC-130:`).** Drafted 2026-05-03 in `SPEC: draft WP-126 / EC-130` commit at `e35dd00`; lint-gate self-review **PASS** (§1–§21); EC-130 drafted 2026-05-03; pre-flight + copilot-check completed 2026-05-03 in `docs/ai/invocations/preflight-wp126.md` (PS-1..PS-4 resolved in-place: single-param `Result<T>` lock, JWKS fetcher injection seam, single-site default substitution, JWKS aliasing defense). New `apps/server/src/auth/hanko/` directory under D-9904 holds five files: `hankoVerifier.types.ts` (config + `JwksFetcher` type + closed-set `HANKO_IDP_TO_AUTH_PROVIDER` lookup with seven keys + WP-112 re-exports), `hankoVerifier.logic.ts` (the `createHankoSessionVerifier(config): SessionVerifier` factory + 8-step `verify(token)` closure following the locked order from WP-126 §Scope (In) §B), `hankoVerifier.logic.test.ts` (17 cases), `jwksCache.logic.ts` (per-instance cache with single-flight refresh / one-shot retry / graceful degradation / aliasing-defense via `Object.freeze` at insertion), `jwksCache.logic.test.ts` (8 cases). Verifier returns `Promise<Result<VerifiedSessionClaim>>` (single-parameter `Result<T>` per `identity.types.ts:139-141`); failure-payload `code` field carries `SessionVerificationErrorCode` values via the project's settled `as never` cast pattern (mirrors `sessionToken.logic.test.ts` `makeFailingVerifier`); the orchestrator translates them at the existing `sessionToken.logic.ts:191-193` cast site (untouched). **Four DECISIONS entries** land at execution in numeric order: D-12601 (built-ins-only path — RS256 via Node v22 `node:crypto`, zero new npm dependency), D-12602 (4-field `HankoVerifierConfig` shape + 3 env vars `HANKO_TENANT_BASE_URL` / `HANKO_EXPECTED_AUDIENCE` / `HANKO_JWKS_REFRESH_INTERVAL_MS`), D-12603 (per-instance cache + 300_000 ms default + single-flight + one-shot retry + graceful degradation + insertion-time `Object.freeze`), D-12604 (federation claim = `amr` array; closed-set object-literal lookup with two-pass priority scan; federated values win over native; citations: Hanko docs + Hanko source `hook_determine_amr_values.go` + `provider.go`). **D-11201 status flips `Active` → `Resolved`** per its body's "Status flips to `Resolved` once WP-126 lands". F-1..F-7 PASS by construction: F-1 (no `'hanko'` enum value anywhere), F-2 (every `@teamhanko/*` import / `hanko.io` URL contained inside `apps/server/src/auth/hanko/` — runtime sources only; `render.yaml` / `.env.example` exempt), F-3 (no `randomUUID` in `auth/hanko/`), F-4 (zero diff against `server.mjs` / leaderboards / profile / sessionToken.logic.ts / accountLookup.logic.ts), F-5 (zero diff against `apps/server/package.json` — built-ins-only), F-6 (replacement-safety — deleting `auth/hanko/` + the catalog row + the env vars requires zero WP-112 / WP-052 / WP-099 file change), F-7 (Vision Alignment cites §3 / §11 / §14 / §15 + NG-1 / NG-3 / NG-6, no-conflict + N/A determinism). Server test baseline `pass 99 / fail 0 / skipped 54` → **`pass 124 / fail 0 / skipped 54`** (+25 logic-pure tests, all always-runs since the WP-126 suite has no DB requirement). Engine baseline `pass 604 / fail 0` UNCHANGED. Twelve files staged in single `EC-130:` commit (8 production / reference + 4 governance): 5 new under `apps/server/src/auth/hanko/`, 3 modified config/reference files (`render.yaml` + `.env.example` + `docs/ai/REFERENCE/api-endpoints.md` — 1 new `Library-only` row for `createHankoSessionVerifier` per D-11804), 4 governance ledgers (this row + STATUS.md + DECISIONS.md + EC_INDEX.md). 01.5 NOT INVOKED (zero engine surface change). 01.6 post-mortem SKIPPED with documented rationale (the locked decisions held without contradiction at execution time; no surprise design tensions warranted the lessons-captured artifact). Production wiring (registering `requireAuthenticatedSession(req, { verifier: createHankoSessionVerifier(config), accountResolver, database })` into authenticated route handlers) remains the responsibility of a separate future request-handler WP that owns long-lived `pg.Pool` lifecycle (D-11204 fail-closed posture preserved until then). Vision: §3, §11, §14, §15, NG-1, NG-3, NG-6 cited; determinism N/A (server-layer broker adapter; engine entirely untouched). Commit prefix `EC-130:` at execution (never `WP-126:` per `01.3-commit-hygiene-under-ec-mode.md`). See [WP-126-external-authentication-hanko-session-verifier.md](WP-126-external-authentication-hanko-session-verifier.md) + [EC-130-external-authentication-hanko-session-verifier.checklist.md](../execution-checklists/EC-130-external-authentication-hanko-session-verifier.checklist.md) + D-12601 + D-12602 + D-12603 + D-12604 + D-11201 (now Resolved).

- [x] WP-128 — UIState Projection Extensions for Board Layout — **Done 2026-05-04 (`EC-131:` commit `c44f539`).** Engine UI projection contract extended along the board-layout wireframe; six engine UI files (3 production + 3 test) modified; 4 governance ledgers updated; 1 mandatory 01.6 post-mortem authored at `docs/ai/post-mortems/01.6-WP-128-uistate-projection-extensions.md`. **D-12801..D-12807 inserted at execution** in numeric order: D-12801 (victoryVP engine projection), D-12802 (horrorsCount always-present zero default), D-12803 (inPlayCards/inPlayDisplay redaction), D-12804 (KO pile fully projected from top-level `G.ko` per types.ts:481), D-12805 (Mastermind attachedBystanders shape + Interpretation B data semantics), D-12806 (Option A safe-skip resolution for 8 missing-G-source projections — each carries `// SAFE-SKIP-WP128` marker; future "G extensions for board layout" WP placeholder row below resolves them in lockstep), D-12807 (01.5 cascade resolution — **no cascade fired**; `PRE_WP080_HASH = '46f7863c'` byte-identical pre/post-projection because UIState is downstream of computeStateHash inputs and Option A safe-skip means no new G fields). 01.5 conditional cascade allowance NOT EXERCISED. Engine baseline `604 / 132 / 0` → **`621 / 135 / 0`** (+17 tests, +3 suites; N+M = 20, exactly at EC-131 §0 budget). All grep gates pass: zero `boardgame.io` imports under `packages/game-engine/src/ui/`; zero `@legendary-arena/registry` imports; zero `Math.random` / `Date.now` / `fetch(` / `require(` in projection / filter files; zero `G.piles.ko` / `gameState.piles.ko` references in `uiState.build.ts` (PS-1 correction held); SAFE-SKIP-WP128 marker count exactly 8 at assignment sites; `git diff packages/game-engine/src/types.ts` empty (no new G field); `git diff packages/game-engine/src/ui/uiAudience.types.ts` empty (no UIAudience extension). `pnpm --filter @legendary-arena/game-engine build` exits 0. WP-128 ships pure Contract-Only (no `G` mutation, no move logic, no phase hooks). **Drafted 2026-05-03** alongside WP-129 and WP-130 as the Board Layout chain seeded from `docs/ai/DESIGN-BOARD-LAYOUT.md` (committed at `277bcca` as the wireframe SPEC artifact). Engine-side projection extension that surfaces every `Pending` row from the wireframe's §4 mapping table through `UIState`. Adds new top-level fields `decks: UIDecksState` (`villainDeckCount`, `heroDeckCount`), `piles: UISharedPilesState` (`bystandersCount`, `woundsCount`, `horrorsCount`, `officersCount`, `sidekicksCount`), `koPile: UIKoPileState` (`count`, `topCard`, `cards[]`); extends `UIPlayerState` with optional `inPlayCards?` / `inPlayDisplay?` / `discardTopCard?` / `victoryCards?` / `victoryVP?` (uppercase `VP` aligned with canonical `PlayerScoreBreakdown.totalVP` per pre-flight 2026-05-03 PS-2); extends `UIMastermindState` with required `attachedBystanders` + `strikePile`; extends `UISchemeState` with required `twistPile`; extends `UICityState` with required `escapedPile`; extends `UITurnEconomyState` with required `piercing` + `woundsDrawn`. Mirrors WP-111 sibling-snapshot pattern verbatim (`buildUIState` extensions + drift-test pinning + per-entry shallow-copy aliasing-defense per D-11105). `01.5 IS INVOKED` (additive UIState shape change; replay-hash literal cascade in `replay.execute.test.ts` permitted as 01.5-cascade allowlist additions if the new fields cause hash drift). 01.6 post-mortem MANDATORY (new long-lived projection surface — `decks` + `piles` + `koPile` + per-player victory-pile contents are new contract types consumed by future board-layout WPs). Five `[DECISION REQUIRED]` blocks (D-DEC-1..D-DEC-5) with recommended defaults per WP-128 §Decision Points; **D-12801..D-12807 inserted at execution** under post-pre-flight mapping: D-12801 = D-DEC-3 redaction; D-12802 = D-DEC-1 victoryVP; D-12803 = D-DEC-2 horrors-always-zero; D-12804 = D-DEC-4 KO pile contents; D-12805 = D-DEC-5 mastermind shape + Interpretation B data semantics; **D-12806 = Option A safe-skip resolution** (locked at pre-flight 2026-05-03 PS-3 per WP-023 / WP-025 / WP-026 / WP-030 safe-skip precedent; covers 8 missing-G-source projections — `mastermind.attachedBystanders`, `mastermind.strikePile`, `scheme.twistPile`, `city.escapedPile`, `economy.piercing`, `economy.woundsDrawn`, `decks.heroDeckCount`, `piles.horrorsCount`); D-12807 = 01.5 cascade resolution captured at session close. Pre-flight + copilot check completed 2026-05-03 (`docs/ai/invocations/preflight-wp128.md` + `copilot-check-wp128.md`); session prompt at `docs/ai/invocations/session-wp128-uistate-projection-extensions.md`. Engine baseline `604/132/0` → projected `604+N/132+M/0` with N+M ∈ [12, 20] new tests. Hard-deps: WP-028 (UIState contract); WP-029 (audience filtering); WP-089 (`LegendaryGame.playerView` wiring); WP-111 (sibling-snapshot precedent). EC-131 drafted in same SPEC commit. Commit prefix `EC-131:` at execution. See [WP-128-uistate-projection-extensions-for-board-layout.md](WP-128-uistate-projection-extensions-for-board-layout.md) + [EC-131-uistate-projection-extensions-for-board-layout.checklist.md](../execution-checklists/EC-131-uistate-projection-extensions-for-board-layout.checklist.md). **Hard prerequisite for WP-129.**

- [x] WP-129 — Board Layout (Desktop Landscape + Mobile Portrait) — **Done 2026-05-04 (`EC-132:` commit `fb2bf95`).** Production Vue component tree implemented per `DESIGN-BOARD-LAYOUT.md §3.1` (desktop, 1280×800–1920×1080) + `§3.2` (mobile portrait, 375×667–414×896) against the WP-128-extended `UIState`. ~22 new SFCs / composables (13 new components under `apps/arena-client/src/components/play/`: `<SchemeTile>`, `<MasterStrikePile>`, `<SchemeTwistPile>`, `<EscapedPile>`, `<SharedDecks>`, `<KOPile>`, `<OpponentPanel>`, `<OpponentVictoryModal>`, `<YourVictoryPile>`, `<YourDeckDiscardZone>`, `<EconomyBar>`, `<TopHudBar>`; 3 page-level SFCs under `apps/arena-client/src/pages/`: `<PlayDesktop>`, `<PlayMobile>`, `<PlayViewport>`; 6 headless composables under `apps/arena-client/src/composables/`: `useCityRow`, `useHqRow`, `useTurnActions`, `useCardCostGating`, `useVictoryPileComposition`, `useViewport`); 5 WP-100 scaffolds rewritten or extended (`HandRow` with `handDisplay` integration; `CityRow` rewritten as 7-cell visual + cost-gating; `HQRow` rewritten as 6-cell visual + cost-gating; `MastermindTile` extended with `attachedBystanders` rendering; `TurnActionBar` rewritten as 3-step structure); 1 modified `App.vue` (route-mount swap `<PlayView>` → `<PlayViewport>`); 2 deleted scaffolds (`PlayView.vue` + `PlayView.test.ts`); 4 governance ledgers updated. **Nine D-decisions inserted in numeric order** (D-12901..D-12909): D-12901 (Mastermind top-left), D-12902 (opponent orientation player-count-dependent), D-12903 (HQ slot count 5 MVP + graceful 6-slot extension), D-12904 (in-play persistence through cleanup), D-12905 (number-with-deck-icon face-down representation), D-12906 (composition counters derived from card effects), D-12907 (re-skin slot reserved for **WP-130**), D-12908 (pre-plan slot reserved for **WP-059**), D-12909 (`BREAKPOINT_MOBILE_MAX_PX = 767` viewport breakpoint locked before first production component file). Disabled-state tooltip precedence (stage → resource → structural) implemented exactly once in `useTurnActions` + `useCardCostGating`; every binding site consumes the returned reason via `aria-disabled` + `title`. Pass-priority button fires `advanceStage` per D-10011 (canonical stage-advance vocabulary, NOT a no-op — per pre-flight PS-5). arena-client baseline `182 / 17 / 0` → **`250 / 30 / 0`** (+68 tests, +13 suites — past the EC-132 §5 ≥30-test floor). Engine baseline `621 / 135 / 0` UNCHANGED (engine never touched). All grep gates pass: zero engine-runtime imports in arena-client (type-only permitted); zero registry-runtime imports; zero `Math.random()` / `Date.now()`; zero `boardgame.io` imports under `components/play/` / `pages/` / `composables/`; zero client-side `throw` / `alert` / `console.error` introduced (per copilot RISK 22 silent/loud lock); zero `vue-router` introduced (PS-2 / WP-102 lock); SAFE-SKIP-WP128 marker count ≥6 (one per consuming SFC: `<EscapedPile>`, `<MastermindTile>`, `<MasterStrikePile>`, `<SchemeTwistPile>`, `<EconomyBar>`); zero modification to `uiMoveName.types.ts` (D-10004 lock — 10-name union reused), `bgioClient.ts` (WP-090 transport-seam lock), `LobbyControls.{vue,test.ts}` (PS-4 scope lock), `stores/uiState.ts` (WP-061 store-seam lock). 01.5 NOT INVOKED. 01.6 post-mortem **SKIPPED** (per WP-126 SKIPPED-with-rationale precedent — no D-decision tension surfaced). Pre-flight (PS-1..PS-7) + copilot check (RISKs 15/22/25/30) reconciled in the SPEC commit `d5bf1f4` 2026-05-04 prior to execution; resolution log mirrored into `docs/ai/invocations/session-wp129-board-layout-desktop-and-mobile.md §20`. Drafted 2026-05-03 as the second WP in the Board Layout chain. Commit prefix `EC-132:` per `01.3` Rule 5. See [WP-129-board-layout-desktop-and-mobile.md](WP-129-board-layout-desktop-and-mobile.md) + [EC-132-board-layout-desktop-and-mobile.checklist.md](../execution-checklists/EC-132-board-layout-desktop-and-mobile.checklist.md). **HUD-bar skin-selector slot reserved for WP-130; bottom-edge pre-plan-UI slot reserved for WP-059.**

- [x] WP-130 — Re-skin / Playmat Selector — **Done 2026-05-04 (`EC-133:` commit `b6651ed`).** Mounts a `🎨 Skin: <name> ▼` button in the WP-129-reserved HUD-bar slot (D-12907) and lets players swap the visual chrome of `<PlayViewport>` between three bundled skins (`classic`, `comic`, `minimal` per D-13003) with selection persisted to `localStorage['arenaClientPlaymatSkin']` per D-13004. **Pre-flight 2026-05-04 PS-1 Option A held at execution** — WP-068 multi-section subsystem dropped as a dependency (commit `bbd58b0` on branch `wp-068-preferences-foundation` never merged to `main`); the playmat store mirrors the **WP-121 / WP-124 single-key preferences precedent** (`apps/registry-viewer/src/composables/{useCardSize,useThemeSize}.ts`) wrapped in a `defineStore('playmat', () => { … })` Pinia setup store. No section registry, no schema-version envelope, no `apps/arena-client/src/main.ts` modification (the store lazy-initializes on first `usePlaymat()` call). **Five D-decision locks land in numeric order (D-13001..D-13005):** D-13001 (bundled-with-client discovery at MVP, R2-published manifest deferred), D-13002 (skin scope = board background + color theme + card-frame style, audio EXCLUDED), D-13003 (locked bundled set `classic` + `comic` + `minimal` with `'classic'` as both default and unconditional fallback), D-13004 (`localStorage`-only persistence per WP-104 column-additive precedent — server sync is a future WP), D-13005 (empty-state / asset-failure unconditional fallback to `'classic'` + `console.warn` per `00.6` Rule 11; asset-failure narrowly defined as manifest-resolution OR class-application errors only). **Six contract surfaces ship in `apps/arena-client/src/prefs/`:** `skinManifest.ts` (canonical source of truth — `SkinName` derives from `Object.keys(skinManifest)` per WP-130 §A; `as const satisfies Record<string, SkinManifestEntry>` shape; asset URLs resolved via `new URL(path, import.meta.url).href` for cross-Vite/Node compatibility), `playmatSchema.ts` (closed-set narrower with `parseSkinName(value): SkinName` falling back to `DEFAULT_SKIN_NAME` on rejection — no Zod added; pure-TS narrowing matches the WP-121/WP-124 precedent), `persistence.ts` (sync `localStorage.{getItem,setItem}` with Rule-11 swallow comment mirroring `useCardSize.ts:130-141` posture), `playmatStore.ts` (Pinia setup store exposing `{ activeSkin, availableSkins, setActiveSkin }` — Pinia auto-unwraps refs at the consumer level; `setActiveSkin` is a synchronous write per EC-133 §3 sync-write lock). `<SkinSelector>` (`apps/arena-client/src/components/play/SkinSelector.vue`) always mounts in the WP-129 reserved slot per EC-133 always-mounted rule; empty-state renders a disabled `🎨 (default)` chip rather than unmounting; Vue 3 `<Teleport to="body">` overlay with D-6401 `tabindex="0"` + root-level `@keydown.escape` (mirrors `<ReplayInspector>` posture); outside-click on backdrop + selection both close. `useSkinApplier` (`apps/arena-client/src/composables/useSkinApplier.ts`) watches `activeSkin` via `watchEffect` and applies the corresponding CSS class **exclusively to `<PlayViewport>`'s root element** (never `<body>` / `document.documentElement` / any global node — verified by EC-133 §10 grep gate); idempotently injects the matching `<link rel="stylesheet">` into `document.head`; falls back to `'skin-classic'` + `console.warn` on missing manifest entry OR class-application error. **One forced cascade modification:** `apps/arena-client/src/components/play/TopHudBar.test.ts` updated from the placeholder-existence assertion (which referenced the now-removed `play-hud-skin-placeholder` span) to a `<SkinSelector>`-mount assertion; required `setActivePinia(createPinia())` per existing test convention (PlayDesktop / PlayMobile / PlayViewport precedent). **Three skin asset bundles** at `apps/arena-client/src/assets/skins/{classic,comic,minimal}/{board-background.png,card-frame.png,theme.css}` ship as 79-byte solid-color placeholder PNGs per the WP-130 §F note (locked contract is the directory structure + manifest mapping, not art fidelity); each `theme.css` defines CSS custom properties scoped to `.skin-<name>`. **Zero engine state effect:** the skin selector NEVER affects `UIState` / `G` / replay determinism / `computeStateHash` / socket transport. **Verification gates all pass.** `pnpm -r build` exits 0. arena-client baseline `257 / 30 / 0` → **`286 / 35 / 0`** (+29 tests / +5 suites — past the EC-133 §0 ≥8-test floor with one `describe()` block per new test file × 5 test files). Engine baseline `679 / 148 / 0` UNCHANGED (engine never touched). Off-scope diffs empty: `git diff packages/ apps/server apps/registry-viewer apps/replay-producer apps/arena-client/src/main.ts apps/registry-viewer/src/composables/{useCardSize,useThemeSize}.ts apps/arena-client/src/stores/uiState.ts apps/arena-client/src/client/bgioClient.ts` returns no output. `apps/arena-client/src/prefs/registerSections.ts` does NOT exist (Option A simplification lock). All §10 grep gates pass: zero engine-runtime imports in `prefs/` / `SkinSelector.vue` / `useSkinApplier.ts` (type-only would be permitted); zero `useUiStateStore` / `G.` / `UIState.` references; zero `submitMove` / `boardgame.io` references; zero `Math.random` / `Date.now` / `fetch(` in `prefs/`; zero hand-duplicated `z.enum(['classic'…])` literal; one match for `Object.keys(skinManifest)` in `playmatSchema.ts`; one `defineStore('playmat'`; one `arenaClientPlaymatSkin` storage key; empty-state `v-if="playmat.availableSkins.length === 0"` + `skin-chip--disabled` both present in `<SkinSelector>`; `<SkinSelector />` mounted in `<TopHudBar>`'s reserved slot; `play-hud-skin-placeholder` removed. Files staged in two-commit topology — Commit A (`EC-133:` `b6651ed`): 25 files (4 prefs source + 3 prefs tests + 9 asset bundle files + 2 selector + 2 applier + 3 modified SFCs/test + 2 governance drafts). Commit B (`SPEC:`): 4 governance ledgers (this file, `EC_INDEX.md`, `DECISIONS.md` D-13001..D-13005 inserted, `STATUS.md` `### WP-130 / EC-133 Executed` block). 01.5 NOT INVOKED (no engine surface change). 01.6 post-mortem **SKIPPED** per WP-121 / WP-124 single-key-preference precedent — single-section single-key preference; the five D-decisions held without contradiction at execution time; the only mid-execution amendment was the localStorage-shim install in test files (JSDOM opaque-origin + `apps/arena-client/src/testing/` outside-allowlist), which is a test-harness implementation detail rather than a design tension worth the lessons-captured artifact. Drafted 2026-05-03 as the third (deferrable) WP in the Board Layout chain. Pre-flight + copilot check (PASS-PASS post-PS-1) at `docs/ai/invocations/preflight-wp130.md`; session prompt at `docs/ai/invocations/session-wp130-reskin-playmat-selector.md` (both gitignored per `.claude/rules/work-packets.md`). Commit prefix `EC-133:` per `01.3` Rule 5. See [WP-130-reskin-playmat-selector.md](WP-130-reskin-playmat-selector.md) + [EC-133-reskin-playmat-selector.checklist.md](../execution-checklists/EC-133-reskin-playmat-selector.checklist.md). **HUD-bar skin-selector slot (D-12907) consumed; bottom-edge pre-plan-UI slot (D-12908) still reserved for WP-059.**

- [x] WP-131 — Authenticated Routes Production Wiring (Hanko Verifier + Account Resolver) — **Done 2026-05-04 (`EC-134:`).** The eleven `/api/me/*` (3) and `/api/teams/*` (8) authenticated routes graduate from D-11204 fail-closed (every request returned 500 with `code: 'session_verifier_not_configured'`) to genuinely authenticated. New file `apps/server/src/auth/accountResolver.logic.ts` exports `productionAccountResolver: AccountResolver` — a thin closure over WP-112's `findAccountByAuthProviderSub` that maps `Result.ok(hit) → Result.ok(hit.accountId)`, `Result.ok(null) → Result.ok(null)` (orchestrator translates to `'unknown_account'`), `Result.fail({ code: 'lookup_failed' })` forwarded verbatim per D-11203. Three resolver tests (hit / clean miss / lookup failure) use a local fake `DatabaseClient` with no PostgreSQL dependency. `apps/server/src/server.mjs` gains `tryConstructHankoVerifier()` exported helper that reads `HANKO_TENANT_BASE_URL` / `HANKO_EXPECTED_AUDIENCE` / `HANKO_JWKS_REFRESH_INTERVAL_MS` from `process.env`, constructs the verifier exactly once at startup, masks the tenant URL to origin-only in the success log line, and branches on `NODE_ENV` per D-13101 — production + missing env throws the locked full-sentence diagnostic, non-production + missing env returns `undefined` to preserve dev ergonomics. Both `register*Routes` call sites thread `verifier` + `accountResolver: verifier === undefined ? undefined : productionAccountResolver` through the existing `RouteDependencies` bundles per D-13103 (`configureSessionValidation` deferred — would require WP-104 / WP-109 route-helper refactor, forbidden under this WP). Two startup-guard tests in `server.mjs.test.ts` exercise `tryConstructHankoVerifier()` directly (production-fatal + dev-mode-undefined paths) — calling `startServer()` would invoke `loadRules()` PostgreSQL, so the helper-export form is the smallest-surface adaptation per WP §D's escape clause. **Four DECISIONS entries land in numeric order:** D-13101 (NODE_ENV-branched startup-policy gate), D-13102 (`accountResolver` location = new sibling file), D-13103 (per-request options via `RouteDependencies` deps bundles, NOT `configureSessionValidation`), D-13104 (startup-log URL masked to origin only — Datadog/Loggly defense). **01.5 NOT INVOKED** (no engine surface change; engine baseline `679/0` UNCHANGED). **01.6 post-mortem authored** at `docs/ai/post-mortems/01.6-WP-131-authenticated-routes-production-wiring.md` covering the test-harness adaptation (`tryConstructHankoVerifier` exported for direct testing in lieu of `startServer()`-end-to-end). **§17 Vision Alignment:** §3 / §11 / §14 / §15 / NG-1, NG-3, NG-6 cited; production wiring graduates fail-closed posture into fail-closed-on-misconfig + verify-on-good-config. **§20 Funding Surface Gate N/A** (server wiring + library-function add; no UI surface; no funding copy / schema referenced). **§21 API Catalog TRIGGERED:** eleven row replacements (Notes column gains canonical sentence "Genuinely authenticated as of WP-131 / EC-134. Production wiring constructs the Hanko verifier at startup from `HANKO_TENANT_BASE_URL` + `HANKO_EXPECTED_AUDIENCE`."; the line 121 row also drops the stale "until WP-126 lands" + "session_verifier_not_configured returns 500" sentences) + one new `Library-only` row for `productionAccountResolver`. **D-11204 status flips `Active` → `Resolved`** per its body's "Status flips to Resolved once production wiring lands". Two-commit topology — Commit A `EC-134:` (5 production/reference + WP-131 + EC-134 + 01.6 post-mortem); Commit B `SPEC:` (4 governance ledgers — DECISIONS.md D-13101..D-13104 inserted between D-13005 and D-13501; this WORK_INDEX.md row flipped; EC_INDEX.md EC-134 row flipped to Done; STATUS.md `### WP-131 / EC-134 Executed` block prepended). Server baseline `pass 124 / fail 0 / skipped 54 (suites 18, tests 178)` → **`pass 129 / fail 0 / skipped 54 (suites 20, tests 183)`** (+5 tests = 3 resolver + 2 startup-guard; +2 suites = new resolver test file + new `describe('startup guard (WP-131)', …)` block). Engine baseline `pass 679 / fail 0 / skipped 0 (suites 148, tests 679)` UNCHANGED. WP-104 / WP-109 / WP-126 / WP-112 contract files byte-identical (`git diff` empty); `render.yaml` / `.env.example` / `apps/server/package.json` / `pnpm-lock.yaml` byte-identical (no new env vars, no new dependencies). Soft-consumer **WP-132** (entitlements read endpoint) inherits the same `verifier` + `accountResolver` deps bundle. (Original draft 2026-05-03; lint-gate self-review **PASS** (§1–§21); EC-134 reserved (next free EC slot after EC-131/132/133 reserved by board-layout chain WP-128/129/130). Closes the **D-11204 fail-closed deferral** that has been in effect since WP-104 / WP-109 / WP-112 landed: the eleven authenticated routes under `/api/me/*` (3 from WP-104) and `/api/teams/*` (8 from WP-109) currently return 500 with `code: 'session_verifier_not_configured'` on every request because their `OwnerProfileRouteDependencies` / `TeamRouteDependencies` bundles ship with `verifier: undefined` + `accountResolver: undefined` per D-11204 fail-closed-by-construction. WP-131 wires the production verifier (from WP-126) and a new production `AccountResolver` into both `register*Routes(...)` call sites in `apps/server/src/server.mjs`. **Two file additions + one wiring modification + catalog row replacements:** `apps/server/src/auth/accountResolver.logic.ts` (new — `productionAccountResolver: AccountResolver` thin closure that calls WP-112's `findAccountByAuthProviderSub(claim.authProvider, claim.authProviderSub, database)` and remaps the row payload to bare `AccountId | null` per the `Result<AccountId | null>` orchestrator contract; never throws — DB faults bubble as `Result.fail({ code: 'lookup_failed' })` per D-11203 verbatim); `apps/server/src/auth/accountResolver.logic.test.ts` (new — three branches: hit, clean miss, DB fault); `apps/server/src/server.mjs` (modified — `startServer()` reads three env vars `HANKO_TENANT_BASE_URL` / `HANKO_EXPECTED_AUDIENCE` / `HANKO_JWKS_REFRESH_INTERVAL_MS` from `process.env`; constructs the verifier exactly once at startup via `createHankoSessionVerifier({ tenantBaseUrl, expectedAudience, jwksRefreshIntervalMs })`; binds `productionAccountResolver`; passes both through the existing `OwnerProfileRouteDependencies` / `TeamRouteDependencies` bundles; production-mode missing-env path is fatal — `startServer()` logs full-sentence diagnostic and exits 1 mirroring `loadRegistry` failure path; non-production missing-env path leaves `verifier: undefined` so authenticated routes continue to return 500 fail-closed, preserving local-dev ergonomics for engineers who do not need authenticated paths); `apps/server/src/server.mjs.test.ts` (modified — startup-guard test asserts production-mode boot with missing `HANKO_TENANT_BASE_URL` exits non-zero with the locked operator-facing diagnostic). The eleven `/api/me/*` and `/api/teams/*` rows in `docs/ai/REFERENCE/api-endpoints.md` are **replaced wholesale per D-11804**: the response-schema "on `'session_verifier_not_configured'` returns `500`" sentence and the trailing notes-column "fail-closed via WP-112 D-11204 until WP-126 lands" sentence are removed in favour of "genuinely authenticated as of WP-131". One new `Library-only` row appended for `productionAccountResolver`. **Invariant:** WP-131 wires existing libraries together — introduces zero new HTTP endpoints; modifies zero engine, registry, preplan, arena-client, registry-viewer, or replay-producer file; modifies zero existing route file (`ownerProfile.routes.ts` / `team.routes.ts` byte-identical post-WP-131). The only production-code touch outside the new resolver helper is `apps/server/src/server.mjs`. **Four DECISIONS entries** at execution: D-13101 (missing-env behavior — production fatal vs. non-production fail-closed), D-13102 (resolver location = `apps/server/src/auth/`), D-13103 (per-request options construction — verifier bound into deps once at startup, options object built per-request to satisfy `requireAuthenticatedSession`'s positional-args contract), D-13104 (startup-log discipline — log `'[server] auth wiring complete (verifier=<bool>, resolver=production)'` without leaking secret values). 01.5 NOT INVOKED (no engine surface change). 01.6 post-mortem OPTIONAL per WP-104 / WP-109 / WP-115 server-layer precedent. §17 Vision Alignment: §3 / §11 / §14 / §15 / NG-1, NG-3, NG-6 cited; no-conflict; production wiring graduates fail-closed posture into fail-closed-on-misconfig + verify-on-good-config posture. §20 Funding Surface Gate: N/A with justification (server wiring + library-function add; no `apps/arena-client/` / `apps/registry-viewer/` modification; no funding copy or schema referenced). §21 API Catalog TRIGGERED (eleven row replacements + one new `Library-only` row per D-11804). Engine baseline `pass 604 / fail 0` UNCHANGED. Server baseline `pass 124 / fail 0 / skipped 54` (post-WP-126; pre-WP-131) → projected `pass 124+N / fail 0 / skipped 54-M` where N+M reflects the new resolver tests + the eleven authenticated routes' previously-skipped DB-dependent tests becoming runnable. Hard-deps: WP-126 (`createHankoSessionVerifier(config)` factory + `HankoVerifierConfig` shape + three env vars); WP-112 (`SessionVerifier` interface + `AccountResolver` type + `requireAuthenticatedSession` orchestrator + `findAccountByAuthProviderSub` lookup helper); WP-104 (`registerOwnerProfileRoutes` deps bundle accepts `verifier?` + `accountResolver?`); WP-109 (`registerTeamRoutes` deps bundle structurally identical); WP-115 (long-lived `pg.Pool` lifecycle anchor at `apps/server/src/server.mjs`); WP-052 (`AccountId` brand + `legendary.players` table + `AuthProvider` enum). **Soft-consumer:** WP-132 (entitlements read endpoint inherits the same `verifier` + `accountResolver` deps bundle; WP-131 should land before WP-132 so `GET /api/me/entitlements` is genuinely authenticated rather than fail-closed). After WP-131 lands, **D-11204 status flips `Active` → `Resolved`** per its body's "Status flips to Resolved once production wiring lands". Commit prefix `EC-134:` at execution. See [WP-131-authenticated-routes-production-wiring.md](WP-131-authenticated-routes-production-wiring.md) and [`docs/ai/execution-checklists/EC-134-authenticated-routes-production-wiring.checklist.md`](../execution-checklists/EC-134-authenticated-routes-production-wiring.checklist.md).)

- [x] WP-132 — Entitlements Data Model & `/me/entitlements` Read API — **Done 2026-05-05 (`EC-135:`).** New server-layer entitlements substrate ships in `apps/server/src/entitlements/` (5 new files: `entitlements.types.ts`, `entitlements.logic.ts`, `entitlements.logic.test.ts`, `entitlements.routes.ts`, `entitlements.routes.test.ts`) + migration `data/migrations/011_create_entitlements.sql` (one new table `legendary.entitlements` with seven columns; partial unique index `entitlements_active_unique` on `(player_id, entitlement_key) WHERE revoked_at IS NULL` as the WP-134 webhook-retry idempotency primitive; secondary lookup index `idx_entitlements_player_id`). Single library export `getEntitlementsForAccount(accountId, database)` performs the standard WP-104 / WP-109 two-query pattern (Step 1: `SELECT player_id FROM legendary.players WHERE ext_id = $1 LIMIT 1`; Step 2: `SELECT entitlement_key, source, source_ref, granted_at, revoked_at FROM legendary.entitlements WHERE player_id = $1 AND revoked_at IS NULL ORDER BY granted_at ASC`). Single HTTP endpoint `GET /api/me/entitlements` registered via `registerEntitlementRoutes(...)` adjacent to `registerOwnerProfileRoutes` / `registerTeamRoutes` per D-13205 = (a); reuses the SAME `{ requireAuthenticatedSession, verifier, accountResolver }` deps bundle WP-131 / EC-134 already constructed. Read-only by construction; WP-132 ships ZERO grant path; WP-134 owns INSERT site; revocation is a future-WP responsibility. Six DECISIONS land in numeric order — D-13201 (module path `apps/server/src/entitlements/` sibling-flat), D-13202 (migration slot 011), D-13203 (`EntitlementKey` six-key cosmetic-only closed set with year-suffix discipline on time-boxed supporter SKUs), D-13204 (`source` three-value closed set `'stripe' \| 'admin_grant' \| 'comp'`), D-13205 (same-commit wiring; route Wired from day one because WP-131 already wired the production verifier), D-13206 (compile-time exhaustive `switch` with `default: const _: never = key` as the load-bearing TS-side parity gate; SQL CHECK ↔ canonical array parity is review-locked). Envelope split locked per D-11802 = (C) — auth/config faults dispatch `{ code: '<closed-set value>' }` (the four 401-mapped session validation codes collapse to a single client-facing `{ code: 'unauthorized' }` per WP-104 D-10403 account-existence-probe defense), operational faults dispatch `{ error: 'internal_error' }`. Status-code domain `{200, 401, 500}` closed. Server baseline `pass 129 / fail 0 / skipped 54 (suites 20, tests 183)` → **`pass 137 / fail 0 / skipped 59 (suites 22, tests 196)`** (+13 = 8 logic + 5 route; +8 pass = 3 logic-pure + 5 route; +5 skipped = 5 DB-required logic tests; +2 suites). Engine baseline `679 / 148 / 0` UNCHANGED. **§21 API Catalog TRIGGERED:** one new `Wired` row for `GET /api/me/entitlements` + one new `Library-only` row for `getEntitlementsForAccount` immediately after the WP-131 `productionAccountResolver` row, per D-11804 replace-whole-row merge semantics. **`docs/ai/REFERENCE/00.2-data-requirements.md §4.1`** carries one new row for `legendary.entitlements`. **01.5 NOT INVOKED** (server-layer-only WP). **01.6 post-mortem SKIPPED with rationale** (server-layer-only WP per WP-101 / WP-102 / WP-104 / WP-115 / WP-126 / WP-131 sibling-helper precedent; the four executor D-decisions held without contradiction; the migration `D-DEC-4` placeholder leak in the `source_ref` `// why:` block was caught and corrected mid-execution by the §5 decision-code-leakage grep gate before commit — a self-corrected gate hit, not a design tension). Vision: §3, §11, §14, §765-794, NG-1, NG-2, NG-3, NG-4, NG-5, NG-6, NG-7. **§20 Funding Surface Gate N/A** (no `apps/arena-client/` modification; that surface lands with WP-133 / WP-134). NG-1 (no pay-to-win) protection is structural via the Layer Boundary; the closed-set lock at the type / SQL CHECK layer is defense-in-depth. **Hard-deps satisfied:** WP-131 / EC-134 (verifier + resolver production-wired); WP-112 (`SessionVerifier` interface + `AccountResolver` type + `requireAuthenticatedSession` orchestrator); WP-115 (long-lived `pg.Pool`); WP-118 (catalog format + D-11804); WP-104 (deps-bundle shape + the `ownerProfile.logic.ts:123` two-query pattern precedent); WP-109 (`bigint`-FK-on-`player_id` + `idx_<table>_<column>` naming); WP-052 (`AccountId` brand + `legendary.players` table). **Soft-consumer:** WP-134 (Stripe fulfillment) consumes `legendary.entitlements` + the partial-unique-index idempotency primitive + the `EntitlementKey` / `source` closed sets exactly as locked here. Two-commit topology — Commit A `EC-135:` (9 production/reference); Commit B `SPEC:` (4 governance ledgers — DECISIONS.md D-13201..D-13206 inserted between D-13104 and D-13501; this WORK_INDEX.md row flipped; EC_INDEX.md EC-135 row flipped to Done; STATUS.md `### WP-132 / EC-135 Executed` block prepended). (Originally drafted 2026-05-03 as the **first WP in the Monetization Stack chain** (WP-132 → WP-133 → WP-134); lint-gate self-review **PASS** (§1–§21); EC-135 reserved (next free EC slot after EC-134 reserved by WP-131). **Project's first monetization-adjacent WP — implements the Vision §765–794 Financial Sustainability substrate at MVP.** Adds `legendary.entitlements` (many-to-1 with `legendary.players`, `ON DELETE CASCADE`) via new migration `data/migrations/011_create_entitlements.sql` with partial unique index `(account_id, entitlement_key) WHERE revoked_at IS NULL` for idempotent grant defense (matches WP-134's `ON CONFLICT DO NOTHING` clause). Closed-set `EntitlementKey` union (cosmetic-only at MVP per D-DEC-3 recommended default; six initial keys: `'supporter_tier_basic_2026'`, `'cosmetic_playmat_classic'`, `'cosmetic_playmat_comic'`, `'cosmetic_playmat_minimal'`, `'cosmetic_cardback_default_plus'`, `'cosmetic_avatar_frame_supporter'`). `source` closed set `('stripe', 'admin_grant', 'comp')`; `source_ref` per-source semantics locked (Stripe required, admin_grant optional, comp requires DECISIONS.md citation). New `apps/server/src/entitlements/` quartet (`entitlements.types.ts` / `entitlements.logic.ts` / `entitlements.routes.ts` + tests) sibling to `profile/` / `teams/`; one new HTTP endpoint `GET /api/me/entitlements` (`authenticated-session-required` per D-9905; `Cache-Control: no-store` first statement; status-code domain `{200, 401, 500}`; ordered by `grantedAt` ASC; revoked rows excluded by contract; ISO-8601 UTC timestamp serialization). **Invariant locked:** zero grant path in WP-132 — `INSERT INTO legendary.entitlements` appears nowhere under `apps/server/src/entitlements/` post-WP-132 (verified by `Select-String`); WP-134 is the only WP that mutates the table. Drift-detection test machine-enforces TS union ↔ canonical `ENTITLEMENT_KEYS` array parity per D-DEC-6 option (a); SQL CHECK parity is human-reviewed at migration time and runtime route-layer guard rejects out-of-set values. **Six DECISIONS entries** at execution: D-13201 (module path `apps/server/src/entitlements/`), D-13202 (migration slot 011), D-13203 (initial closed set), D-13204 (source closed set), D-13205 (route-wiring posture — recommended same-commit per WP-104 precedent), D-13206 (drift-test posture — compile-time exhaustive switch). 01.5 NOT INVOKED (server-layer only; engine never imports entitlements per Layer Boundary). 01.6 post-mortem OPTIONAL per WP-104 / WP-115 server-layer precedent. §17 Vision Alignment: §3 / §11 / §14 / §765–794 / NG-1..NG-7 cited; no-conflict; non-goal proximity check confirms cosmetic-only invariant at the type level. §20 Funding Surface Gate: N/A with justification distinguishing supporter-tier paid revenue (Vision §784 #1 + #2) from tournament-funding donate flows (Vision §791 #3 — WP-097's surface). §21 API Catalog TRIGGERED (one new `Wired` row + one `Library-only` row per D-11804). No new npm dependencies. Twelve files projected (3 production + 3 test + 1 migration + 5 governance/reference). Server baseline grows by ~12 tests when executed. Hard-deps: WP-052 (`AccountId` + `legendary.players` + `Result<T>` + `DatabaseClient`); WP-112 (`requireAuthenticatedSession` + `SessionVerifier` + `AccountResolver`); WP-115 (long-lived `pg.Pool` lifecycle anchor); WP-118 (HTTP API catalog + D-11804); **WP-131 strongly preferred to land first** so `/api/me/entitlements` is genuinely authenticated rather than fail-closed `'session_verifier_not_configured'`. Soft-consumer: WP-108 reframed (2026-05-03) as a UI consumer of this endpoint. Commit prefix `EC-135:` at execution. See [WP-132-entitlements-data-model-and-me-read.md](WP-132-entitlements-data-model-and-me-read.md). EC-135 not yet drafted.

- [x] WP-133 — Stripe Checkout Session Creation & Webhook Ingestion (No Fulfillment) — **Done 2026-05-05 (`EC-136:`).** New server-layer billing substrate ships in `apps/server/src/billing/` (7 new files: `billing.types.ts`, `billing.config.ts`, `billing.config.test.ts`, `billing.logic.ts`, `billing.logic.test.ts`, `billing.routes.ts`, `billing.routes.test.ts`) + migration `data/migrations/012_create_stripe_events_and_checkout_sessions.sql` (two new tables — `legendary.stripe_events` + `legendary.stripe_checkout_sessions` per D-13302 Option A FK form `account_id text REFERENCES legendary.players(ext_id) ON DELETE CASCADE`; partial index `stripe_events_unprocessed_idx` for WP-134's read path; secondary index `stripe_checkout_sessions_account_idx`). Two HTTP endpoints: `POST /api/billing/checkout-session` (`authenticated-session-required`; status `{200, 400, 401, 500, 503}`) + `POST /api/billing/webhook/stripe` (`guest`; status `{200, 400, 500}`). Webhook handler ingests EVERY signature-verified event without `event.type` filter — WP-134 is the sole classifier. WP-133 INSERTs `legendary.stripe_checkout_sessions` rows with `intent_status = 'open'` only and writes `process_error = NULL` always; WP-134 owns all transitions. Stripe SDK confined to `apps/server/src/billing/` — `server.mjs` imports the `createStripeClient(billingConfig)` factory from `billing.config.ts` rather than `Stripe` directly per the EC-136 §5 layer-boundary grep gate. Nine DECISIONS land in numeric order — D-13301 (module path), D-13302 (migration slot 012 + Option A FK form correcting the WP-133 v1.0 #FK-BUG), D-13303 (`stripe@22.1.0` exact-pin + `apiVersion: '2025-09-30.clover'` date-stamped pin + changelog citation <https://docs.stripe.com/upgrades>), D-13304 (route-level raw-body middleware with 1mb cap), D-13305 (`STRIPE_PRICE_ALLOWLIST` env var validated against `ENTITLEMENT_KEYS`), D-13306 (`event.id` UNIQUE + `ON CONFLICT DO NOTHING`), D-13307 (one-time `mode: 'payment'` only), D-13308 (defer Stripe Customer creation; pass `customer_email` only), D-13309 (env-derived `PUBLIC_BASE_URL` → server-derived `successUrl`/`cancelUrl` redirect-manipulation defense). `BillingErrorCode` 8-member closed union locked verbatim; envelope split `{ code: '<closed-set>' }` for auth/config faults vs `{ error: 'internal_error' }` for operational faults. Server baseline `pass 137 / fail 0 / skipped 59 (suites 22, tests 196)` → **`pass 168 / fail 0 / skipped 59 (suites 29, tests 227)`** (+31 = 12 config + 9 logic + 10 routes; +7 suites). Engine baseline `679 / 148 / 0` UNCHANGED. **§21 API Catalog TRIGGERED:** two new `Wired` rows in `### Server-Registered Routes` per D-11804 (WP-134 replaces the webhook row in full at WP-134 close). **`docs/ai/REFERENCE/00.2-data-requirements.md §4.1`** carries two new rows for `legendary.stripe_events` and `legendary.stripe_checkout_sessions`. **01.5 NOT INVOKED** (server-layer-only WP). **01.6 post-mortem SKIPPED with rationale** (the nine D-13301..D-13309 decisions held without contradiction; the FK-bug correction was anticipated by EC-136 §0 and resolved at PS-1 time; the Stripe SDK layer-boundary refactor — moving `new Stripe(...)` from `server.mjs` into a `createStripeClient(billingConfig)` factory exported from `billing.config.ts` — was a self-corrected EC-136 §5 grep-gate hit, not a design tension). Vision: §3, §11, §14, §15, §765-794, NG-1, NG-2, NG-3, NG-4, NG-5, NG-6, NG-7. **§20 Funding Surface Gate N/A** (paid-tier purchase plumbing, not tournament-funding donation surface; backend-only WP). NG-1 protection is structural via the Layer Boundary — engine / registry / preplan packages cannot import from `apps/server/src/billing/`; the closed-set `EntitlementKey` lock inherited from WP-132 D-13203 ensures all priced entitlements are cosmetic-only. **Hard-deps satisfied:** WP-132 (`EntitlementKey` closed union + `ENTITLEMENT_KEYS` canonical array + `legendary.entitlements` table); WP-131 / EC-134 (verifier + resolver production-wired); WP-126 (startup env-var construction pattern); WP-118 (catalog format + D-11804); WP-115 (long-lived `pg.Pool` + `Cache-Control: no-store` first-statement lock + project-owned 500 envelope); WP-112 (`SessionVerifier` + `AccountResolver` + `requireAuthenticatedSession`); WP-104 (sibling-flat module structure + caller-injected deps-bundle pattern); WP-052 (`AccountId` + `legendary.players` + `ext_id` ↔ `AccountId` mapping per D-5201). **Soft-consumer:** WP-134 (Stripe fulfillment) consumes `legendary.stripe_events` (read via `WHERE processed_at IS NULL` partial index; flips `processed_at` + writes `process_error`) and `legendary.stripe_checkout_sessions` (transitions `intent_status` + writes `completed_at` in the same transaction that INSERTs into `legendary.entitlements`). Two-commit topology — Commit A `EC-136:` (production/reference); Commit B `SPEC:` (4 governance ledgers). (Originally drafted 2026-05-03 as the **second WP in the Monetization Stack chain**; lint-gate self-review **PASS** (§1–§21); EC-136 reserved. Ships **the Stripe wiring half** of the monetization flow without any consequence on entitlement state — webhook events are recorded but NEVER acted on (WP-134 owns fulfillment). New migration `data/migrations/012_create_stripe_events_and_checkout_sessions.sql` creates `legendary.stripe_events` (UNIQUE on Stripe `event_id` per D-DEC-6; `payload jsonb` stores **the full Stripe event envelope** verbatim from `stripe.webhooks.constructEvent`, NOT just `event.data.object` — preserves `api_version` for forensic drift detection) and `legendary.stripe_checkout_sessions` (UNIQUE on Stripe `session_id`; carries denormalized `account_id` + `price_id` + `entitlement_key` for WP-134 lookup; `intent_status` closed-set CHECK `('open', 'completed', 'expired', 'canceled')`). New `apps/server/src/billing/` module (sibling to `entitlements/`) with `billing.types.ts` / `billing.config.ts` / `billing.logic.ts` / `billing.routes.ts` + tests. Two new HTTP endpoints: `POST /api/billing/checkout-session` (`authenticated-session-required`; request body shape **exactly `{ priceId: string }`** — extra fields like `successUrl` / `cancelUrl` return 400 `invalid_request` per D-DEC-9 redirect-manipulation defense) and `POST /api/billing/webhook/stripe` (`guest` per WP-115 precedent — Stripe signature IS the auth; raw-body verification via `stripe.webhooks.constructEvent` per D-DEC-4 route-level `koa-body` raw mode; response `{ received: true, duplicate: boolean }` on signature-verified events; WP-134 extends this to add `processed` + `reason` fields per replace-whole-row at WP-134 close). **Invariant locked:** WP-133's webhook handler writes only to `legendary.stripe_events`; `INSERT INTO legendary.entitlements` appears nowhere under `apps/server/src/billing/` post-WP-133 (verified by `Select-String`); WP-133 also INSERTs `legendary.stripe_checkout_sessions` rows with `intent_status = 'open'` only — NO transition to `'completed'` / `'expired'` / `'canceled'` (WP-134 owns the lifecycle). **One new npm dependency:** `stripe` SDK, exact-pinned per D-DEC-3; `apiVersion` parameter pinned in `billing.config.ts` as a **security and correctness invariant** (Stripe webhook payloads are versioned; unpinned versions risk silent shape drift that breaks WP-134's parser); D-13303 entry MUST cite the Stripe API version changelog URL. **Three new env vars:** `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` + `STRIPE_PRICE_ALLOWLIST` (comma-separated `priceId:entitlementKey` pairs validated against `ENTITLEMENT_KEYS` at startup; production-mode missing-env exits 1 mirroring WP-126 / WP-131 startup guard); a fourth `PUBLIC_BASE_URL` lands if executor picks D-DEC-9 = (a) recommended default. `BillingConfig` returned via `Object.freeze` + `ReadonlyMap<string, EntitlementKey>` typing prevents runtime mutation of the price allowlist (mid-request reassignment caught by freeze; `.set()` calls caught by TS). **Nine DECISIONS entries** at execution: D-13301 (module path), D-13302 (migration slot 012), D-13303 (Stripe SDK exact-pin + API version + changelog citation), D-13304 (raw-body middleware approach), D-13305 (price allowlist source = env var), D-13306 (Stripe Customer creation deferred to future WP), D-13307 (one-time payments only at MVP — subscriptions deferred), D-13308 (idempotency key = `event.id` UNIQUE), D-13309 (`successUrl` / `cancelUrl` server-derived only — redirect-manipulation defense). 01.5 NOT INVOKED. 01.6 post-mortem MANDATORY at execution per at least three triggers (new long-lived abstraction `BillingConfig` / `stripeClient` startup construction; new code seam `apps/server/src/billing/`; new contract surfaces `BillingResult<T>` + `stripe_events.payload` envelope schema). §17 Vision Alignment: §3 / §11 / §14 / §15 / §765–794 (#2 one-time cosmetic purchases) / NG-1..NG-7 cited; no-conflict. §20 Funding Surface Gate: N/A with justification (paid-tier purchase plumbing, not tournament-funding donate surface; "Become a Legendary Supporter" copy ≠ donation copy). §21 API Catalog TRIGGERED (two new `Wired` rows). Eighteen files projected (8 production + 4 test + 1 migration + `package.json` + `pnpm-lock.yaml` + `.env.example` + `render.yaml` + 5 governance/reference). Server baseline grows by ~25 tests when executed. Hard-deps: WP-052 / WP-112 / WP-115 / WP-118 / **WP-132 (consumed `EntitlementKey` closed union for the price allowlist)**; WP-131 ideally lands first (auth posture); webhook ingestion route works regardless. Commit prefix `EC-136:` at execution. See [WP-133-stripe-checkout-session-and-webhook-ingestion.md](WP-133-stripe-checkout-session-and-webhook-ingestion.md). EC-136 not yet drafted.

- [x] WP-134 — Webhook → Entitlement Fulfillment Processor — **Done 2026-05-07 (`EC-140:`; commits `b281744` + `ddd6e3c`).** Closed-loop monetization is now LIVE for cosmetic SKUs only. New `apps/server/src/billing/processStripeEvent.logic.ts` exports `processStripeEvent({ eventRecord, billingConfig, database })` with a 4-phase body (Phase 0a structural type guard for `payload: unknown`, Phase 1 early-return guards, Phase 2 five-axis cross-validation, Phase 3 transactional fulfillment write). Idempotent INSERT clause `ON CONFLICT (player_id, entitlement_key) WHERE revoked_at IS NULL DO NOTHING RETURNING id` matches WP-132's partial unique index byte-for-byte (#CONFLICT-TARGET correction); `accountId → player_id` resolution at Phase 3 step 6 mirrors the WP-104 / EC-135 `ownerProfile.logic.ts:123` two-query precedent verbatim (#ENT-FK correction). Locked write ordering (INSERT entitlement → UPDATE session → UPDATE event) inside `BEGIN; ... COMMIT;` envelope with step 10 (event row's `processed_at = now()`) as the LAST write so partial-write crashes recover via the recovery script's `WHERE processed_at IS NULL` selector. WP-133's `recordStripeEvent` is NOT modified — PS-1 path (a) re-fetch via shared local helper `loadStripeEventRecordByEventId(pool, eventId)` used by both the newly-inserted branch and the duplicate-delivery branch in `billing.routes.ts`. Webhook handler dispatches `processStripeEvent` synchronously after `recordStripeEvent` per D-13401; response shape extends to `{ received, duplicate, processed, reason }` per D-13402 with `processed: true ↔ reason ≠ null` invariant; always-200 on signature-verified events per D-13404 (row-absent edge case is the SOLE 500 exception). New `scripts/process-stripe-events.mjs` recovery script with two-phase lifecycle per D-13405 (startup-fatal exit 2 / scan-loop-tolerant exit 0 / pool teardown via `try { ... } finally { await pool.end(); }` / Stripe SDK confinement preserved by importing `loadBillingConfig` from `billing.config.ts`); tsx registered programmatically via `tsx/esm/api` so the spec'd `node --env-file=.env scripts/process-stripe-events.mjs` invocation resolves TypeScript imports without an explicit `--import tsx` flag. `INSERT INTO legendary.entitlements` appears EXACTLY once under `apps/server/src/billing/` (verified by Grep). Five DECISIONS entries (D-13401..D-13405) inserted in numeric order between D-13309 (WP-133) and D-13501 (WP-135); D-13403 is the bundled architectural lock covering the five-axis cross-validation + Phase 0a guard + accountId→player_id resolution + (player_id, entitlement_key) conflict target + transactional posture + path (a) re-fetch helper. API catalog (`docs/ai/REFERENCE/api-endpoints.md`) updated per D-11804 (webhook row replaced wholesale with extended response schema + `Authorizing WP: WP-133, WP-134` joint authorship; one new `Library-only` row appended for `processStripeEvent`). Server baseline `pass 168 / fail 0 / skipped 59 / suites 29 / tests 227` → `pass 184 / fail 0 / skipped 66 / suites 31 / tests 250` (+16 pass, +7 skipped DB-required, +2 suites, +23 tests — matches EC-140 §2 lock exactly). Engine baseline `698 / 0 / 0 / 150 / 698` UNCHANGED (engine never touched; 01.5 NOT INVOKED per server-layer-only WP). 01.6 post-mortem skipped with rationale recorded in STATUS block (server-layer-only WP that landed exactly on the §2 lock; the only deviations — EC §6 `line_items` recursive grep over-broadness and the recovery script's tsx-via-`tsx/esm/api` registration — are non-blocking and documented). Two-commit topology — Commit A `EC-140:` (5 production/test files + 1 reference doc); Commit B `SPEC:` (4 governance ledgers). Vision: §3, §11, §14, §15, §765-794, NG-1, NG-2, NG-3, NG-4, NG-5, NG-6, NG-7. Commit hashes recorded post-execution.

- [ ] WP-134 — (legacy draft text follows; superseded by execution above) Drafted 2026-05-03 as the **third WP in the Monetization Stack chain**; lint-gate self-review **PASS** (§1–§21); EC-137 reserved. Ships the **single fulfillment processor** that reads recorded Stripe events and grants entitlements. New `apps/server/src/billing/processStripeEvent.logic.ts` exports `processStripeEvent({ eventRecord, billingConfig, database })` returning `Result<FulfillmentSuccess>` matching WP-052's locked `Result<T>` shape verbatim. **WP-134 introduces no new migration** — consumes WP-132 + WP-133 schema. **WP-134 introduces no new HTTP endpoint** — modifies WP-133's existing webhook handler body. The catalog row for `POST /api/billing/webhook/stripe` is replaced **wholesale per D-11804** (response schema gains `processed` + `reason` closed-set fields; `Authorizing WP` becomes `WP-133, WP-134` joint authorship). New `scripts/process-stripe-events.mjs` recovery script reads unprocessed events via `WHERE processed_at IS NULL ORDER BY received_at ASC LIMIT 100` and drives the same processor (manual + Render cron at 15-minute cadence per D-DEC-5 recommended default). **Three locked invariants** (the WP's correctness floor): (1) **`legendary.entitlements` INSERT site is exactly one location** under `apps/server/src/billing/` (verified by `Select-String`) — `processStripeEvent.logic.ts` step 7; uses `ON CONFLICT (account_id, entitlement_key) WHERE revoked_at IS NULL DO NOTHING` matching WP-132's partial unique index for idempotent retry; (2) **`processed_at` lifecycle**: set ONLY on terminal success / no-op (`'fulfilled'` / `'duplicate'` / `'unhandled_event_type'` / `'unpaid_session'` / `'already_processed'`); failures leave `processed_at = NULL` with `process_error` set so the recovery script's filter continues to surface failed rows for retry; (3) **Write ordering**: entitlement INSERT → sessions UPDATE → stripe_events UPDATE last (within a `BEGIN; ... COMMIT;` transaction where `DatabaseClient` exposes one — matches WP-104 `PUT /api/me/links` precedent), so a partial-write crash heals on the next recovery pass via idempotency. **Two-axis cross-validation** (per D-DEC-3) before INSERT: `event.payload.data.object.client_reference_id` ↔ `sessionRow.account_id` AND `event.metadata.entitlementKey` ↔ `sessionRow.entitlement_key` ↔ `priceAllowlist.get(sessionRow.price_id)`. **Authoritative source is the session row + allowlist**, NOT event payload — event metadata is a consistency check only; the processor MUST NOT rely on `payload.line_items` (Stripe omits without explicit expansion, which D-DEC-1 forbids in the processing path). **Duplicate-delivery self-heal** (per D-DEC-3 routes amendment): on `recordStripeEvent` returning `inserted: false`, the webhook handler loads the existing event row and inspects `processed_at` — if NULL, calls `processStripeEvent` against the existing row (turns the duplicate into a retry opportunity); if non-NULL, skips. **Always-200 on signature-verified events** per D-DEC-4 (Stripe-driven retries can compound during incidents; recovery script is the controlled retry mechanism). **Five DECISIONS entries** at execution: D-13401 (synchronous-on-webhook posture at MVP scale), D-13402 (response schema extension `+ processed + reason`), D-13403 (two-axis cross-validation strictness), D-13404 (always-200 on signature-verified events), D-13405 (recovery scheduling = manual + Render cron 15-minute cadence). 01.5 NOT INVOKED. 01.6 post-mortem MANDATORY at execution per at least two triggers (new long-lived abstraction `processStripeEvent` fulfillment processor; new contract surface `FulfillmentResult` + `FulfillmentSuccessReason` + `FulfillmentErrorCode` closed unions). §17 Vision Alignment: §3 (two-axis cross-validation defends fairness even after Stripe signature verification) / §11 / §14 / §15 / §765–794 / NG-1..NG-7 cited; no-conflict; cosmetic-only invariant inherits from WP-132's `EntitlementKey` closed set by construction. §20 Funding Surface Gate: N/A with justification (backend-only fulfillment processor for paid-tier purchases; no §20.1 trigger surface). §21 API Catalog TRIGGERED (replace-whole-row update on the WP-133 webhook row + one new `Library-only` row for `processStripeEvent` per D-11804). **Documented trade-off (operator runbook):** validation-class failures (`'session_lookup_failed'` / `'cross_validation_failed'` / `'price_not_in_allowlist'`) are deterministically terminal but still leave `processed_at = NULL` per locked semantic — they loop in cron stderr noise as the operator-visibility signal; manual `UPDATE stripe_events SET processed_at = now()` is the resolution path once the cause is understood. Future WP may add a `retry_count` column or terminal-fault sentinel to mechanically distinguish transient (DB) from terminal (validation) failures; out of scope for WP-134. Nine files projected (1 new processor + 1 new processor test + 2 modified routes/test files + 1 new recovery script + 4 governance ledgers). Server baseline grows by ~13 tests when executed (covering the 13 documented branches: already-processed, unhandled-type, unpaid, success, duplicate idempotency, session miss, account mismatch, entitlement-key mismatch, price-allowlist drift, INSERT fault, session-update fault, event-update fault, crash-recovery partial-write resume). Hard-deps: WP-052 / WP-115 / WP-118 / **WP-132 (`legendary.entitlements` table + `EntitlementKey` closed union — the WP-134 INSERT site WP-132 deferred) + WP-133 (`legendary.stripe_events` + `legendary.stripe_checkout_sessions` + `BillingConfig.priceAllowlist` + Stripe SDK + webhook ingestion route)**. After WP-134 closes, the full purchase-to-entitlement loop is live for cosmetic SKUs; user clicks "Upgrade" (future arena-client WP) → server creates Checkout Session (WP-133) → user pays at Stripe-hosted Checkout → Stripe sends `checkout.session.completed` → WP-133 ingests + WP-134 fulfills inline → entitlement row created → `GET /api/me/entitlements` (WP-132) returns the new row. Commit prefix `EC-137:` at execution. See [WP-134-webhook-entitlement-fulfillment.md](WP-134-webhook-entitlement-fulfillment.md). EC-137 not yet drafted.

- [x] WP-135 — HQ Population & Hero Deck Reservoir. **Executed 2026-05-04** under EC-138 (two-commit topology — 01.5 cascade fired; pre-edit `PRE_WP080_HASH = '46f7863c'` → post-edit `'2baeecc3'`). D-13501..D-13503 landed in numeric order. Engine baseline `621/135/0` → `679/148/0` (+58 tests, +13 suites). Commit A `5417c54` (`EC-138:` — production + cascade); Commit B `ff91c61` (`SPEC:` — governance close + post-mortem). — Drafted 2026-05-04 in response to WP-129 / EC-132 smoke-test surfacing the engine gap inherited from WP-016. EC-138 reserved (next free EC slot after the Monetization Stack chain WP-131..134 reserved EC-134..137). **First sub-WP closing the deferred-placeholder cluster below** (the `G-state extensions for board layout` placeholder lists eight WP-128 D-12806 safe-skip sites; this WP closes ONE — `decks.heroDeckCount`). Engine-side packet that builds a deterministic per-match hero-deck reservoir at `G.heroDeck: CardExtId[]` from `MatchSetupConfig.heroDeckIds`, shuffles it via `ctx.random.Shuffle`, populates 5 HQ slots from the first 5 cards at `Game.setup()`, and refills slots from the deck on each successful `recruitHero`. Closes the obsolete `WP-016 scope` comment at `buildInitialGameState.ts:308-309` (the WP-016 deferred this; never landed). Extends `G.cardStats` and `G.cardDisplayData` with hero card instances (cost, attack, recruit, name, imageUrl) so cost-gating and display-name resolution work end-to-end at the new ext_id format `<setAbbr>/<heroSlug>/<cardSlug>` (D-DEC-2). Three executor decisions land at execution as **D-13501..D-13503** in numeric order: D-13501 (rarity → copy-count map, recommended default 5/3/3/3 per Marvel Legendary canonical 14-cards-per-hero rule), D-13502 (hero card instance ext_id format `<setAbbr>/<heroSlug>/<cardSlug>` per D-10014 set-qualification), D-13503 (empty-deck recruit behavior — vacated slot stays `null`; no auto-reshuffle of recruited cards back into the shared hero pool). Graduates `decks.heroDeckCount` projection from the WP-128 D-12806 safe-skip constant `0` to `gameState.heroDeck.length` at exactly one site in `uiState.build.ts`; SAFE-SKIP-WP128 marker count drops from 8 to 7. **01.5 IS INVOKED** as a conditional cascade per WP-128 D-12807 ("Future contributor note": adding `G.heroDeck` changes `computeStateHash` inputs; binary procedure — capture `PRE_WP080_HASH` pre-edit, run full engine test post-edit, update literal IFF hash diverges). 01.6 post-mortem MANDATORY (new long-lived `G` field; new contract surface `buildHeroDeck` factory; new sibling-snapshot site for the rarity-driven hero-card-instance walk). Engine baseline `621/135/0` → projected `621+N/135+M/0` with N+M ∈ [12, 25] new tests. Projected file count ~22 (3 new + 9 modified production + 6 modified test + 4 governance + 1 post-mortem; 23rd file if 01.5 cascade fires). Hard-deps: WP-016 (`recruitHero` move existence — extended here); WP-018 (`buildCardStats` sibling-snapshot pattern); WP-111 (`buildCardDisplayData` sibling-snapshot pattern); WP-128 (`UIDecksState.heroDeckCount` projection — graduates here); WP-129 (board-layout client surface — visible consumer of HQ + heroDeckCount; no client modification required by this packet). Soft-consumers: future engine WPs that close the remaining 7 safe-skip sites (`mastermind.attachedBystanders`, `mastermind.strikePile`, `scheme.twistPile`, `city.escapedPile`, `economy.piercing`, `economy.woundsDrawn`, `piles.horrorsCount`) — each closes in its own WP. **Once WP-135 lands, the engine's shipped state is no longer "structurally unwinnable" via starter cards alone** (Magneto's 4 tactics × 6 attack = 24 attack to win; HQ population enables hero recruitment which is the only path to that attack output). Commit prefix `EC-138:` at execution. See [WP-135-hq-population-and-hero-deck-reservoir.md](WP-135-hq-population-and-hero-deck-reservoir.md) + [EC-138-hq-population-and-hero-deck-reservoir.checklist.md](../execution-checklists/EC-138-hq-population-and-hero-deck-reservoir.checklist.md).

- [x] WP-136 — JSDOM Opaque-Origin Storage Fix in arena-client Test Harness. **Executed 2026-05-04** under EC-139 (two-commit topology — 01.5 NOT INVOKED; 01.6 post-mortem SKIPPED with rationale). Commit A `28284b3` (`EC-139:` — harness fix + 4 shim retirements + WP/EC mid-execution reconciliation); Commit B (`SPEC:` — governance close). D-13601 inserted at execution. arena-client baseline `286/35/0` preserved exactly. Engine baseline `679/148/0` UNCHANGED. — Drafted 2026-05-04 to retire four inline `MemoryStorage` shim copies introduced by WP-130 / EC-133 because the WP-130 modify allowlist excluded `apps/arena-client/src/testing/`. Adds `{ url: 'http://localhost/' }` to the JSDOM constructor in `apps/arena-client/src/testing/jsdom-setup.ts` so the test window carries a non-opaque tuple origin and surfaces a working `window.{localStorage,sessionStorage}` to consumers under WHATWG Storage rules. **Mid-execution amendment 2026-05-04 PS-1:** the URL fix alone produced a working `dom.window.localStorage` but did not surface a working bare `localStorage` on `globalThis` — production code (`persistence.ts:58,80,83`) and the four prefs/composables/components tests read bare `localStorage`, which resolves through `globalThis`, not `dom.window`. Two `installGlobal('localStorage', dom.window.localStorage)` / `installGlobal('sessionStorage', dom.window.sessionStorage)` calls were appended to `jsdom-setup.ts`, mirroring the established WP-065 install pattern verbatim; the WP and EC files were amended in the same commit (WP §Goal + §Non-Negotiable Constraints + §Scope (In) A + §Acceptance Criteria; EC §Guardrails + §Required `// why:` Comments + §Files to Produce + §Common Failure Smells with the `ReferenceError: localStorage is not defined` smell as the new top entry). After WP-136 lands, any future arena-client test that depends on `Storage` simply imports `jsdom-setup` and uses the native API (via either bare `localStorage` or `window.localStorage`) without per-file boilerplate. Five code files modified plus two governance files (WP-136 + EC-139) all in Commit A; four governance ledgers (DECISIONS, WORK_INDEX, EC_INDEX, STATUS) in Commit B. **Lessons learned (PS-1):** when retiring a shim that wrote to `globalThis`, verify the replacement mechanism delivers identity to the same location, not to a sibling object. Hard-deps: WP-065 (`jsdom-setup.ts` precedent — original installer for component tests); WP-130 (introduced the four inline shims this packet retires). Commit prefix `EC-139:`. See [WP-136-jsdom-opaque-origin-storage-fix.md](WP-136-jsdom-opaque-origin-storage-fix.md) + [EC-139-jsdom-opaque-origin-storage-fix.checklist.md](../execution-checklists/EC-139-jsdom-opaque-origin-storage-fix.checklist.md).

- [x] WP-137 — Hero Card-Instance Distinctness + Data-Driven cardCounts — **Done 2026-05-07 (`EC-137:`; commits `aa6ee70` + `4ce7a71`).** Engine baseline `679 / 148 / 0` → **`698 / 150 / 0`** (+19 tests; suite delta +2 — one new `describe()` in `buildHeroDeck.test.ts` for cardCounts resolution + one in `invariants.test.ts` for the 100-seed regression — per RS-3). Registry baseline `31 / 3 / 0` → **`32 / 3 / 0`** (+1 cardCounts smoke). D-13701/D-13702/D-13703 inserted in numeric order. **01.5 conditional cascade NOT FIRED** — `replay.execute.test.ts:54 PRE_WP080_HASH = '2baeecc3'` byte-identical post-WP-137 (replay regression-guard fixture uses an empty mock registry, so per-copy fan-out adds zero entries to G.cardStats / G.cardDisplayData; computeStateHash inputs unchanged). **PS-5 mid-execution amendment** authorized 2026-05-06: pre-flight §6 test allowlist was incomplete; `buildInitialGameState.loadout.test.ts` + `buildInitialGameState.determinism.test.ts` added (mechanical literal-string updates to grammar; no logic, helpers, matchers, or new tests). Total scope 21 base files (no cascade). 01.6 post-mortem authored at `docs/ai/post-mortems/01.6-WP-137-hero-card-instance-distinctness-and-data-driven-card-counts.md`. Drafted 2026-05-06 to fix the per-copy ext_id collision bug surfaced by WP-135 (every physical copy of a hero card was pushed into the reservoir under the same ext_id string, causing `checkNoCardInMultipleZones` to trip RNG-dependently when shuffle dealt duplicate copies into HQ) and to consume the now-populated registry `cardCounts` field made authoritative by Phase A (commit `cdd37aa` 2026-05-06 — card-data conversion pipeline migrated in-repo to `scripts/convert-cards/`; cardCounts populated for 36 npm-derived sets via `convert-cards-v15.mjs` and 4 outliers via `apply-card-counts.mjs`). Pre-flight 2026-05-06 issued **NOT READY** initially with four blocking PS items (PS-1 versioning file path drift, PS-2 migration model mismatch, PS-3 `replay.execute.test.ts` cascade not in allowlist, PS-4 this WORK_INDEX row missing) and six RS clarifications; all eight resolved in-place via WP/EC text + allowlist edits 2026-05-06. **Closes the deferred placeholder below** — promotes data-driven counts (`HeroSchema.cardCounts`) over rarity-map extension to AMWP-class labels; the placeholder row stays in place at execution time, replaced with a back-reference to WP-137. Appends `#<copyIndex>` (decimal, zero-indexed, contiguous) to hero card-instance ext_ids in `buildHeroDeckCards`; fans out `G.cardStats` and `G.cardDisplayData` per copy with a shared exported helper `resolveHeroCardCopyCount` defined in `setup/buildHeroDeck.ts` and imported by `economy/economy.logic.ts` and `setup/buildCardDisplayData.ts` (RS-4 lock); name-keyed `cardCounts` lookup against `card.name` (NOT slug) plus rarity-map fallback on `card.rarityLabel`; **engine version bump 1.0.0 → 1.1.0** in `package.json` + `versioning.check.ts:CURRENT_ENGINE_VERSION_VALUE` lockstep; **data axis bump 1 → 2** in `versioning.check.ts:CURRENT_DATA_VERSION`; **first migration registered** at key `'1.0.0->1.1.0'` mapping to `migrateHeroExtIdsForCopyIndex` in `versioning.migrate.ts` (best-effort schema compatibility only — pre-WP-137 replays expected to fail re-verification); adds additive optional `cardCounts: Record<string, number> | null` to `HeroSchema` in `packages/registry/src/schema.ts`. Three D-decisions land at execution: **D-13701** (cardCounts authoritative when present + schema additive change), **D-13702** (`<setAbbr>/<heroSlug>/<cardSlug>#<copyIndex>` ext_id grammar extending D-13502), **D-13703** (deferred Phase 7 placeholder closure — promotes data-driven counts over rarity-map extension to AMWP-class labels). Hard-deps: WP-135 (`buildHeroDeck` reservoir + `RARITY_COPY_COUNT` map + ext_id format under D-13501/02), WP-018 (`buildCardStats` sibling-snapshot pattern), WP-111 (`buildCardDisplayData` sibling-snapshot pattern), WP-113 (replay-hash regression guard + qualified-ID alignment). Cross-cutting precondition asserted in `## Assumes`: Phase A landed (cdd37aa reachable); `data/cards/2099.json/heroes[0].cardCounts` populated; `data/cards/core.json/captain-america.cardCounts` populated. **Known data anomaly (RS-1, documented in WP §Known Data Anomaly):** `core.captain-america.cardCounts` ships `{Avengers:3, Teamwork:4, Diving:6, Day:7}` (sum 20) in the generated data file but the upstream input `scripts/convert-cards/inputs/hero-card-counts.json` says `{Avengers:5, Teamwork:5, Diving:3, Day:1}` (sum 14, canonical). WP-137 ships with the pipeline drift baked in; a separate follow-up data-layer task investigates `convert-cards-v15.mjs` and re-runs the pipeline to regenerate `data/cards/*.json` with corrected values. After that lands, captain-america's reservoir contribution returns to 14 with no engine code change. Until then, captain-america loadouts play with a 20-card hero deck — does not block WP-137's bug-fix surface (per-copy ext_id distinctness + invariant compliance across all sets). Engine baseline `679 / 148 / 0` → projected `(679 + N) / 150 / 0` with `N ≥ 8` new tests and **suite delta locked at +2** (RS-3) — one new `describe()` in `buildHeroDeck.test.ts` for cardCounts resolution, one in `gameRules.checks.test.ts` for the 100-seed regression. **01.5 IS INVOKED** as a conditional cascade per WP-128 D-12807 / WP-135 cascade template — the per-copy fan-out in `G.cardStats` and `G.cardDisplayData` changes `computeStateHash` inputs; `replay.execute.test.ts:54 PRE_WP080_HASH = '2baeecc3'` literal expected to need regeneration (capture pre-edit, run engine test post-edit, update IFF hash diverges; record pre/post pair in WP-137 §Verification Steps Step 7). 01.6 post-mortem MANDATORY (new ext_id grammar surface; first migration registered in `migrationRegistry`; first time engine consumes registry `cardCounts` data). Vision: §3 (determinism strengthened — RNG-dependent `checkNoCardInMultipleZones` violation removed), §22 (replay faithfulness preserved via versioning); NG-1..7 not crossed. Commit prefix `EC-137:` at execution. See [WP-137-hero-card-instance-distinctness-and-data-driven-card-counts.md](WP-137-hero-card-instance-distinctness-and-data-driven-card-counts.md) + [EC-137-hero-card-instance-distinctness-and-data-driven-card-counts.checklist.md](../execution-checklists/EC-137-hero-card-instance-distinctness-and-data-driven-card-counts.checklist.md) + pre-flight + copilot check at `docs/ai/invocations/preflight-wp137.md` and `docs/ai/invocations/copilot-check-wp137.md`.
...
- [ ] WP-147 — PhysicalCard `companionSlug` + Physical-Side Order. **Drafted 2026-05-10; not yet executed; lint gate self-review PASS (21 sections clean, including §17 Vision Alignment with clauses cited and §20/§21 N/A justifications); copilot check returned APPROVED — READY TO EXECUTE with surgical edits applied inline.** Extends the WP-138 `physicalCards[]` surface (D-13801/13802/13803) with a per-physical-card `companionSlug?: string` optional field so the printed artwork's named non-hero companion character (e.g., Drax's Irani Rael / Rhomann Dey on the cost-4 + cost-6 split cards in `mgtg`) appears between heroSlug and side slugs in the canonical `imageUrl` filename. Two DECISIONS land at execution: **D-14701** introduces `companionSlug` (Zod `.optional()`, regex `^[a-z0-9-]+$`, full-sentence error message; Drax/Irani Rael + Drax/Rhomann Dey is the first application; future split or companion cards across the registry follow the same pattern); **D-14702** overrides D-13802's `Array.prototype.sort()` lock **narrowly for `sides.length === 2`** in favor of source-data physical-side order (side A on the left/top of the printed card is first in the array). D-14702 is scoped tightly: D-13802's sort lock remains in effect for single-side filenames and any future automatic ordering operation; the override applies only to two-side cards where the JSON's `sides[]` array is authoritative (determinism preserved because the JSON file is identical across environments, replacing mechanical UTF-16 ordering with source-data ordering). **Architecture shift:** `heroImageUrl()` and `R2_BASE_URL` relocate from `scripts/convert-cards/convert-cards-v15.mjs` (local definitions deleted) into a new `packages/registry/src/heroImageUrl.ts` with proper TypeScript types + 10 `.test.ts` cases (signature, side-order preservation, companion insertion, length guards, companionSlug regex, empty-string guard, determinism duplicate-call assertion). Convert script imports both via `../../packages/registry/dist/heroImageUrl.js` (no workspace plumbing — operational contract already requires `pnpm --filter @legendary-arena/registry build` to have run; native Node module-resolution error is the operator preflight signal, backed by Verification Step 1b `Test-Path`). **Locked literal values for the Drax fix** (mgtg.json hero.slug stays `"drax"`, hero.name stays `"Drax"` — companion is per-physicalCard, NOT a hero pairing): p1 becomes `{ id:"p1", count:2, sides:["remove-his-spine","also-illegal"], companionSlug:"rhomann-dey", imageUrl:"…mgtg/mgtg-hr-drax-rhomann-dey-remove-his-spine-also-illegal.webp" }`; p3 becomes `{ id:"p3", count:3, sides:["i-am-invisible","xandar-is-invincible"], companionSlug:"irani-rael", imageUrl:"…mgtg/mgtg-hr-drax-irani-rael-i-am-invisible-xandar-is-invincible.webp" }`. p2/p4/p5/p6 unchanged; no other hero in `mgtg.json` touched; no other set's JSON touched. **Bit-identity invariant:** the 1245 existing single-side `imageUrl` values remain bit-identical after the change (single-side path semantically unchanged); the 39 existing two-side `imageUrl` values remain bit-identical because their current `sides[]` arrays are alphabetical by construction (audit of physical-side correctness for the 37 non-Drax two-side entries is **explicit out-of-scope follow-up** — separate WP). Grep-enforced invariant: every `heroImageUrl(...)` call site in `convert-cards-v15.mjs` passes four positional arguments (pass `undefined` explicitly when no companion source); Verification Step 4b detects 3-arg calls; Step 4c is a positive sanity match. Hard-deps: WP-137 (cardCounts authority), WP-138 (physicalCards introduction — D-13801/13802/13803), WP-140 (`_skipPair[]` annotation grammar — round-trip cleanly). 01.5 NOT INVOKED (engine-zero WP; no `LegendaryGameState` shape change; no move added; no phase hook; no replay-hash cascade). 01.6 post-mortem advisory at execution discretion (no triggers fire — additive optional schema field, narrowly-scoped DECISIONS override, single hero data fix). Test-count delta: **+14 tests** (+4 schema in `registry.smoke.test.ts`, +10 in new `heroImageUrl.test.ts`). Engine baseline projected UNCHANGED; registry baseline `39 / 4 / 0` → `~53 / ~5 / 0` (+14 tests / +1 suite). §17 cited per `00.3 §17.1` triggers (card data + card images — Vision §2 Content Authenticity, §10a Registry Viewer); §20 N/A (schema + script + data only — no global navigation / viewer / profile / tournament-funding surfaces); §21 N/A (no HTTP endpoints, no `apps/server/src/**` library functions). NG-1..NG-7 not crossed. Out of scope: **host migration `images.barefootbetters.com` → `images.legendary-arena.com`** (orthogonal future WP — `R2_BASE_URL` relocates here to set up that future single-point change), audit of the 37 other two-side physicalCards' physical-side correctness, R2 image renames for the two affected Drax files, schema raises on the `sides.length` ceiling. See [WP-147-physical-card-companion-slug-and-side-order.md](WP-147-physical-card-companion-slug-and-side-order.md) + [EC-150-physical-card-companion-slug-and-side-order.checklist.md](../execution-checklists/EC-150-physical-card-companion-slug-and-side-order.checklist.md). Commit prefix `EC-150:` at execution.

- [x] WP-138 — Physical Card Abstraction Layer (Split-Side Hero Cards) — **Phase 1a — Done 2026-05-08 (`EC-141:` Commit A `763f84b`; SPEC governance close in companion commit). Phase 1b/2/3 deferred to follow-up WPs.** Registry baseline `32 / 3 / 0` → **`39 / 4 / 0`** (+7 tests / +1 suite — exactly at the EC-141 §Locked Values lock; one new `describe('physicalCards (WP-138 Phase 1a)')` block in `packages/registry/src/registry.smoke.test.ts`). **Engine baseline `698 / 150 / 0` UNCHANGED** at execution (proves no consumer migration; Phase 1a scope-clean). 01.5 NOT INVOKED — registry-only change, no LegendaryGameState shape change, no engine move addition, no phase hook. D-13801..D-13806 preserved verbatim from draft (no re-authoring at execution); cited from `// why:` comments at the schema definition sites (10 anchor hits in schema.ts). All 40 `data/cards/*.json` regenerated via `convert-cards-v15.mjs` (36 npm-derived sets through extended pipeline) + `apply-card-counts.mjs` (4 outliers — 2099, amwp, wpnx, wtif — with apply-card-counts extended in lockstep to populate `physicalCards[]` via solo-auto-path because the four outliers have no `inputs/cards/*.js` source). Falcon-Winter-Soldier reference fixture validates: `physicalCards.length === 4` (3 split + 1 solo), sum count === 14, image URLs sorted via UTF-16 code-unit ordering per D-13802 (`atone-attune`, `reload-relocate`, `new-plan-new-wings`, `captain-americas-legacy`). Solo heroes (Spider-Man, etc.) get `physicalCards.length === cards.length` with every entry `sides.length === 1` per D-13803 uniform model. Un-curated split heroes (gamora in mgtg, etc.) fall through to solo-auto-path under Phase 1a — structurally valid but semantically wrong (deck size still over-counts because physicalCards mirrors cards 1:1 instead of grouping faces); non-blocking because no consumer reads physicalCards[] as authoritative yet. **`HeroCardSchema.imageUrl` PRESERVED** under Phase 1a (Phase 3 follow-up WP removes it). Convert pipeline: non-strict exits 0 with **262 audit warnings** (Phase 1b worklist surface; every paired-equal `cardCounts` pattern lacking explicit `physicalCards[]` declaration is named with set/hero/value/cards/patch-file); `--strict` (or `LEGENDARY_CONVERT_STRICT=1`) exits **non-zero** as expected (CI green-state requires Phase 1b). Drift validation enabled at registry load via `HeroSchema.superRefine` enforcing orphan-side rejection (WP-138 §8), duplicate-membership rejection (WP-138 §9), and drift detection (sum of physicalCards counts per side === cardCounts[sideName] when populated) — all three fail load with full-sentence errors. Runtime `sideToPhysicalCard: Map<string, PhysicalCard>` cache exposed via `CardRegistry.getPhysicalCardForSide(heroSlug, sideSlug)` with namespaced compound key `<heroSlug>/<sideSlug>` per D-13806 (registry-internal cache, never persisted). Single Phase 1a reference patch landed: `bkwd / falcon-winter-soldier` per §7.8 lock. **Scope-clean:** `git diff --stat packages/game-engine/ apps/registry-viewer/ apps/arena-client/ apps/server/` empty post-execution. One scope addition not in EC-141 §Files to Produce: `scripts/convert-cards/apply-card-counts.mjs` extended in lockstep (justified by WP §Files Expected to Change "all 40 sets gain physicalCards[]" — the four outliers are not built by convert-cards-v15.mjs and would have failed `HeroSchema.safeParse` if not extended). 01.6 post-mortem authored at `docs/ai/post-mortems/01.6-WP-138-physical-card-abstraction-layer.md` per WP §Definition of Done item 8 (mandatory; four triggers fired). Lint gate (`docs/ai/REFERENCE/00.3-prompt-lint-checklist.md`) invoked and passed at session start; documented in post-mortem §Lint-Gate Pass. Closes the `bkwd / falcon-winter-soldier` portion of the v16 cardSlug image migration's 44-unmatched-files surface (the remaining ~24 split-side heroes unblock when Phase 1b lands their patches; Phase 3 R2 rename pass closes the loop). The Wolfsbane Night Vision / Wolf Out diagnostic that motivated the v16 cardSlug rename is **not yet end-to-end resolved** — Phase 1a delivers the data primitive; Phase 2 migrates engine + viewer; Phase 3 renames images on R2. Vision: §3 (determinism strengthened — deck size for split heroes is now structurally correct via physicalCards primitive, even though Phase 1a engine code still reads cardCounts), §22 (replay faithfulness preserved — per-side ext_id grammar from D-13502 unchanged); NG-1..7 not crossed. Original drafted body retained below for reference; this completion line records the executed state.

- [x] WP-144 — Arena-Client Production Bundle Isolation. **Done 2026-05-09 (`EC-144:` Commit A `bb0493c`; SPEC governance close in companion commit).** Splits `@legendary-arena/game-engine` into Runtime-Safe Engine Surface (`.`) + Setup-Tooling Surface (`./setup`) via `package.json` `exports` map. New barrel `packages/game-engine/src/setup-tooling/index.ts` re-exports `loadScoringConfigForScenario` / `loadAllScoringConfigs` (from scoring/scoringConfigLoader.ts) plus the full par.storage runtime export set (write/read{Simulation,Seed}ParArtifact + buildParIndex / lookupParFromIndex / loadParIndex / resolveParForScenario / validateParStore / validateParStoreCoverage / ParStoreReadError / PAR_ARTIFACT_SOURCES + scenarioKeyToFilename / scenarioKeyToShard / sourceClassRoot / computeArtifactHash). Pure-type re-exports of par.storage stay in the runtime barrel and types.ts as compile-time-only. Eliminates two pre-WP tree-shaking workarounds in the same commit: namespace-import shape in scoringConfigLoader.ts replaced with standard named imports; `stubParStoragePlugin` in apps/arena-client/vite.config.ts replaced with a `build.rollupOptions.onwarn` hard-fail handler. Three independent structural enforcement layers for Boundary Leakage: subpath exports + `"sideEffects": false` (binary audit confirmed all six categories absent across 97 modules reachable from src/index.ts), Vite `onwarn` hard-fail, and arena-client tsconfig path guard mapping `@legendary-arena/game-engine/setup` to `./src/__do-not-import-setup__`. Migrates `apps/server/src/par/parGate.{mjs,test.ts}` value imports of `loadParIndex` / `lookupParFromIndex` / `ParStoreReadError` to `@legendary-arena/game-engine/setup`; type-only `ScenarioScoringConfig` import stays at root. **D-14401 lands** (six clauses: doctrine — structural over heuristic enforcement; naming lock — Runtime-Safe / Setup-Tooling Surface; runtime purity invariant — zero `node:*` reachable from runtime barrel; closed-list quarantine future-proof rule with grandfathering of existing Node-IO source files at `scoring/scoringConfigLoader.ts` + `simulation/par.storage.ts`; tree-shaking prohibition + Boundary Leakage failure-class label; side-effects audit pass with module enumeration). Contract A (fresh-tree CF-shaped build via topology filter) verified: `pnpm install --frozen-lockfile && pnpm --filter "@legendary-arena/arena-client..." build` exits 0 with byte-identical `apps/arena-client/dist/` across consecutive runs. Contract B (build-log mechanical gate) clean: zero matches for `__vite-browser-external|externalized.*node:` in build log. Engine `698 / 150 / 0` UNCHANGED; registry `39 / 4 / 0` UNCHANGED; server `250 / 184 pass / 0 fail / 66 skipped` UNCHANGED; arena-client `286 / 35 / 0 / 0` UNCHANGED. `pnpm-lock.yaml` clean. arena-client SOURCE files unchanged (only `vite.config.ts` + `tsconfig.json` change); WP-090-locked import line at `bgioClient.ts:16` byte-identical. Registry / preplan / viewer / wiki-viewer / physical-card chain UNTOUCHED. **Mid-execution amendments documented in 01.6 post-mortem:** (1) the EC-144 checklist file did not exist at session start (the session prompt referenced it as `authoritative execution contract` but no governance artifact existed); user agreed to author EC-144 inline as part of Commit A so the EC-### commit-msg hook gate passes. (2) Vite `onwarn` regression probe with the literal session-prompt §13 step 9 shape (`import { readFile } from 'node:fs/promises'`) was tree-shaken silently with `"sideEffects": false`; the binding-used variant produces non-zero exit as expected. (3) TS path-guard regression probe under vue-tsc 2.x with `moduleResolution: Bundler` did not catch the deliberate `/setup` import — vue-tsc resolves the subpath via the engine package's `exports` field, bypassing the `paths` mapping; the path guard is documented best-effort and the verify-time grep gate `grep -rn "@legendary-arena/game-engine/setup" apps/arena-client/` is the working backstop. 01.5 NOT INVOKED (no `LegendaryGameState` shape change; no move added; no phase hook; no replay-hash cascade). 01.6 post-mortem authored at `docs/ai/post-mortems/01.6-WP-144-arena-client-production-bundle-isolation.md` per three triggers (new long-lived architectural contract D-14401; first use of subpath exports in `packages/*`; touches a registered package's `exports` surface). Marketing-repo coordination receipts (WP-007a CF Pages build command amendment) NOT WP-144 acceptance; land in a separate commit window in `legendary-arena/legendary-arena-website`. Vision: §3 (build determinism strengthened — arena-client production bundle is now reproducible byte-identically), §22 (build determinism distinct from gameplay determinism); NG-1..7 not crossed. See [WP-144-arena-client-production-bundle-isolation.md](WP-144-arena-client-production-bundle-isolation.md) + [EC-144-arena-client-production-bundle-isolation.checklist.md](../execution-checklists/EC-144-arena-client-production-bundle-isolation.checklist.md). Lint gate (`docs/ai/REFERENCE/00.3-prompt-lint-checklist.md`) invoked and passed at session start; documented in post-mortem §Lint-Gate Pass. Commit prefix `EC-144:` on Commit A; `SPEC:` on Commit B.

- [x] WP-146 — `cards.legendary-arena.com` Cutover Prep — Server CORS. **Done 2026-05-10 (`EC-149:` Commit A `5999d10`; SPEC governance close in companion commit).** Direct sibling of EC-147 / WP-007a (the `play.*` equivalent). Single-file edit: insert one entry `https://cards.legendary-arena.com` into the `Server({ origins: [...] })` literal array at `apps/server/src/server.mjs` adjacent to the existing `https://cards.barefootbetters.com` entry; expand the preceding `// why:` block to enumerate the new entry's purpose (registry-viewer SPA at the new hostname) and the dual-running rationale for the legacy entry. Pre-existing four entries (`https://play.legendary-arena.com`, `https://legendary-arena-play.pages.dev`, `https://cards.barefootbetters.com`, `http://localhost:5173`) retain relative order; final array length: 5. Legacy `cards.barefootbetters.com` entry **preserved byte-identical** for the dual-running window during the operator-driven Cloudflare Pages dashboard custom-domain swap; removal owned by a separate post-cutover cleanup SPEC commit (mirrors WP-007a's dual-listing of `play.legendary-arena.com` + `legendary-arena-play.pages.dev`). **D-14601 lands at execution** (dual-running retention rationale; mirrors WP-007a's dual-listing precedent + locks the no-unit-test-coverage rationale per EC-149 §Guardrails — boardgame.io `Server` cors-package match is upstream-tested, Step 7 post-deploy curl is the canonical verification surface). **No `domains.json` flip** — `cards` row stays at `state: "planned"`; planned → live flip is a separate SPEC commit at cutover lock time (mirrors WP-007a's `2276224`). **No `apps/registry-viewer/CLAUDE.md` / `.env.example` / `DOMAINS.md` prose update** — owned by lock-time SPEC commits. **No `render.yaml` / Render dashboard change** — Render auto-redeploys from `main`. Engine `698 / 150 / 0` UNCHANGED at execution; server `250 / 184 / 66 / 0` UNCHANGED. Hard-deps: WP-007a / EC-147 (the four-entry origins array baseline must be in place — verified via the four-line grep at session §5 baseline re-derivation). Soft-deps: none. 01.5 NOT INVOKED — engine surface zero, no `LegendaryGameState` shape change, no move added, no phase hook, no replay-hash cascade. 01.6 post-mortem SKIPPED — mechanical CORS allowlist addition with no design tension surfaced; the no-unit-test rationale documented in EC-149 §Guardrails (per copilot check FIX) is the only notable governance lock and is captured in D-14601's scope clause. **Cutover sequencing gate (operator, post-push):** Cloudflare Pages dashboard custom-domain swap (detach `cards.barefootbetters.com`, attach `cards.legendary-arena.com`) proceeds only after Render confirms successful redeploy with Commit A's hash and `curl -I -H "Origin: https://cards.legendary-arena.com" https://api.legendary-arena.com/games/legendary-arena` returns HTTP/200 with `Access-Control-Allow-Origin` set. Vision: §3 (build determinism — single-file allowlist literal preserved per code-style Rule 7); NG-1..7 not crossed. See [WP-146-cards-domain-cutover-cors-prep.md](WP-146-cards-domain-cutover-cors-prep.md) + [EC-149-cards-domain-cutover-cors-prep.checklist.md](../execution-checklists/EC-149-cards-domain-cutover-cors-prep.checklist.md). Lint gate (`docs/ai/REFERENCE/00.3-prompt-lint-checklist.md`) implicitly satisfied via the WP draft + preflight + copilot check chain. Commit prefix `EC-149:` on Commit A; `SPEC:` on Commit B.

- [x] WP-145 — Architecture Inventory ↔ Engineering Wiki Integration. **Done 2026-05-10 (`EC-145:` Commit A `b73f18f`; SPEC governance close in companion commit). Locked options: A3 + B1 + C1 + D1.** Surfaces the deterministic monorepo architecture inventory at `wiki/architecture-inventory.md` (committed under B1; sole writer is `scripts/architecture-inventory.mjs`) on the engineering wiki at `ewiki.legendary-arena.com/architecture-inventory/`. New CI workflow `.github/workflows/architecture-inventory.yml` regenerates on cron `0 6 * * 1` (Mondays 06:00 UTC) and opens a PR via `peter-evans/create-pull-request@v6` on output diff (D1: PR-on-diff). Inventory step uses `continue-on-error: true` so a script crash leaves the step visibly red in the GitHub UI but does not cascade into the wiki deploy (visible-failure invariant per EC-145 §Guardrails; `|| true` and exit-code-swallowing shell tricks forbidden). **Three DECISIONS land at execution: D-14501** (cadence + diff policy: A3 weekly cron, D1 PR-on-diff; A2 BLOCKED because the script's `TODAY_UTC` header makes per-build invocation non-deterministic across midnight UTC and date-input hardening is forbidden by the script-immutability invariant), **D-14502 amends D-13810** (single-file generator-authored exception for `wiki/architecture-inventory.md` under the wiki source-readonly contract; named, bounded, single-writer; B2/B3 alternatives rejected for git-log visibility / schema cost reasons), **D-14503 amends `wiki/SCHEMA.md`** (reserved-filename row + `§ File Layout` update + `§ Lint Targets` front-matter / required-section conformance exception). **Script-immutability invariant preserved:** `scripts/architecture-inventory.mjs` SHA-256 byte-identical pre/post (`A8ED10CE...`). **Engine `698 / 150 / 0` UNCHANGED; registry `39 / 4 / 0` UNCHANGED; server `250 / 184 pass / 0 fail / 66 skipped` UNCHANGED.** Scope-clean: `git diff --stat scripts/ packages/ apps/server/ apps/registry-viewer/ apps/arena-client/ apps/replay-producer/ data/` empty post-execution. **Mid-execution scope expansion** documented in 01.6 post-mortem and codified under D-14503: the EC enumerated SCHEMA.md as the only file gaining a C1 amendment, but the existing wiki-viewer build pipeline (`apps/wiki-viewer/scripts/check-links.mjs` link-integrity gate + `apps/wiki-viewer/layouts/_default/_markup/render-link.html` render hook) actively rejected the inventory page's repo-rooted link shape (`docs/...`, `packages/...`). Commit A added a minimal exemption to both files (anchored to D-14503 in `// why:` comments) granting the inventory page the same out-of-tree treatment `../`-prefixed links already receive elsewhere; without those changes the wiki-viewer determinism check would have failed. WP §Open Decisions C explicitly contemplated "the viewer's templates handle as a special case"; D-14503 formalizes that contemplation. **Pre-session governance bundle gap:** WP-145 + EC-145 + preflight + session-prompt artifacts existed on sibling branch `claude/dazzling-pare-5dce5c` (5 SPEC commits stacked on `8a0621a`) but were not merged to `main` at execution-worktree spawn time; user authorized cherry-pick of the 5 SPEC commits onto the execution branch in chronological order to satisfy the EC-### commit-msg hook gate. Two consecutive `pnpm wiki-viewer:build` runs are byte-identical (D-13808 determinism contract preserved post-WP-145; aggregate hash `6EE0B920...`). 01.5 NOT INVOKED — engine-zero WP, no `LegendaryGameState` shape change, no move added, no phase hook. **01.6 post-mortem MANDATORY** (three triggers fired: first generated artifact landing under `wiki/`; first reserved-file accommodation amending `wiki/SCHEMA.md` since v1; D-13810 amended for the first time) — authored at `docs/ai/post-mortems/01.6-WP-145-inventory-wiki-integration.md`. Verification steps 7-8 (workflow_dispatch + URL reachability) deferred to post-push manual smoke test per EC-145 §After Completing. Vision: §15 (Built for Contributors — primary alignment; engineering inventory now reachable via rendered page instead of requiring a clone + Node + script invocation), §14 (Explicit Decisions, No Silent Drift — D-14501..D-14503 lock cadence/location/schema accommodation; future changes require explicit superseding entries), §7 (Strict Layer Separation — no runtime imports introduced; integration is build-time + CI-time only); NG-1..7 not crossed. See [WP-145-inventory-wiki-integration.md](WP-145-inventory-wiki-integration.md) + [EC-145-inventory-wiki-integration.checklist.md](../execution-checklists/EC-145-inventory-wiki-integration.checklist.md) + preflight + session prompt at `docs/ai/invocations/preflight-wp145-inventory-wiki-integration.md` and `docs/ai/invocations/session-wp145-inventory-wiki-integration.md`. Lint gate (`docs/ai/REFERENCE/00.3-prompt-lint-checklist.md`) implicitly satisfied via the WP draft + preflight + copilot check chain (WP §Lint Gate Status section confirms structural lint pass; §5 + §12 resolved at session start under the Recommended Execution Profile). Commit prefix `EC-145:` on Commit A; `SPEC:` on Commit B.

- [x] WP-140 — Physical Card Phase 1b — Per-Set Patch Curation Across Remaining Split-Side Heroes. **Done 2026-05-09 (`EC-143:` Commit A `d51a7ac`; SPEC governance close in companion commit).** Registry baseline `39 / 4 / 0` UNCHANGED; Engine baseline `698 / 150 / 0` UNCHANGED — patches + tooling + data only, no engine / viewer / registry-source / schema modification. **D-13901 landed** (`_skipPair` annotation grammar + matching contract: unordered 2-set, length lock, no-duplicate, existing-slug, mutual-exclusion-with-physicalCards; cluster coverage rule; idempotency invariant; deterministic per-hero 9-step execution order; log emission format with deterministic pair ordering). Worklist freeze captured at `docs/ai/session-context/wp-140-worklist.txt` (262 entries; byte-identity verified before any patch authoring). All 262 audit-warning candidates resolved: 30 split-pair `physicalCards[]` declarations across 4 NEW sets (`cvwr` 17 splits, `mgtg` 8 splits, `msis` 3 splits, `xmen` 5 splits — totaling 30 split pairs across 26 new split heroes; bkwd's 3 split pairs already curated under WP-138 Phase 1a) plus per-cluster-member 1-side `physicalCards[]` declarations for the size-3+ clusters where `_skipPair` cannot cover atomically; remaining size-2 false-positive clusters resolved via `_skipPair[]` annotations across the 30+ patch files. **True split-side heroes identified via the npm-source `divided: 1` / `divided: 2` structural field** — pairs are consecutive cards within a hero's `cards[]` array; this is upstream npm data per WP-140 §E (a permitted source) and structural metadata, not a forbidden inference signal per §7.9. The `subtitle:` field also marks split sides but is not always present on both faces (only on the "secondary" character side); `divided:` is the authoritative pairing signal. Convert pipeline now exits 0 under `--strict` (CI green-state restored; inverse of WP-138 Phase 1a's expected `--strict` exit-1 posture). All 40 `data/cards/*.json` regenerated; 6 changed (antm, cvwr, mgtg, msis, wwhk, xmen) — sets with declared `physicalCards[]` entries; the other 34 remain byte-identical to HEAD because `_skipPair`-only patches don't modify `physicalCards[]` synthesis (D-13901 idempotency invariant; verified by SHA-256 spot-check on `3dtc/howard-the-duck`). **Mid-execution amendment:** `applyPatch` was leaking patch-only fields (any underscore-prefixed key) into `data/cards/*.json` output; added an underscore-prefix exclusion to `applyPatch`'s field-copy loop so `_skipPair` (and any future patch annotation grammar) is consumed by `buildPhysicalCards` but never written to output JSON. Convert-script extension introduces five new helpers above `buildPhysicalCards`: `validateSkipPair`, `identifyClusters`, `isClusterCovered`, `synthesizePhysicalCards` (with auto-fill for partially-declared `physicalCards`), and `validateDriftAgainstCardCounts`. Auto-fill preserves D-13803 uniform model — every hero card gets at least one `physicalCards` entry, declared or auto-generated. **Scope-clean:** `git diff --stat packages/game-engine/ apps/registry-viewer/ apps/arena-client/ apps/server/ packages/registry/` empty post-execution. **`HeroCardSchema.imageUrl` PRESERVED** (Phase 3 follow-up WP-142 removes it). 01.5 NOT INVOKED — tooling + data WP, no `LegendaryGameState` shape change. 01.6 post-mortem authored at `docs/ai/post-mortems/01.6-WP-140-physical-card-phase-1b-patch-curation.md` per WP §Definition of Done item 9 (mandatory; three triggers fired: high-touch curation across 32+ patch files; first use of `_skipPair` annotation locks the false-positive escape-hatch grammar; Phase 1b completion unblocks Phase 2 / WP-141). Lint gate (`docs/ai/REFERENCE/00.3-prompt-lint-checklist.md`) invoked and passed at session start; documented in post-mortem §Lint-Gate Pass. Closes the audit-warning surface for WP-138 Phase 1a; Phase 2 (WP-141) is now unblocked. Vision: §1, §2, §10 (card content semantics — split-hero deck composition structurally correct for every curated hero), §3 (determinism strengthened — `_skipPair` filtering is set-membership match against literal slug strings, no comparator / locale / ordering dependency); NG-1..7 not crossed. Original drafted body retained as archive line below.

- [ ] (original draft body retained for archival reference; superseded by completion line above) WP-140 (drafted) — Physical Card Phase 1b — Per-Set Patch Curation Across Remaining Split-Side Heroes. **Drafted 2026-05-08; not yet executed; lint gate not yet invoked.** Per-set patch curation across the remaining ~24 split-side heroes (`bkwd` other splits not in canonical reference patch, `mgtg`, `msis`, `msmc`, `msp1`, `wpnx`, `wwhk`, `xmen`). Consumes the 262-entry audit-warning worklist surfaced by WP-138 Phase 1a's convert run (every paired-equal `cardCounts` pattern lacking explicit `physicalCards[]` declaration is named with set/hero/value/cards/patch-file). Phase 1b WP must distinguish **true split pairs** (get explicit `physicalCards[]` block per WP-138 v17 patch format) from **false positives** (independent cards whose count matches by coincidence — get a declarative `_skipPair` annotation in the patch suppressing the warning). New decision **D-13901** (`_skipPair` annotation grammar; locked at execution). Hard-deps: **WP-138 (Phase 1a — done)**; soft-deps: nothing else. Engine source UNTOUCHED; registry source UNTOUCHED; only patch files (`scripts/convert-cards/inputs/patches/*.patch.json`) and `data/cards/*.json` (regenerated) are modified. Acceptance: `node scripts/convert-cards/convert-cards-v15.mjs --strict` exits 0 (CI green-state restored); every audit-warning candidate from the 262-entry worklist is either resolved (declared `physicalCards[]`) or suppressed (`_skipPair` annotation). Engine + viewer baselines UNCHANGED. 01.5 NOT INVOKED — registry-data-only change. 01.6 post-mortem MANDATORY (high-touch curation across ~24 patches; first use of `_skipPair` annotation). Vision: §3 (determinism strengthened — split-hero deck composition becomes structurally correct for every hero, not just bkwd's reference patch); NG-1..7 not crossed. Phase 2 (WP-141) cannot start until WP-140 lands every per-set patch — engine cannot trust `physicalCards[]` for split heroes until every split hero is curated. See [WP-140-physical-card-phase-1b-patch-curation.md](WP-140-physical-card-phase-1b-patch-curation.md). Commit prefix `EC-NNN:` at execution (next free EC slot at execution time; not pre-claimed at draft).

- [ ] WP-141 — Physical Card Phase 2 — Engine + Viewer Consumer Migration. **Drafted 2026-05-08; not yet executed; lint gate not yet invoked.** Engine consumer migration (`buildCardDisplayData.ts` switches `card.imageUrl` reads to `getPhysicalCardForSide(...).imageUrl`; `buildHeroDeck.ts` computes deck reservoir from `physicalCards[].count` instead of summed-side `cardCounts`; the WP-137 fan-out helper `resolveHeroCardCopyCount` shifts source from cardCounts to physicalCards.count) + viewer consumer migration in `apps/registry-viewer/src/registry/shared.ts` (9 production read-sites across henchman / mastermind / scheme / villain / sidekick projection branches), `CardDetail.vue`, `CardGrid.vue`. Three new decisions land at execution: **D-13807** (`G.heroDeck` split-hero representation — single-side ext_id stand-in per D-13804 Option A; per-side ext_id grammar D-13502 unchanged), **D-13808** (deck-size arithmetic source switches to `physicalCards[].count`; legacy summed-side `cardCounts` math removed from the three fan-out sites — `buildHeroDeck.ts`, `buildCardStats` in `economy.logic.ts`, `buildCardDisplayData.ts`), **D-13810** (viewer imageUrl lookup pattern — `getPhysicalCardForSide()` at registry-projection time in `shared.ts`; per-side `card.imageUrl` reads removed). Acceptance gate per WP-138 §Migration Phasing: `grep -rn 'card\.imageUrl' packages/game-engine/src/ apps/registry-viewer/src/` returns zero matches outside test fixtures. Hard-deps: **WP-138 (Phase 1a — done)**, **WP-140 (Phase 1b — must land first per WP-138 §Migration Phasing — engine cannot trust `physicalCards[]` for split heroes until every split hero is curated and `--strict` mode exits 0)**. **01.5 IS INVOKED** at execution per `docs/ai/REFERENCE/01.5-runtime-wiring-allowance.md` — deck-reservoir size for split heroes changes (Falcon/Winter Soldier 27 → 14); `computeStateHash` inputs shift via `G.heroDeck` array length and content; replay-hash regeneration cascade fires (capture pre-edit `PRE_WP080_HASH`, run engine test post-edit, update literal IFF hash diverges; record pre/post pair in WP §Verification Steps Step 7 per WP-135 / WP-137 cascade template). 01.6 post-mortem MANDATORY (open D-13804 design question resolved at execution; replay-hash cascade fires; consumer-migration boundary pattern locked). Engine baseline projected `698 / 150 / 0` → `~710-718 / ~151-153 / 0` (+12-20 tests / +1-3 suites). Registry baseline UNCHANGED. Vision: §3 (determinism strengthened — split-hero deck composition correct end-to-end); §22 (replay faithfulness preserved across the Phase 2 engine change because per-side ext_id grammar D-13502 is unchanged; per-copy `#<copyIndex>` suffix from D-13702 layers on top); NG-1..7 not crossed. Out of scope: `HeroCardSchema.imageUrl` schema removal (Phase 3 / WP-142); R2 image rename (Phase 3 / WP-142); `physicalInstanceId` ext_id channel (separate later WP per D-13804 deferred path); replay-record `{ cardId, sidePlayed }` schema extensions (separate WP). See [WP-141-physical-card-phase-2-consumer-migration.md](WP-141-physical-card-phase-2-consumer-migration.md). Commit prefix `EC-NNN:` at execution (next free EC slot; not pre-claimed at draft).

- [ ] WP-142 — Physical Card Phase 3 — `HeroCardSchema.imageUrl` Removal + R2 Image Rename. **Deferred placeholder; not yet drafted.** Hard-deps WP-141 (Phase 2)'s grep gate clearing `card.imageUrl` reads from `packages/game-engine/src/` and `apps/registry-viewer/src/`. Removes `imageUrl` field from `HeroCardSchema` in `packages/registry/src/schema.ts` (Zod schema enforces zero `cards[].imageUrl` keys); convert-script ceases emitting `imageUrl` on `cards[]` entries; final regeneration of all 40 `data/cards/*.json` without per-side imageUrl. R2 image migration: rename split-pair files to combined-name pattern (`{abbr}-hr-{hero}-{sortedA}-{sortedB}.webp`); delete legacy per-side hero card image files for split heroes after the new files are verified. Closes the 44-unmatched-files surface from the 2026-05-07 v16 cardSlug rename and the Wolfsbane Night Vision / Wolf Out diagnostic that motivated WP-138 end-to-end. Drafted after WP-141 lands; numbers not pre-claimed beyond the WP-142 slot reservation here.

- [ ] (original draft body retained for archival reference; superseded by completion line above) WP-138 (drafted) — Physical Card Abstraction Layer (Split-Side Hero Cards). **Drafted 2026-05-07 (rescoped to Phase 1a per pre-flight PS-3); EC slot retargeted EC-140 → EC-141 (EC-140 claimed by WP-134); not yet executed; lint gate not yet invoked.** Introduces a new first-class registry abstraction `PhysicalCard` to fix the data-model conflation between *physical card* (the artifact in a deck) and *card side* (the per-face gameplay entry). The current model treats every gameplay-distinct face as a top-level `hero.cards[]` entry — correct for solo heroes, broken for split-side heroes (Falcon / Winter Soldier in `bkwd` with paired sides Attune / Atone, Relocate / Reload, New Wings / New Plan; the entire MS-GotG roster in `mgtg`; ~25 heroes total across `bkwd`, `mgtg`, `msis`, `msmc`, `msp1`, `wpnx`, `wwhk`, `xmen`). Surfaced 2026-05-07 by the v16 cardSlug image migration (`scripts/convert-cards/migrate-renamed-to-v16.mjs`) leaving 44 unmatched files, the bulk of which are split-side hero images for which no v16 single-slug filename was correct: duplicating one image under both side slugs silently inflates apparent deck size; combining names without schema support leaves the JSON's per-side `imageUrl` pointing at non-existent files. Industry baseline: Scryfall's card data API models split / transform / dual-faced layouts as **one card object with a `card_faces[]` sub-collection**; generic deck-engine designs treat a deck as a list of card objects (not card fragments); Tabletop Simulator double-faced cards are one object with multiple states. Adopts Scryfall-aligned **Option 2** (parallel collections — `hero.cards[]` retains per-side gameplay data; new `hero.physicalCards[]` is the deck-composition primitive) over the lighter `pairsWith` pointer (Option 1, leaves the model lying about deck composition) and the heavier `cards[].sides[]` consolidation (Option 3, invasive refactor of every consumer). Six DECISIONS land at execution: **D-13801** (`physicalCards[]` is the authoritative deck-composition surface; `cardCounts` becomes a derived view validated against physicalCards counts), **D-13802** (`physicalCard.imageUrl` is canonical; card-side `imageUrl` removed; split-pair filename = `{abbr}-hr-{hero}-{sortedA}-{sortedB}.webp`), **D-13803** (solo heroes use single-side `physicalCards` entries — uniform model, no special cases), **D-13804** (D-13502 hero card-instance ext_id format `<setAbbr>/<heroSlug>/<cardSlug>` is unchanged; physical-card identity is a registry concept, not an ext_id concept), **D-13805** (split-pair declarations are sourced exclusively from patches at `scripts/convert-cards/inputs/patches/*.patch.json`; auto-detection from `cardCounts` paired-equal counts is forbidden, surfaces as warnings only), **D-13806** (runtime `sideToPhysicalCard: ReadonlyMap<sideSlug, physicalCardId>` index is computed at registry load, never persisted, exposed via `CardRegistry.getPhysicalCardForSide(heroSlug, sideSlug): PhysicalCard | undefined`). Schema additions in `packages/registry/src/schema.ts`: `PhysicalCardSchema` with `id`, `count`, `imageUrl`, `sides: [string] | [string, string]`; `HeroSchema.physicalCards` required and non-empty when `cards[]` is non-empty; `HeroCardSchema.imageUrl` removed (transition window acceptable). Convert script extension in `scripts/convert-cards/convert-cards-v15.mjs`: `heroImageUrl(setAbbr, heroSlug, sides)` accepts the sides array; emits `{abbr}-hr-{hero}-{sides[0]}.webp` (one side) or `{abbr}-hr-{hero}-{sortedA}-{sortedB}.webp` (two sides, lex-sorted); drops `imageUrl` emission from individual `cards[]` entries; validates cardCounts against physicalCards counts and fails on drift. Patch format extension: per-hero `physicalCards: [{ id, count, sides }]` block (v17 patch format); curate explicit declarations for the ~25 split-side heroes identified in the renamed-v16 unmatched audit. Loader updates in `packages/registry/src/impl/{localRegistry,httpRegistry}.ts`: validate `physicalCards` per `PhysicalCardSchema`; build the runtime side-to-physicalCard index; expose via `getPhysicalCardForSide`. Engine consumer migration: replace `card.imageUrl` reads with `getPhysicalCardForSide(...)` lookup at every site (notably `buildCardDisplayData.ts` per WP-111 / WP-137 and `buildHeroDeck.ts` per WP-135 — deck reservoir computed from `physicalCards[].count`, not summed-side `cardCounts`); per-side ext_ids unchanged (D-13804). UI consumer migration in `apps/registry-viewer/`: card-rendering components fetch image via the runtime index; viewer continues to display per-side cards (each side keeps gameplay info, shared image loads once). Image migration on R2: rerun (or extend) `migrate-renamed-to-v16.mjs` to produce combined-name files for split-pair physicalCards; the 44 unmatched files from the v16 migration become matched once pair declarations land. Hard-deps: WP-003 (registry verification + immutable files lock; this WP justifies the schema.ts modification via D-13801..D-13806), WP-082 / WP-083 (registry schema + fetch-time validation), WP-135 (`buildHeroDeck` reservoir + `RARITY_COPY_COUNT` map + ext_id format under D-13501 / D-13502 — D-13502 explicitly preserved by D-13804), WP-137 (`cardCounts` authoritative for copy counts when populated — becomes a derived view under D-13801). 01.5 likely INVOKED at execution (deck-reservoir size for split heroes changes — `computeStateHash` inputs via `G.cardStats` / `G.cardDisplayData` may shift; cascade procedure per WP-135 / WP-137 template). 01.6 post-mortem MANDATORY (new long-lived registry abstraction; first introduction of physical-card vs card-side distinction; ~25 patch-file curations across split-side heroes — high-touch surface). Vision: §3 (determinism strengthened — deck size now correct for split heroes), §22 (replay faithfulness preserved — per-side ext_id grammar unchanged); NG-1..7 not crossed. Out of scope: restructuring `cards[]` to embed `sides[]` (Option 3; deferred); replay-record schema extensions for `{ cardId, sidePlayed }` (separate WP); anti-cheat / integrity work on physical-card identity (Phase 9 surface per WP-107 placeholder); image asset content audits (renames files; does not validate artwork-vs-data correspondence). Commit prefix `EC-141:` at execution (next free EC slot after EC-140 / WP-134; EC-140 was claimed by WP-134 between WP-138 draft and execution-readiness, so WP-138 retargets per the locked precedent). See [WP-138-physical-card-abstraction-layer.md](WP-138-physical-card-abstraction-layer.md) + [EC-141-physical-card-abstraction-layer.checklist.md](../execution-checklists/EC-141-physical-card-abstraction-layer.checklist.md). **Executive review (2026-05-07) APPROVE WITH MINOR HARDENING applied:** D-13802 amended to lock the sort comparator to ASCII / UTF-16 code-unit ordering (`Array.prototype.sort()` with no comparator; never `localeCompare` without explicit `'en'` locale); D-13804 amended to declare explicitly that per-side ext_id alone does not uniquely identify a physical card instance (replay / audit / future instanceId tracking is a separate WP); `PhysicalCardSchema.sides` typed `readonly string[]` with validator-enforced `1 <= length <= 2` (extensibility for future triple-face cards without a TS-type migration); convert-cards-v15.mjs gains a `--strict` flag (CI invocations fail on missing-pair warnings; local dev iteration may omit); migration sequenced into three locked phases (Phase 1 additive — physicalCards landed alongside cards[].imageUrl; Phase 2 consumer migration — engine + viewer switch to runtime index, grep-gate clears `card\.imageUrl` in src; Phase 3 removal — schema enforces; R2 cleanup pass). Lint gate (`docs/ai/REFERENCE/00.3-prompt-lint-checklist.md`) MUST be invoked and pass at execution-session start; this packet's draft does not constitute a lint-gate pass.


- [ ] **(deferred placeholder)** G-state extensions for board layout — resolves WP-128 Option A safe-skips (D-12806). WP-128 ships eight projections with safe-skip values (`[]` or `0`) under `// SAFE-SKIP-WP128` markers because the underlying `G` sources don't exist today: `G.mastermind.attachedBystanders` (Master Strike captures — distinct from city-villain `G.attachedBystanders` per D-DEC-5 Interpretation B); `G.mastermind.strikePile` (resolved Master Strike cards — currently filterable from `villainDeck.discard` but not preserved as a first-class pile); `G.scheme.twistPile` (resolved Scheme Twist cards — same shape as strike pile; today `G.scheme` doesn't exist as a top-level field, only the `selection.schemeId` reference); `G.city.escapedPile` (escaped villain cards — today only the counter `G.counters[ESCAPED_VILLAINS]` increments, the cards themselves are dropped); `G.turnEconomy.piercing` + `G.turnEconomy.woundsDrawn` (per-turn counters — require move-logic changes alongside the field add); ~~`G.heroDeck` (hero-deck reservoir — HQ refill mechanism today is static; recruited cards leave HQ to player discard with no replenishment)~~ **— SUPERSEDED by WP-135 (drafted 2026-05-04; closes the `decks.heroDeckCount` safe-skip site in lockstep with HQ population)**; `G.piles.horrors` (Horrors pile for scenarios that use them — currently no scenario uses horrors so D-DEC-2 default of `0` is functionally correct, but the field add is part of this WP for completeness). After WP-135 lands, **7 of the original 8 safe-skip sites remain**. The future WP(s) must (a) add each remaining `G` field via the existing setup pipeline, (b) wire move logic that populates the fields (Master Strike resolution → strike pile; Scheme Twist resolution → twist pile; villain escape → escapedPile; wound draw → woundsDrawn counter; piercing-effect cards → piercing counter), (c) update each safe-skip site in `uiState.build.ts` from constant to derivation (remove the `// SAFE-SKIP-WP128` marker; replace `[]` / `0` with the actual `G` reference + per-entry shallow copy per WP-111 D-11105 aliasing-defense pattern), (d) extend the drift test to pin actual values against fixture matches. **01.5 IS INVOKED** for each sub-WP (new `LegendaryGameState` fields + setup-orchestrator shape change + new move logic across the engine surface). Replay-hash literal cascade likely (each new G field changes `computeStateHash` inputs); EC-131 §2 binary cascade procedure applies. Hard-deps: WP-128 (the safe-skip allowlist; the `// SAFE-SKIP-WP128` markers are the change-point map). Soft-deps: WP-129 will continue rendering the empty piles / zero counters until each sub-WP lands; no client-side change required when each source flips. Scope estimate per remaining cluster: 8-15 files. **Splitting is preferred** — each cluster can land independently and the corresponding `// SAFE-SKIP-WP128` markers in `uiState.build.ts` are removed in lockstep, providing a clean per-cluster acceptance signal. WP-135 is the locked-in template for each subsequent sub-WP. EC-NNN reserved at draft time. No commit prefix until drafted.
...
- [x] WP-139 — Engineering Wiki Viewer (Hugo, Build-Time Projection). **Executed 2026-05-08 (Commit A `5a47da2` `EC-142:` — `apps/wiki-viewer/` source + `.github/workflows/wiki-viewer.yml` + `package.json` + `render.yaml`; Commit B `SPEC:` — governance close + post-mortem).** Engine + registry + server + registry-viewer baselines UNCHANGED at 698/0/0 + 32/0/0 + 250/184/66/0 + 31/0/0. 13 wiki content routes rendered (10 entity + 1 section landing + `SCHEMA.md` + `README.md`) + 1 homepage redirect. Determinism check passes byte-identical across two consecutive builds (Chroma syntax highlighting disabled in `hugo.toml` to remove non-deterministic `<pre>` CSS output — finding recorded in post-mortem §5c). Production output JS-free (zero `<script>` tags in `apps/wiki-viewer/public/`). Layer-boundary clean: zero matches in source / template / config files for `@legendary-arena/(game-engine|registry|preplan)|apps/server` (rendered HTML at `/wiki/cardextid/` legitimately mentions `@legendary-arena/registry` in prose copied from the source `cardextid.md`; verification grep refined to scope to source/template/config only — post-mortem §4a). Projection-is-copy invariant holds: `docs/wiki/INDEX.md` SHA-256 unchanged pre/post build (`scripts/project-wiki.mjs` uses `copyFileSync` exclusively + cheap existence-assertion regression guard). Six required `// why:` comments present (anchors at `project-wiki.mjs` + `check-links.mjs` + `render-link.html` + `hugo.toml` + `apps/wiki-viewer/README.md` + `single.html` metadata-panel partial site). D-13808..D-13811 land in numeric order above D-13807 — gate-4 race-window grep at session start confirmed D-13808+ free (no WP-140 collision). 01.5 NOT INVOKED (no `LegendaryGameState` field added; no `buildInitialGameState` shape change; no new move; no new phase hook; engine surface zero). 01.6 post-mortem authored at `docs/ai/post-mortems/01.6-WP-139-engineering-wiki-viewer.md` per three triggers (new long-lived app under the new `docs-app` category alongside `apps/registry-viewer/`; first build-time projection of `docs/wiki/`; first Hugo adoption in the monorepo) — verdict WP COMPLETE. Hugo Extended `0.135.0` pinned in `apps/wiki-viewer/.hugo-version`; hand-rolled minimal theme (Open Decision B locked; no third-party theme dependency); GitHub Actions workflow `.github/workflows/wiki-viewer.yml` runs link integrity + Hugo build + determinism check + JS-free invariant on push to `main` touching `docs/wiki/` or `apps/wiki-viewer/`. Render `static_site` service `legendary-arena-wiki` declared in `render.yaml`; Rendered viewer URL recorded in `docs/wiki/README.md`'s new "Rendered viewer" section (placeholder `https://legendary-arena-wiki.onrender.com/` to be confirmed against the first successful Render deploy). Vision: §7, §14, §15. Commit prefix `EC-142:` on Commit A; `SPEC:` on Commit B (never `WP-139:` per 01.3 P6-36 — commit-msg hook rejects). Introduces `apps/wiki-viewer/` — a Hugo-built static-site projection of `docs/wiki/` (the engineering wiki created by the four `SPEC: wiki ...` commits on `claude/festive-moore-eba215`, pending merge to `main`). The viewer is **build-time only** (no runtime backend, no SSR), a strict **read-only projection** (HTML regenerated from `docs/wiki/` on every build; publish target never hand-edited), and **layer-boundary clean** (no runtime imports from `@legendary-arena/game-engine`, `@legendary-arena/registry`, `@legendary-arena/preplan`, or `apps/server/**`). Renders 13 content routes: 10 entity pages + 1 section landing (from projected `_index.md`) + `SCHEMA.md` + `README.md`. Build pipeline does (a) a content projection step (`scripts/project-wiki.mjs` copies `docs/wiki/*.md` → `apps/wiki-viewer/content/wiki/` and renames *only the copy* `INDEX.md → _index.md` — the source under `docs/wiki/` is never modified), (b) a deterministic pre-build link-integrity check (`scripts/check-links.mjs` operating on the projected markdown tree, case-sensitive) that fails the build on any broken internal link, (c) `hugo` with templates and a markdown render hook (`layouts/_default/_markup/render-link.html`) that resolves internal `.md` refs and rewrites out-of-tree `../` links to GitHub blob URLs. **Determinism is locked** at byte-identical output across two consecutive builds for the controlled file set `apps/wiki-viewer/public/**/*.html` + `*.css`; required Hugo knobs: `enableGitInfo = false`, RSS / sitemap / taxonomy outputs disabled. **Production output is JS-free** at v1 (no `<script>` tags). **Four DECISIONS land at execution** (placeholder block in the WP draft retargets to next-free `D-NNNN` at session start; if WP-140 from `claude/stupefied-bhaskara-f7dcc6` lands first claiming D-13901, WP-139 takes the contiguous block above the latest landed; if WP-139 lands first, it takes D-13808..D-13811): framework selection (Hugo over Docusaurus / custom — MDX forbidden as the practical reason Docusaurus was rejected), external-link strategy (GitHub blob rewrite default), reserved-file handling (build-time content projection — no schema change), hosting target (Render static site default). **Eight Open Decisions** (A project location, B theme, C Hugo version pin, D reserved-file handling, E external-link strategy, F hosting target, G build trigger, H search) — A / D / E / F / G must lock before execution per lint gate §5/§12; B and C tightened to "lock concrete value before session start" per copilot check finding #12. Hard-deps: **wiki source merged to `main` first** (the four `SPEC: wiki ...` commits on `claude/festive-moore-eba215` — verified 13 markdown files via `git ls-tree`; until merge lands this WP is BLOCKED per its own §Assumes); D-13807 (this WP's `docs-app` category classification, landed in the same governance pass as the WP draft). Engine + registry + server + registry-viewer test baselines must remain UNCHANGED. 01.5 NOT INVOKED (no `G`-shape changes). 01.6 post-mortem MANDATORY (new long-lived app; first build-time projection of `docs/wiki/`; first Hugo adoption in the monorepo). Vision: §15 (Built for Contributors — primary alignment), §22 (build determinism, distinct from gameplay determinism); NG-1..7 not crossed (engineering wiki is internal contributor documentation, no monetization or competitive surface). Out of scope: search (deferred via Open Decision H), MDX or any client-side interactivity (forbidden), versioned docs, authentication, comments / annotations. Commit prefix `EC-142:` at execution. See [WP-139-engineering-wiki-viewer.md](WP-139-engineering-wiki-viewer.md) + [EC-142-engineering-wiki-viewer.checklist.md](../execution-checklists/EC-142-engineering-wiki-viewer.checklist.md) + pre-flight + copilot check at `docs/ai/invocations/preflight-wp139-engineering-wiki-viewer.md` and `docs/ai/invocations/copilot-check-wp139.md`. **Pre-flight v4 reissued 2026-05-08 with verdict READY TO EXECUTE** after PS-2 (wiki source merge to `main`) cleared via the four `SPEC: wiki ...` commits + `c7e258a` (D-13807). PS-1 (`apps/wiki-viewer/` classification) closed by D-13807; PS-3 (WORK_INDEX + EC_INDEX rows) closed by this row + EC_INDEX-142 row. PS-5 (Open Decisions A/D/E/F/G defaults) locked at session start with WP-stated defaults. Copilot check binding pass 30/30 PASS verdict CONFIRM honored.

- [x] WP-127 — Registry Viewer: Grid Tile Team & Ability Text (Threshold-Gated) — **Done 2026-05-02 (Commit A `1323266` `EC-129:`).** Drafted 2026-05-02; lint-gate self-review PASS (PS-1 Vision Alignment block resolved); EC-129 drafted (next free EC slot — EC-127 was claimed by WP-125 on 2026-05-01, EC-128 by WP-104 on 2026-05-02; per the locked precedent EC-103 → EC-111, EC-101 → EC-114, EC-109 → EC-115, EC-121 → EC-122, EC-123 → EC-127, EC-104 → EC-128, the WP-keyed EC retargets to EC-129; WP number unchanged). Amends D-9601 in place (no new D-NNN number) to permit two threshold-gated field-set additions on `apps/registry-viewer/src/components/CardDataTile.vue` only when `cardSize.value >= 190px`: a `Team` row inserted between the existing `Class` and `Cost` rows (mirroring sidebar ordering at `CardDataDisplay.vue:90–93`) and an `Ability` block appended beneath the existing `</dl>` (mirroring `CardDataDisplay.vue:130–141` with plain-text bullets — no tokenization). Below threshold the WP-096 baseline is byte-identical (seven labelled rows, no `Team`, no `Ability` block, 3:4 aspect-ratio lock). Above threshold and only inside the `viewMode === 'data'` branch, `CardGrid.vue`'s `.img-wrap` receives a new `data-expanded` class; the new sibling rule `.img-wrap.data-expanded { aspect-ratio: auto; }` lets the tile grow vertically to fit the ability block. Image-mode tiles never receive the class. **Three production files (one new + two modified):** `apps/registry-viewer/src/composables/cardTileThresholds.ts` (new — single-export module: `export const ABILITY_THRESHOLD_PX = 190;` with zero imports, no default export, no additional named exports — preserves D-12101's locked `useCardSize.ts` surface verbatim), `apps/registry-viewer/src/components/CardDataTile.vue` (modified — three new imports, `useCardSize` destructure, `showAbilityRow` `computed`, `hasAbilityText` helper byte-identical to `CardDataDisplay.vue:53–59`, `Team` row template, `Ability` block template, four new scoped CSS rules `.ability-block` / `.ability-block-title` / `.ability-lines` / `.ability-line` mirroring sidebar palette with tile-scaled font sizes (0.55rem block title / 0.6rem ability line), two new `@media print` overrides mirroring `CardDataDisplay.vue:253–259`, JSDoc updated to document the threshold-gated rows; the seven existing rows + their CSS + the existing `@media print` block are byte-identical pre- and post-execution), `apps/registry-viewer/src/components/CardGrid.vue` (modified — one new import, class binding `:class="{ 'data-expanded': viewMode === 'data' && cardSize >= ABILITY_THRESHOLD_PX }"` on `.img-wrap`, one new sibling CSS rule `.img-wrap.data-expanded { aspect-ratio: auto; }`; the existing `.img-wrap` rule, the grid column track, the type-badge rule, the tile-info rule, and all `.tile-*` rules are byte-identical pre- and post-execution). Plus four governance files: `WP-127-*.md`, `EC-129-*.checklist.md`, `WORK_INDEX.md` row, `EC_INDEX.md` row, `DECISIONS.md` D-9601 amendment, `STATUS.md` (this entry). Total staged set across both commits: exactly 9 files (EC-129 §After Completing compliance). **D-12101 lock preserved verbatim:** `apps/registry-viewer/src/composables/useCardSize.ts` is byte-identical pre- and post-WP-127 — the threshold constant lives in a sibling single-export module rather than expanding the locked composable surface ("exactly two names plus the four range constants"). All five mandatory `// why:` comments present (cardTileThresholds.ts module-header JSDoc; CardDataTile.vue useCardSize import; CardDataTile.vue showAbilityRow computed; CardDataTile.vue module-header JSDoc update; CardGrid.vue class binding). **Verification.** `pnpm --filter registry-viewer build` exits 0 (97 modules, 213.70 kB JS / 40.91 kB CSS, gzip 64.84 kB / 7.03 kB; pre-packet baseline 96 modules / 213.03 kB / 40.35 kB — single new module). `pnpm --filter registry-viewer typecheck` (`vue-tsc --noEmit`) exits 0. `pnpm --filter registry-viewer test` reports `tests 31 / suites 6 / pass 31 / fail 0` — UNCHANGED from pre-session baseline (no tests added per the WP-066 / WP-094 / WP-096 / WP-114 / WP-121 / WP-122 / WP-123 / WP-124 / WP-125 viewer-side precedent). All §10 verification gates pass: exactly one `ABILITY_THRESHOLD_PX = 190` definition across `apps/registry-viewer/src`; ABILITY_THRESHOLD_PX referenced 8 times across the tree (1 def + 2 imports + 5 references in code + JSDoc); zero `\b190\b` matches in `CardDataTile.vue` or `CardGrid.vue` (single source of truth via the constant); zero forbidden imports (`boardgame.io`, `@legendary-arena/{game-engine,preplan,server}`, `pg`, `node:`) in any of the three files; `git diff apps/registry-viewer/src/composables/useCardSize.ts`, `git diff apps/registry-viewer/src/composables/useCardViewMode.ts`, `git diff apps/registry-viewer/src/components/CardDataDisplay.vue`, `git diff apps/registry-viewer/src/components/CardDetail.vue`, `git diff apps/registry-viewer/src/components/CardSizeSlider.vue`, `git diff apps/registry-viewer/src/App.vue`, `git diff apps/registry-viewer/package.json`, `git diff packages/registry/`, `git diff packages/game-engine/`, `git diff apps/server/`, `git diff apps/arena-client/`, `git diff data/` all empty. One DECISIONS amendment (no new D-NNN): D-9601 amend-in-place block dated 2026-05-02 documents the threshold-gated relaxation of rules #1 and #4, the conditional aspect-ratio rule, the new threshold module path, the five-locks-below-threshold preservation, and the future field-addition + threshold-tuning amendment paths. 01.5 NOT INVOKED (zero new `LegendaryGameState` field, zero `buildInitialGameState` shape change, zero new `LegendaryGame.moves` entry, zero new phase hook). 01.6 post-mortem OPTIONAL per WP-066 / WP-094 / WP-096 / WP-114 / WP-121 / WP-122 / WP-123 / WP-124 / WP-125 viewer-side precedent — not authored this session (no new contract surface beyond a single-export constant module, no new long-lived abstraction, no new code subdirectory; one in-place D-amendment whose template is reusable but introduces no new abstraction). §17 Vision Alignment: §10a (Registry Viewer public surface — search and browse quality on `cards.barefootbetters.com`) advanced; NG-1..NG-7 not crossed. §20 Funding Surface Gate: N/A — registry-viewer UI affordance; no funding-adjacent UI, no payment surface, no donation prompt, no storefront cross-link. §21 API Catalog: N/A (no `apps/server` files touched, no HTTP surface affected). Manual smoke recommended-not-gated per §12 — threshold crossing 180→190→180 reveals/hides the `Team` row + `Ability` block; image-mode tiles stay 3:4 at all sizes; below-threshold data tiles stay 3:4. Dependencies: WP-096 / D-9601 (locked grid-tile data view this WP amends — hard); WP-121 / D-12101 (locked `useCardSize` composable surface preserved verbatim — hard); WP-066 (image-vs-data toggle — soft). Commit prefix `EC-129:` at execution (never `WP-127:` per `01.3-commit-hygiene-under-ec-mode.md`). See [WP-127-registry-viewer-grid-tile-team-and-ability-text.md](WP-127-registry-viewer-grid-tile-team-and-ability-text.md) + [EC-129-registry-viewer-grid-tile-team-and-ability-text.checklist.md](../execution-checklists/EC-129-registry-viewer-grid-tile-team-and-ability-text.checklist.md).
...
- [x] WP-116 — Disconnect & Reconnect Semantics (Architecture) ✅ Reviewed (2026-04-30 pre-flight: 2 BLOCKING + 3 RECOMMENDED PS items, all 5 resolved in prep commit `cddfa3f`; copilot-check re-run PASS post-resolution; v1.1 refresh at `ea674a8` 2026-05-02 introduced no new PS items) — **Completed 2026-05-02** (governance-only WP, single `SPEC:` commit, no EC per D-10001 + 2026-04-26 Amendment). Locks application-layer disconnect / reconnect policy on top of WP-090's Socket.IO transport before further multiplayer features (team play WP-109, future ranked play, future spectator surface) re-litigate the same questions. (WP-115 originally appeared in this list at v1.0 draft time as a forward-looking multiplayer feature; corrected at WP-116 v1.1 — WP-115 ships read-only HTTP, not a live-match surface.) **Six new DECISIONS entries:** `D-11601` rejoin grace window = phase-aware (Option B; longer in `play`, shorter in `lobby` / `setup`; concrete magnitudes deferred to future implementation WP), `D-11602` turn-handover during `play.main` = pause match (Option B; structural pause definition forbids `ctx.events.*` calls as side effects of disconnect; bounded by D-11604), `D-11603` lobby ready-state on rejoin = cleared on disconnect (Option B; `G.lobby.ready[playerId]` cleared, rejoining player must re-ready), `D-11604` mid-match abandonment threshold = hard timeout (Option A; match forcibly ends, replay emitted with `endReason: 'abandoned'`; full `endReason` closed-set forward-linked to future implementation WP), `D-11605` replay-on-abort = replay always emitted with explicit `endReason` (Option A; one record shape distinguished by discriminator; partial replays must be byte-replayable per Vision §22), `D-11606` spectator behavior on player drop = deferred (Option A default; standalone N/A entry per WP-117 D-11703 interpretation precedent). **Architecture docs gain a `## Disconnect & Reconnect Semantics` section** (`docs/ai/ARCHITECTURE.md` after `## Internationalization`; `docs/02-ARCHITECTURE.md` after `## Transport`) with a 12-cell phase × event matrix and the literal "Disconnect tracking does not mutate `G`" + "Disconnect / reconnect events do not advance RNG state or implicitly execute turn logic" statements. `.claude/rules/architecture.md` gets a one-line cross-link pointer (no policy duplication). §17.1 #4 (multiplayer reconnection — Vision §4) primary trigger; §17.1 #2 (replays — Vision §22, §24) secondary trigger via D-11605 = A. §20 N/A (no funding surface). NG-1..NG-7 not crossed. No code touched. No tests added or removed. `pnpm -r test` baseline unchanged (server `pass 56 / fail 0 / skipped 32` post-WP-115; engine `pass 604 / fail 0`). 6 files modified per the resolved scope-lock. Pre-flight artifact: [`docs/ai/invocations/preflight-wp116.md`](../invocations/preflight-wp116.md) (gitignored scratchpad). Dependencies: WP-090. See [WP-116-disconnect-reconnect-semantics.md](WP-116-disconnect-reconnect-semantics.md).
