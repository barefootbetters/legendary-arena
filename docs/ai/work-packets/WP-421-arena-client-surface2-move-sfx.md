# WP-421 — Arena-Client Surface-2 Player-Action Move SFX (tactile local feedback)
...
- In `setup`, build the player once (`const playMoveSound = useMoveSounds();`,
  ...
  WP-421 files; no other `App.vue` behavior changes.
...
- [ ] `docs/ai/DECISIONS.md` updated — land **D-24241** as Active.
- [ ] `docs/ai/work-packets/WORK_INDEX.md` has WP-421 checked off with the date.
- [ ] `docs/ai/execution-checklists/EC_INDEX.md` — EC-456 row Status `Done`.
- [ ] `docs/05-ROADMAP-MINDMAP.md` — WP-421 node glyph `📝 → ✅`, then
...
- **D-24241 (reserved; land Active at execution)** — The arena-client gains the
