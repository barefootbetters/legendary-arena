# WP-452 — Co-op Win-Rate + Loss-Cause Harness
...
- The harness writes **no committed artifact and adds no CI freshness gate** — it
  prints; each consuming WP records its number in its own STATUS/PR (D-24272).
...
- No T3 `BotAlly` policy — that is a later WP. The harness only selects the existing
...
- No committed win-rate artifact and no CI freshness gate (D-24272).
...
- **§20–26 Scoring / PAR / leaderboards — NOT crossed.** The harness is an
  ...
  artifact (D-24272). It shares the simulation subsystem with the balance-PAR
...
- [ ] `docs/ai/DECISIONS.md` — **D-24272** landed (co-op-strength metric)
- [ ] `docs/ai/work-packets/WORK_INDEX.md` has WP-452 checked off with today's date
...
- **Pre-flight (01.4): READY TO EXECUTE.** Every §Assumes claim verified on `main`
  @ `edf8f7e9`: `runSimulation(config, registry: CardRegistryReader, …)`,
