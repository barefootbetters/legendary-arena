...
**Related Context (not a hard dep):** WP-263 ✅ — the sibling-field sim projection (`hollowEffects` / `hollowEffectsDropped`) this is co-located with. Not blocking; but the default-equivalence `deepStrictEqual` tests compare the whole `CapturedGameResult` / `SweepCellResult`, so those sibling fields ride along (they read `[]` / `0` under the mock registry).

> **Executor lock:** WP-264 is *only* additive parameter threading. Add `maxTurns` as a trailing optional parameter defaulting to `MAX_TURNS_PER_GAME`, and forward it through the existing simulation path. Do not change result shapes, exports, cards, registry boundaries, randomness, replay fixtures, or call-site behavior. Omitting `maxTurns` must be deep-equal to explicitly passing the default. `maxTurns` is a caller-owned positive integer — WP-264 adds **no** runtime validation and **no** throw path (see §Contract).

---

## Goal

After this session, the engine's simulation entry points accept an optional `maxTurns` parameter that caps a game's per-turn loop, threaded through to the loop and the outcome builder, **defaulting to the existing `MAX_TURNS_PER_GAME` (200)** so every current call site stays source-unchanged and its observable result is **deep-equal** to today's. This lets a downstream sweep run **short, terminating** games (a bounded sweep) instead of every game grinding to the 200-turn cap. Today `MAX_TURNS_PER_GAME` is a hardcoded constant with no override, so a competent-play sweep over not-yet-implemented heroes either runs minute-scale games or never terminates (the cards do nothing → no progress → 200-turn spin). This packet is the **engine enabler for WP-265** (the real-signal runtime-observed-hollows cron), which needs fast, bounded, terminating heuristic games to be CI/cron-affordable.
...
- **Baseline:** drafted against `origin/main` @ `0a60968b`; the SPEC has since landed on `main` (PR #382 → `fcf3274f`). The executor must **re-baseline against the current `origin/main`** at execution time (`git fetch origin main --prune && git rev-parse origin/main`) rather than checking out the stale drafting ref.
...
**Additive + behavior-preserving default:** `maxTurns` is an **optional trailing** parameter on each entry point, defaulting to `MAX_TURNS_PER_GAME` (200). Every existing call site omits it and stays **source-unchanged**; the observable result of omitting `maxTurns` is **deep-equal** to explicitly passing `MAX_TURNS_PER_GAME`. The change adds no new move/rule/phase, mutates nothing new in `G`, and draws no new randomness.

**`maxTurns` validity (caller-owned; no throw):** `maxTurns` is a caller-owned **positive integer**. WP-264 adds **no** runtime validation, clamping, or coercion — an out-of-contract value is not rejected, it simply flows into `while (turnsElapsed < maxTurns)` (so `maxTurns <= 0` deterministically yields a zero-turn game, never an error). Do **not** add a `RangeError` / throw path: this file already degrades degenerate inputs (empty seed, no policies, `games < 1`) to zeroed/empty results rather than throwing, and `maxTurns` follows that same posture. The sole consumer (WP-265's cron) passes a known small literal (~30–50).
...
- `packages/game-engine/src/simulation/simulation.runner.ts` — **modified**:
  - `runPerTurnLoop(...)` **already ends in the optional `onMoveDispatched?: (move: ReplayMove) => void`**; append `maxTurns: number = MAX_TURNS_PER_GAME` **after** it (do NOT reorder the existing callback). The `while (turnsElapsed < maxTurns)` head + the stuck-game `turnsElapsed = maxTurns` break use it.
  - `buildGameOutcome(...)` gains a trailing `maxTurns: number = MAX_TURNS_PER_GAME`; the `effectiveTurns` cap (`turnsElapsed >= maxTurns ? maxTurns : turnsElapsed`) uses it.
  - `simulateOneGame(config, registry, gameIndex, nextRandom, maxTurns = MAX_TURNS_PER_GAME)` forwards it to `buildGameOutcome` and to `runPerTurnLoop`. **Because `maxTurns` sits after the optional `onMoveDispatched`,** this non-capturing call passes `undefined` for the callback to reach it: `runPerTurnLoop(gameState, config.policies, numPlayers, gameIndex, nextRandom, undefined, maxTurns)`.
  - `simulateOneGameAndCaptureMoves(setupConfig, registry, policies, seed, gameIndex, maxTurns = MAX_TURNS_PER_GAME)` passes it to the warm-up `simulateOneGame` calls, to `runPerTurnLoop` (its real callback is already the 6th arg, so `maxTurns` is simply appended 7th), and to `buildGameOutcome`.
...
- `packages/game-engine/src/simulation/simulation.captureMoves.test.ts` — **modified**:
  - **Bound respected:** `simulateOneGameAndCaptureMoves(..., maxTurns)` with a small cap (e.g. `5`) returns a `CapturedGameResult` whose captured trace is bounded — assert on **`captured.moves.length`** only. `CapturedGameResult` exposes **no** turn-count field (its set is locked to `{ endgameReached, hollowEffects, hollowEffectsDropped, moves, outcome }`), so do NOT reference a phantom `turnsElapsed`. Assert the small-cap run's `moves.length` ≤ the uncapped run's; choose the existing 2-seat random-policy fixture, whose uncapped run never reaches endgame under the mock registry and so spins to the 200 safety cap — a cap of `5` therefore strictly bounds it and the assertion actually bites.
  - **Default-equivalence:** a call **omitting** `maxTurns` is `deepStrictEqual` to the same call passing the literal `200`. (`MAX_TURNS_PER_GAME` is module-local, **not** exported — use the literal; do NOT export it merely to test.)
  - **Warm-up forwarding:** add at least one default-equivalence assertion at **`gameIndex = 1`** so the warm-up `simulateOneGame` loop actually runs (one prior game) and is proven to forward the same `maxTurns`. A `gameIndex = 0` test collapses the warm-up to a no-op and never exercises the PRNG-parity path the constraints call out.
- `packages/game-engine/src/simulation/sweep.runner.test.ts` — **modified**:
  - **Forwarding:** one deterministic cell run with a small `maxTurns` has **`SweepCellResult.moveCount`** (a real field) ≤ the same cell run uncapped.
  - **Default-equivalence:** the full callback sequence omitting `maxTurns` is `deepStrictEqual` to the sequence passing the literal `200`.

## Out of Scope
- **WP-265 (the real-signal cron + harness `--deep` mode)** — the consumer of this param; it adds no engine code.
- **An options-object refactor of `sweepSetupMatrix` / the sim entry points** — the additive trailing param is the minimal change; the 10-arg signature smell is **intentionally accepted** for WP-264, noted not fixed. Do not refactor to an options object, and do not reorder existing parameters.
...
- `maxTurns: number` — optional trailing parameter on `runPerTurnLoop`, `simulateOneGame`, `buildGameOutcome`, `simulateOneGameAndCaptureMoves`, `runSimulation`, and `sweepSetupMatrix`; **default = `MAX_TURNS_PER_GAME` (200)**. When supplied, the per-turn loop runs at most `maxTurns` turns and `buildGameOutcome` caps `effectiveTurns` at `maxTurns`. On `runPerTurnLoop` it is appended **after** the existing optional `onMoveDispatched?`.
- **Validity (caller-owned):** `maxTurns` is a caller-owned **positive integer**. WP-264 adds **no** runtime validation, clamp, or coercion — out-of-contract values flow straight into `while (turnsElapsed < maxTurns)` (so `maxTurns <= 0` deterministically yields a zero-turn game). No `RangeError` / throw path is added: this file degrades degenerate inputs (empty seed, no policies, `games < 1`) to zeroed/empty results rather than throwing, and `maxTurns` follows that posture. The only consumer (WP-265) passes a known small literal.
- **Default-equivalence invariant:** any call that omits `maxTurns` is **deep-equal** (`deepStrictEqual`) to the same call passing `maxTurns = MAX_TURNS_PER_GAME`. This is the load-bearing guarantee the tests pin.
- `MAX_TURNS_PER_GAME` remains the single source of the default and stays **module-local** (not exported); no new export is added. Tests asserting default-equivalence use the literal `200`.

## Acceptance Criteria
- [ ] `maxTurns` is an optional trailing param (default `MAX_TURNS_PER_GAME`) on `runPerTurnLoop`, `simulateOneGame`, `buildGameOutcome`, `simulateOneGameAndCaptureMoves`, `runSimulation`, and `sweepSetupMatrix`; supplying a small cap bounds the per-turn loop (capture path: fewer `captured.moves`; sweep path: lower `moveCount`).
- [ ] **Default-equivalence:** omitting `maxTurns` is `deepStrictEqual` to passing the literal `200` on both `simulateOneGameAndCaptureMoves` (including at `gameIndex = 1`, exercising warm-up forwarding) and `sweepSetupMatrix` (asserted in the two tests).
- [ ] **Validity is caller-owned** — no `RangeError` / clamp / coercion added; degenerate `maxTurns` degrades deterministically, consistent with the file's existing degenerate-input posture.
- [ ] No result-shape change — `CapturedGameResult` / `SweepCellResult` / `CapturedOutcomeSummary` / `SimulationResult` unchanged; the two field-set drift guards stay green untouched (no `maxTurns` field added to any result).
- [ ] Existing non-test call sites stay source-unchanged — none is edited merely to pass `200` explicitly.
...
- [ ] `git diff --name-only` shows only the four Files Expected to Change + the listed governance files.
...
git diff --name-only                                   # only the 4 sim files + the listed governance files
node scripts/roadmap-counts.mjs --check                # exits 0 (WP-264 node present)
```
...
- [ ] Only the four implementation/test files plus the listed governance files are modified (nothing else)
