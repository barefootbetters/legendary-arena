...
5. **jsdom:** no `PointerEvent`, no `TouchEvent` constructor needed — tests drive the controller seam and dispatch `new window.Event('touchmove' | 'contextmenu' | 'lostpointercapture', { cancelable: true })` to read `defaultPrevented`. `mock.timers.enable({ apis: ['setTimeout'] })` drives the 350 ms timer (and mocks `clearTimeout`); the default `capturePointer` already guards a missing `setPointerCapture` (`useSlashGesture.ts:272`).
