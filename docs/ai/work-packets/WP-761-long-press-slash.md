...
- On the controller: `isLongPressArmed` (a ref written only by `syncLongPressState()`), `isLongPressHoldEnabled` (the Hold gate) and `shouldPreventTouchScroll()`. Internal (not exposed): `isLongPressLive`, written only by `syncLongPressState()`. A dedicated `armStroke()`.
- Adapter: a non-passive `touchmove` listener (`{ passive: false }`) calling `preventDefault()` when `shouldPreventTouchScroll()`; a `contextmenu` listener; a `lostpointercapture` listener that ends an armed long-press stroke like `pointercancel` only when `event.target` is the row and the `pointerId` matches. These three are attached **only while `isLongPressHoldEnabled`** (the Hold gate: re-evaluated when the fit, the setting or the live long-press state changes — never detached while a long press is live), via a separate `watch([rowElement, isLongPressHoldEnabled], …, { immediate: true, flush: 'post' })` that attaches the three listeners to the current row and detaches them in `onCleanup`, mirroring WP-756's adapter watch (`useSlashGesture.ts:591-603`); the WP-756 listeners keep their existing setting gate.
...
- After every transition above (pending, armed, each cancel path, `pointerup`, setting off, dispose), `isLongPressArmed.value === shouldPreventTouchScroll()` and `isLongPressHoldEnabled.value === (setting on && (row does not fit || a long press is pending or armed))`; and across a mid-hold fit flip (the row starts fitting while pending, then while armed) `isLongPressHoldEnabled.value` stays true until the long press ends.
