# EC-299 — Registry Viewer Hero Mechanic Filter Surface (Execution Checklist)

**Source:** docs/ai/work-packets/WP-268-registry-viewer-mechanic-filter-surface.md
**Layer:** Registry Viewer (`apps/registry-viewer`)

## Before Starting (Hard Gate — run each; STOP if any fails)
- [ ] WP-267 landed — schema exported: `node -e "const s=require('./packages/registry/dist/schema.js'); process.exit(s.CardMechanicsIndexSchema?0:1)"` → exit 0 (else `pnpm -r build`; STOP if still absent)
- [ ] WP-267 landed — feed validates: `node -e "const s=require('./packages/registry/dist/schema.js'); const d=require('./data/metadata/card-mechanics.json'); process.exit(s.CardMechanicsIndexSchema.safeParse(d).success?0:1)"` → exit 0
...
- [ ] No mechanic client yet: `test -f apps/registry-viewer/src/lib/cardMechanicsClient.ts` → **ABSENT**
...
- Empty non-blocking fallback: `{ version, scope, mechanics: [], cards: {} }`
- Filter composition: **OR within selected mechanics, AND with text query + existing filters**
- `hidden: true` mechanics: **excluded** from the default ribbon
...
- Do NOT touch any producer-side file (`scripts/`, `packages/registry`, `data/metadata/`, `.github/`) — that is WP-267
- If precondition A/B fails: STOP — WP-267 not landed; this WP is BLOCKED
...
## After Completing
- [ ] No forbidden import: `grep -rE "from\s+'[^']*(game-engine|apps/server|apps/dashboard|/scripts/)" apps/registry-viewer/src/lib/cardMechanicsClient.ts apps/registry-viewer/src/components/MechanicFilter.vue` → **NO MATCH**
- [ ] Schema subpath + path present: `grep -F '@legendary-arena/registry/schema' …/cardMechanicsClient.ts` ≥1; `grep -F 'card-mechanics.json' …/cardMechanicsClient.ts` ≥1
- [ ] No ability-text parsing for mechanic filtering: `grep -F 'parseAbilityText' …/MechanicFilter.vue …/App.vue | grep -i mechanic` → **NO MATCH**
- [ ] `pnpm --filter registry-viewer typecheck` exit 0; `test` exit 0 (new tests; prior count preserved); `build` exit 0
- [ ] `git diff --name-only | grep -E '^(scripts/|packages/registry/|data/metadata/|\.github/)'` → **NO MATCH**
- [ ] `git status --short | wc -l` → 7 (4 viewer + 3 governance)
- [ ] STATUS/WORK_INDEX/EC_INDEX flipped; DECISIONS **not** touched
- [ ] Commit prefix: `EC-299:` (code) + `SPEC:` (governance)

## Common Failure Smells
- Grid blanks when feed is missing → fallback not non-blocking; client must return the empty structure, never throw
- Hidden mechanics appear as chips → ribbon must filter `hidden === false`
...
- Producer file shows in the diff → out of scope; revert (that's WP-267)
