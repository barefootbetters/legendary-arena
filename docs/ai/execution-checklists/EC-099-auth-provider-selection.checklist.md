...
- [ ] `docs/ai/DECISIONS.md` does not yet contain `D-9901..D-9905` (verified by `grep -nE "^## D-990[1-5] " docs/ai/DECISIONS.md` returning zero matches). The file's last decision before `## Final Note` is `D-9801` (WP-098, commit `1df0033`); D-9701 (WP-097, `c5344cc`) sits immediately above it; D-10001..D-10014 (WP-100 cluster) sit higher still because decisions are appended chronologically, not in numeric order — D-9901..D-9905 will land **after D-9801**, before `## Final Note`, in the same chronological-append slot.
- [ ] `docs/ai/REFERENCE/00.3-prompt-lint-checklist.md` contains §20 (Funding Surface Gate Trigger), inserted by WP-098 between §19 and `## Final Gate`. WP-099's lint self-review walks §1–§20; §20 row is N/A with a concrete non-tautological justification (auth identity is not a funding surface).
...
- D-9901..D-9905 are inserted as a contiguous numeric-order block (`D-9901` → `D-9902` → `D-9903` → `D-9904` → `D-9905`) **immediately before `## Final Note`** at the foot of `DECISIONS.md`, specifically *after* D-9801 (the current foot anchor). This matches the now-two-precedent placement convention: WP-097 / D-9701 (commit `c5344cc`) and WP-098 / D-9801 (commit `1df0033`) both inserted in the same chronological-tail slot. Do not attempt to interleave with D-10001..D-10014 (which sit higher because they were drafted chronologically earlier).
