# EC-767 — Effect-search taxonomy: Reveal + Defeat a Mastermind + KO a Wound (Execution Checklist)
...
{ "slug": "defeat-mastermind", "label": "Defeat a Mastermind", "emoji": "👑", "order": 110,
  "matchers": [ { "type": "regex", "pattern": "\\bdefeat (a|one|the|that|each|any|\\d+) Masterminds?\\b", "flags": "i" } ] }
```
- Expected corpus counts: **reveal-top-of-deck 119**, **ko-a-wound 22**, **defeat-mastermind 5** (±a couple if `main` card-data shifted).
...
- **`defeat-mastermind` mirrors `defeat-villain`** (`Masterminds?\b`); accept the honest low count (5).
...
- [ ] Corpus validation: 119 / 22 / 5, each FP + FR spot-checked
...
- [ ] **Live-on-surface (D-24026):** on `cards.legendary-arena.com`, the 3 chips appear in the Effects dropdown and filter to ~119 / 22 / 5 (operator-pending post-upload)
