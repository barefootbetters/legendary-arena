...
**Last updated:** 2026-04-19 (EC-071 executed at commit `97560b1` — WP-063 replay snapshot producer; `buildSnapshotSequence` + `apps/replay-producer/` landed; first `cli-producer-app` per D-6301; execution surfaced D-6305 reconciling `ReplayInputsFile` with WP-027 canonical `ReplayMove` and surfacing explicit `playerOrder` + `registry` helper params; repo-wide tests 464 → 486; WP-064 Replay Inspector now unblocked)