...
| EC-074 | WP-064 | Client UI (`apps/arena-client/src/replay/` + `src/components/log/` + `src/components/replay/` + `src/fixtures/replay/`) | Game Log & Replay Inspector: four new SFCs (`<GameLogPanel />`, `<ReplayInspector />`, `<ReplayFileLoader />`, plus `loadReplay.ts` parser) that consume `ReplaySnapshotSequence` from WP-063 and drive `useUiStateStore().setSnapshot` on index changes. **Consumer-side `version === 1` assertion per D-6303** carries three locked full-sentence error templates mirroring the WP-063 CLI wording (invalid version / missing `snapshots` / empty `snapshots`). **`<ReplayInspector />` is non-leaf** under `@legendary-arena/vue-sfc-loader` (WP-065 pipeline) and uses `defineComponent({ setup() {...} })` form per P6-30 / P6-40; leaf SFCs may stay in `<script setup>`. Keyboard controls `←` / `→` / `Home` / `End` with `tabindex="0"` root + listeners-on-root — locks **D-64NN** as first repo precedent (confirmed via WP-062 HUD review: no prior keyboard-stepper art). Fixture is 6–12 snapshots produced by the committed WP-063 CLI using `advanceStage` (for `G.currentStage` stage transitions) + unknown-move records (for `UIState.log` growth via `applyReplayStep`'s warning-and-skip at `replay.execute.ts:162–166`); **phase transitions unreachable via producer per D-0205, rescoped to stage transitions** per the WP-064 2026-04-19 amendment. Fixture path `apps/arena-client/src/fixtures/replay/three-turn-sample.{json,inputs.json,cmd.txt}`. Sourcemaps inherit Vite defaults (WP-061); **do NOT copy** WP-063's `process.setSourceMapsEnabled(true)` pattern (cli-producer-app category only, D-6301). WP-064 adds no workspace package — `pnpm-lock.yaml` MUST be absent from commit diff per P6-44. JSDoc in new files uses paraphrase form for forbidden API names per P6-43 to avoid Verification Step 4 false-positive matches. **Note:** EC-062–EC-064 unused; EC-065–EC-073 historically bound; following the EC-061 → EC-067, EC-066 → EC-068, EC-062 → EC-069, EC-063 → EC-071, EC-080 → EC-072, and EC-079 → EC-073 retargeting precedent, WP-064 uses EC-074 — the next free slot. Commits use `EC-074:` prefix (never `WP-064:` per P6-36). 01.5 NOT INVOKED. 01.6 post-mortem MANDATORY (new long-lived abstraction — `parseReplayJson` is the first consumer-side D-6303 assertion site + new keyboard focus precedent D-64NN). See `EC-074-log-replay-inspector.checklist.md` + `docs/ai/preflight-wp064.md` (READY verdict 2026-04-19). | Draft |
...
**Last updated:** 2026-04-19 (EC-074 drafted for WP-064 Game Log & Replay Inspector; READY verdict at `docs/ai/preflight-wp064.md`; locks three consumer-side version-error templates mirroring WP-063 CLI, `defineComponent` form for non-leaf `<ReplayInspector />`, `tabindex="0"` keyboard focus as first repo precedent D-64NN, and `advanceStage` + unknown-move fixture path replacing the unreachable "phase transition" wording from the original WP-064 draft; P6-43 / P6-44 precedents now encoded in EC-074 Common Failure Smells; prior EC-071 execution close footer retained below as historical context — EC-071 executed at commit `97560b1` with repo-wide tests 464 → 486.)