...
- [ ] Live-on-surface (D-24026): on the deployed bundle, with audio armed + Effect-Intensity full, drive `lastPlayEffectsFired` 1→2→3(→5): the flash scales small→medium→big(→legendary), the word appears at `medium`+; toggling Effect-Intensity off blanks visuals AND silences; OS reduced-motion keeps the word, drops the shake. Record in STATUS.md.
