...
>
> *Staleness sweep 2026-04-27: migration slot retargeted `007` → `008`
> (slot `007` consumed by EC-053 / `56e8134` after this EC was drafted);
> test baselines updated to post-WP-113 floor (engine `522/116/0` →
> `570/126/0`; server `36/6/0` → `51/8/0`); per the WP-099 post-WP-098
> staleness sweep precedent.*
...
- [ ] WP-103 migration `006_create_replay_blobs_table.sql` already landed (`fe7db3e`); WP-053 migration `007_create_competitive_scores_table.sql` already landed (`56e8134`); new migration uses slot **`008`**
...
- [ ] `pnpm -r build` exits 0; `pnpm test` exits 0 with engine **570 / 126 / 0** + server **51 / 8 / 0** (or 51 with skips when no test DB) — post-WP-113 baseline per STATUS.md `## Current State`
- [ ] `git diff --name-only packages/ apps/ data/migrations/` empty at start

## Locked Values (do not re-derive)

- Migration number: `data/migrations/008_add_handle_to_players.sql` (slot `006` taken by WP-103, slot `007` taken by WP-053)
...
- WP-052 contract files (`identity.types.ts`, `identity.logic.ts`) NOT modified; WP-052 migrations `004` and `005` NOT modified; WP-103 migration `006` NOT modified; WP-053 migration `007` NOT modified; `packages/game-engine/src/types.ts` NOT modified — all verified by `git diff`.
...
- All exported symbols introduced by this packet use the `Handle` prefix (`HandleClaim`, `HandleErrorCode`, `HANDLE_ERROR_CODES`, `RESERVED_HANDLES`, `HANDLE_REGEX`); future packets adding handle-related symbols (e.g., `HandleSuggestion`, `HandleAvailability`) MUST continue this convention to keep the namespace ownership explicit.
...
- `008_add_handle_to_players.sql` partial unique index: `WHERE handle_canonical IS NOT NULL` permits multiple pre-claim NULLs while enforcing global uniqueness once any handle is claimed.
- `008_add_handle_to_players.sql` `ADD COLUMN IF NOT EXISTS` + `CREATE UNIQUE INDEX IF NOT EXISTS`: WP-052 idempotency precedent for migration runner safety.
...
- `data/migrations/008_add_handle_to_players.sql` — **new** — `ADD COLUMN IF NOT EXISTS` for three columns + partial `CREATE UNIQUE INDEX IF NOT EXISTS … WHERE handle_canonical IS NOT NULL`
...
- [ ] WP-101 acceptance criteria pass; engine baseline **570 / 126 / 0** unchanged; server **51 / 8 / 0** → **63 / 9 / 0** (+12 tests / +1 suite; or 63 with skips when no test DB)
...
- [ ] `Select-String -Path "data\migrations\008_add_handle_to_players.sql" -Pattern "ADD COLUMN IF NOT EXISTS"` returns three matches
- [ ] `Select-String -Path "data\migrations\008_add_handle_to_players.sql" -Pattern "WHERE handle_canonical IS NOT NULL"` returns at least one match
...
- [ ] `git diff` empty for `identity.types.ts`, `identity.logic.ts`, migrations `004`/`005`/`006`/`007`, and `packages/game-engine/src/types.ts`
...
- Migration named `006_*` or `007_*` — STOP; WP-103 already took slot `006` (`fe7db3e`); WP-053 already took slot `007` (`56e8134`). Rename to `008_add_handle_to_players.sql` and re-grep.
- Engine baseline reads anything other than `570 / 126 / 0` after run — STOP; the post-WP-113 baseline is `570 / 126 / 0`. Re-read WP-101 §Assumes; if engine drifted from `570 / 126 / 0`, this WP touched the engine (layer violation).
