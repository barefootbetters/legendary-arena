...
## How This Page Publishes

This page mirrors `docs/09-CHANGELOG.md` in the repository — the single source
of truth, hand-authored, newest first. It is **not** a generated or scheduled
artifact: there is no cron and nothing to regenerate. It republishes whenever
the source file changes, through a build-time projection:

1. Edit `docs/09-CHANGELOG.md` and merge to `main`.
2. That change triggers the `wiki-viewer` CI workflow (its `paths` filter watches
   `docs/09-CHANGELOG.md`). The workflow runs `project-wiki.mjs`, which projects
   the changelog into the wiki as `changelog.md` — injecting page frontmatter,
   dropping the docs-ordering `# 09 — Changelog` heading, and rewriting the
   changelog's `docs/`-relative links to absolute GitHub URLs — then runs
   `check-links.mjs` and the Hugo build as a gate.
3. On a green gate, the workflow fires a **Render Deploy Hook**. Render's
   `legendary-arena-wiki` static site (its own auto-deploy is off, so this hook
   is the sole, gated trigger) rebuilds and serves this page.

So merging a changelog edit updates this page automatically — no manual publish
step. (By contrast, the dashboard's coverage feed *is* a derived artifact,
regenerated by `pnpm ledger:heroes` and CI-gated for freshness; this page needs
neither, because its content is authored directly rather than derived.)
