...
// why: WP-674 / D-24489 — recruit-per-count token is the recruit sibling ([keyword:recruit-per-count:<source>:<perUnit>]); same three-segment shape, drives a recruit grant instead of attack (noir Follow Big Leads)
...
// why: D-24132 — put-any-number-bottom-hq token (the multi-select sibling) has two segments ([keyword:put-any-number-bottom-hq:<n>]); <n> uses [1-9]\d* to reject the zero-magnitude form
// why: D-24133 — put-bottom-hq-icon-reward token (the mandatory single-card + icon-reward sibling) has two segments ([keyword:put-bottom-hq-icon-reward:<n>]); <n> is the icon-reward amount, [1-9]\d* to reject the zero-magnitude form
// why: D-24139 — return-zero-cost-discard token (mandatory discard-to-hand return) has two segments ([keyword:return-zero-cost-discard:<n>]); <n> is the return count (1 for the Defend the Weak form), [1-9]\d* to reject the zero-magnitude form
// why: D-24156 — gain-wound-self / gain-wound-each tokens (the plain "gain a Wound" family) are single-segment, no magnitude ("gain a Wound" is exactly one Wound)
// why: D-24291 — defeat-with-bystander token (Silent Sniper "Defeat a Villain or Mastermind that has a Bystander.") is single-segment, no magnitude (it defeats exactly one eligible target)
// why: WP-498 / D-24301 — return-on-discard token (Cyclops Unending Energy "If a card effect makes you discard this card, you may return this card to your hand.") is single-segment, no magnitude (a reactive marker)
// why: WP-535 / D-24345 — copy-powers token (Rogue "Copy Powers": "Play this card as a copy of another Hero you played this turn.") is single-segment, no magnitude (it copies exactly one Hero)
// why: WP-545 / D-24354 — recruit-threshold token (Thor "Surge of Power": "If you made 8 or more Recruit this turn, you get +3 Attack.") has two segments ([keyword:recruit-threshold:<n>]); <n> is the recruit-made threshold, [1-9]\d* to reject the zero-magnitude form. The engine parses <n> into a recruitMadeThisTurnAtLeast condition that gates the line's printed +3 attack.
// why: WP-665 / D-24476 — draw-threshold token (Amadeus Cho "Gamma-Draining Nanites": "Draw a card. Then, if you drew two cards this turn, Transform this into Like Totally Smart Hulk.") has two segments ([keyword:draw-threshold:<n>]); <n> is the cards-drawn threshold, [1-9]\d* to reject the zero form. The engine parses <n> into a cardsDrawnThisTurnAtLeast wait-and-see condition that gates ONLY the transform ability line (abilities[1]); the "Draw a card" (abilities[0]) is a separate, unconditional hook.
// why: WP-667 / D-24480 — optional-ko-hand-discard token (She-Hulk "Radioactive Riot": "Once this turn, if you made at least 6[icon:recruit] this turn, you may KO a card from your hand or discard pile.") is single-segment, no magnitude (no reward — the KO is deck-thinning). It parks a no-reward entry into the shipped optional-ko-reward pending queue with koZones ['hand','discard']; the co-located [keyword:recruit-threshold:6] gates the hook (wait-and-see). The +3 Attack is the card's printed Attack stat, not a hook effect.
// why: WP-668 / D-24481 — reveal-herodeck-attack token (She-Hulk "Jade Giantess": "For every 2[icon:recruit] you made this turn, Reveal the top card of the Hero Deck, put it on the bottom of that deck, and you get that card's printed[icon:attack].") has two segments ([keyword:reveal-herodeck-attack:<n>]); <n> is the "for every N Recruit" divisor, [1-9]\d* to reject the zero form. The engine parses <n> into the effect magnitude; the handler computes floor(G.turnEconomy.recruit / n) reveals of the top Hero-Deck card, granting each card's printed attack and rotating it to the bottom. The co-located printed [icon:attack] is subsumed by the keyword (the attack-per-count precedent), so it emits no phantom flat attack.
// why: WP-580 / D-24389 — recruit-as-attack token (Thor "God of Thunder": "You can use Recruit as Attack this turn.") is single-segment, no magnitude (it sets a turn-scoped conversion flag).
// why: WP-592 / D-24401 — steal-abilities token (Rogue "Steal Abilities": "Each player discards the top card of their deck. Play a copy of each of those cards.") is single-segment, no magnitude (the deterministic discard-then-copy handler computes its targets from G at play time).
// why: WP-656 / D-24467 — defeated-villain-or-mastermind token (Emma Frost "Diamond Form": "Whenever you defeat a Villain or Mastermind this turn, you get +3 Recruit.") is single-segment, no magnitude (a boolean wait-and-see gate). The engine parses it into a defeatedVillainOrMastermindThisTurn condition that gates the line's printed +3 recruit, so the recruit fires once per qualifying defeat this turn rather than unconditionally on play.
// why: WP-659 / D-24470 — reveal-from-hand token (Emma Frost "Psychic Link": "Each player may reveal another X-Men Hero. Each player who does draws a card.") is single-segment, no magnitude. It tells the engine the co-located [team:x-men] token is the reveal CRITERION, not a requiresTeam play-gate — fixing the mis-parse that wrongly gated the card on "another X-Men Hero played this turn" — and drives the reveal-from-hand → draw mechanic (each player holding a matching Hero draws one).
// why: WP-663 / D-24474 — optional-play-villain-top token (Emma Frost "Shadowed Thoughts": "[hc:covert]: You may play the top card of the Villain Deck. If you do, you get +2 Attack.") has two segments ([keyword:optional-play-villain-top:<n>]); <n> is the attack reward, [1-9]\d* to reject the zero form. The engine parks an optional pending choice — accept plays the top Villain-Deck card (city entry / Master Strike / Scheme Twist) + grants +n Attack, decline nothing — and suppresses the printed +2[icon:attack] so the reward is the choice's, not a second unconditional grant.
// why: WP-676 / D-24492 — smash token (the wwhk "Smash N": "You may discard another card from your hand. If you do, you get +N attack.") has two segments ([keyword:smash:<n>]); <n> is the +N Attack a discard grants, [1-9]\d* to reject the zero form. It is APPENDED alongside the inert display token [keyword:Smash N] (capital-S + space, which KEYWORD_PATTERN cannot capture), so the appended lowercase token drives the engine while the display text is unchanged. The engine parks an optional discard-for-attack pending choice per Smash instance (two "Smash 2" entries on Hurl Trucks → two parks). The bare magnitude-less display verb [keyword:Smash] (in the co-printed conditional-KO clauses) is deliberately NOT marked and safe-skips at the executeSingleEffect magnitude pre-gate.
// why: WP-679 / D-24495 — attack/recruit-per-count gain an OPTIONAL 4th `perEach` segment
// (`:[1-9]\d*`) for "for each N" forms; the grant is magnitude × floor(count / perEach). vnom's
// 3-segment markers still match (perEach absent ≡ 1). The two Undercover source-shape tokens
// (undercover-hand-shield-hero / undercover-officer-stack) are single-segment, no magnitude —
// each an option of the two shld mixed choose-ones, dispatched to WP-678's send executor.
// why: WP-683 / D-24500 — [keyword:here-hold-this] (Deadpool's directed Bystander capture) and
// [keyword:random-acts] (his optional gain-Wound-to-hand + simultaneous multi-seat pass-left) are
// single-segment, no-magnitude bare tokens, added to the locked forms alongside the WP-681 pair.
// why: WP-700 / D-24519 — [keyword:put-hand-on-deck-top:<n>] ("Draw N cards. Then put a card from
// your hand on top of your deck." — Gambit's Stack the Deck + siblings) is a two-segment token
// whose <n> is the printed draw count ([1-9]\d* rejects the zero form). It is APPENDED to the
// prose ability line; the compound handler draws N then parks the mandatory put-on-top choice.
// why: WP-714 / D-24537 — [keyword:kidnap-per-count:<source>:<perUnit>] (Ultron's Genetic
// Experimentation: "[hc:tech]: Kidnap a Bystander for each other [hc:tech] Ally you played this
// turn.") is the bystander-capture sibling of attack/recruit-per-count — same three-segment shape,
// but the engine effect captures N = magnitude × count Bystanders instead of granting a resource.
// No optional 4th perEach segment (this card's rate is 1); <source> is a lowercase-hyphen slug,
// <perUnit> uses [1-9]\d* to reject the zero-magnitude form.
// why: WP-702 / D-24521 — [keyword:reveal-top-dispose] ("Reveal the top card of your deck. Discard
// it or put it back." — Gambit's Hypnotic Charm entry 1 + standalone family) and
// [keyword:reveal-top-dispose-others] ("Do the same thing to each other player's deck." — Hypnotic
// Charm entry 2, [hc:instinct]-gated) are single-segment, no-magnitude bare tokens (per-card
// disposition, no count). For the wpnx SPLIT form the marker rides the FIRST "Look at the top
// card…" entry (abilityIndex 0); the second "Discard it or put it back." entry stays decorative prose.
// why: WP-719 / D-24541 — [keyword:covering-fire] (Hawkeye's Covering Fire: "[hc:tech]: Choose one:
// each other player draws a card or each other player discards a card." — both identical printings
// core + msp1) is a single-segment, no-magnitude bare token (a choose-one branch, not a count). The
// [hc:tech]: prefix stays the parser-read class gate; this marker only feeds the choose-one park.
// why: WP-765 / D-24598 — [keyword:blood-frenzy] / [keyword:blood-frenzy-recruit] (hero Blood Frenzy:
// +1 attack — or recruit, Morbius Mesmerize — per distinct VP value in the Victory Pile) are
// single-segment, no-magnitude bare tokens APPENDED beside the inert space-form display token
// [keyword:Blood Frenzy] (which KEYWORD_PATTERN cannot capture). [keyword:day-night-both] is the
// fused Sunlight / Moonlight composite keyword; the parser emits it itself for the allowlisted
// cards, and it is accepted here only so the three new keywords share one locked form.
// why: WP-767 / D-24600 — [keyword:optional-ko-your-hero] (Snarling Fangs’ Moonlight "you may KO
// one of your Heroes") is a single-segment, no-magnitude bare token appended after the
// [keyword:defeated-villain-or-mastermind] per-defeat trigger on the same line.
const VALID_TOKEN_PATTERN =
  /^\[keyword:rescue:\d+\]$|^\[keyword:reveal\]$|^\[keyword:reveal:\d+\]$|^\[keyword:reveal:(?:team|hc)-[a-z][a-z0-9-]*:draw\]$|^\[keyword:reveal:always:choose-discard-or-return\]$|^\[keyword:reveal-ko\]$|^\[keyword:reveal-min:\d+\]$|^\[keyword:reveal-ko-or-draw:\d+\]$|^\[keyword:reveal-cost-attack\]$|^\[keyword:reveal-odd-draw\]$|^\[keyword:reveal-attack-choose:[1-9]\d*\]$|^\[keyword:reveal-ko-attack:[1-9]\d*\]$|^\[keyword:draw:[1-9]\d*\]$|^\[keyword:attack-per-count:[a-z][a-z-]*:[1-9]\d*(?::[1-9]\d*)?\]$|^\[keyword:recruit-per-count:[a-z][a-z-]*:[1-9]\d*(?::[1-9]\d*)?\]$|^\[keyword:kidnap-per-count:[a-z][a-z-]*:[1-9]\d*\]$|^\[keyword:optional-ko-reward:[a-z][a-z-]*:[1-9]\d*\]$|^\[keyword:optional-put-bottom-hq:[1-9]\d*\]$|^\[keyword:put-any-number-bottom-hq:[1-9]\d*\]$|^\[keyword:put-bottom-hq-icon-reward:[1-9]\d*\]$|^\[keyword:return-zero-cost-discard:[1-9]\d*\]$|^\[keyword:gain-wound-self\]$|^\[keyword:gain-wound-each\]$|^\[keyword:shuffle-discard-empty-reward:[a-z][a-z-]*:[1-9]\d*\]$|^\[keyword:ko-wound-reward:[a-z][a-z-]*:[1-9]\d*\]$|^\[keyword:ko-wound\]$|^\[keyword:discard-to-play:[1-9]\d*\]$|^\[keyword:defeat-with-bystander\]$|^\[keyword:return-on-discard\]$|^\[keyword:copy-powers\]$|^\[keyword:recruit-threshold:[1-9]\d*\]$|^\[keyword:draw-threshold:[1-9]\d*\]$|^\[keyword:optional-ko-hand-discard\]$|^\[keyword:recruit-as-attack\]$|^\[keyword:steal-abilities\]$|^\[keyword:defeated-villain-or-mastermind\]$|^\[keyword:reveal-from-hand\]$|^\[keyword:optional-play-villain-top:[1-9]\d*\]$|^\[keyword:reveal-herodeck-attack:[1-9]\d*\]$|^\[keyword:smash:[1-9]\d*\]$|^\[keyword:undercover-hand-shield-hero\]$|^\[keyword:undercover-officer-stack\]$|^\[keyword:attack:[1-9]\d*\]$|^\[keyword:do-over\]$|^\[keyword:first-hero-condition\]$|^\[keyword:optional-ko-shield-officer\]$|^\[keyword:diving-block\]$|^\[keyword:pure-fury\]$|^\[keyword:here-hold-this\]$|^\[keyword:random-acts\]$|^\[keyword:put-hand-on-deck-top:[1-9]\d*\]$|^\[keyword:reveal-top-dispose\]$|^\[keyword:reveal-top-dispose-others\]$|^\[keyword:covering-fire\]$|^\[keyword:no-more-draws\]$|^\[keyword:reveal-three-assign\]$|^\[keyword:reveal-three-assign-again\]$|^\[keyword:optional-discard-draw:[1-9]\d*\]$|^\[keyword:reveal-top-may-ko\]$|^\[keyword:blood-frenzy\]$|^\[keyword:blood-frenzy-recruit\]$|^\[keyword:day-night-both\]$|^\[keyword:optional-ko-your-hero\]$|^\[keyword:reveal:cost-zero:discard\+attack-fixed-[1-9]\d*\]$/;
// why: WP-681 / D-24498 — [keyword:attack:N] is a flat conditional attack grant (High-Tech
// Weaponry's "[hc:tech]: You get +1[icon:attack]." — the [hc:tech]: prefix is the class gate
// the parser already reads, attack:N is the grant); [keyword:do-over] + [keyword:first-hero-condition]
// are Deadpool's Do-Over (the do-over keyword + its first-hero-played-this-turn condition marker,
// both on the same ability line); [keyword:optional-ko-shield-officer] is Nick Fury's Battlefield
// Promotion (optional KO of a S.H.I.E.L.D. Hero + gain a S.H.I.E.L.D. Officer to hand).
// why: WP-682 / D-24499 — [keyword:diving-block] is Captain America's Diving Block (reactive
// wound interception, no magnitude — the reveal/decline offer + card count are computed at
// wound time); [keyword:pure-fury] is Nick Fury's Pure Fury (free conditional defeat, no
// magnitude — the eligible-target set + the KO S.H.I.E.L.D.-Hero count are computed at play time).
// why: WP-729 / D-24550 — [keyword:reveal:team-<slug>:draw] / [keyword:reveal:hc-<slug>:draw] is the
// first PARAMETERIZED reveal marker (Card Shark "reveal top; if it's an [team/hc:X] Hero, draw it"):
// a reveal-rule TRAIT predicate consumed by REVEAL_RULE_PATTERN. Closed-set extension (D-21601
// lineage) — the only token whose predicate segment is a team-/hc- slug rather than a cost.
// why: WP-734 / D-24554 — [keyword:reveal:always:choose-discard-or-return] is the co2e Card Shark 2e
// (co2e/gambit/kinetic-card) non-match disposition: paired with the team-x-men:draw rule, an X-Men
// match draws + stops and a non-match parks the D-22003 discard-or-return choice. Closed-set
// extension — the only `always:` reveal token; reuses the shipped choose action, no engine change.
// why: WP-731 / D-24552 — [keyword:no-more-draws] is Venompool's Shenanigans turn-scoped draw
// restriction, applied alongside [keyword:draw:2] on the one ability line (the draw fires
// first, then the lock arms). No magnitude.
// why: WP-753 / D-24580 — [keyword:reveal-three-assign] ("Reveal the top three cards of your deck.
// Draw one of them, discard one, and KO one." — vnom Crystal of Kadavus, 3dtc + dims Interplanetary
// Visitor) and [keyword:reveal-three-assign-again] (Crystal's "[team:venomverse][team:venomverse]: Do
// this ability again.", appended to that gated line so the parser keeps its requiresTeam condition)
// are single-segment, no-magnitude bare tokens (a per-card assignment and a repeat counter).
// why: WP-754 / D-24581 — [keyword:optional-discard-draw:N] ("You may discard a card. If you do,
// draw a card." — vnom Hungry for Action's Digest 3 line, gotg Gritty Scavenger, asrd Bio-Engineered
// Cyborg, shld GW Bridge, antm Risky Science's [hc:tech] line) carries the draw count N; and
// [keyword:reveal-top-may-ko] ("Reveal the top card of your deck. You may KO it." — vnom Gruesome
// Feast + mgtg Remove His Spine on their Excessive Violence lines, vill Electroshock Therapy) is a
// single-segment, no-magnitude bare token. On a Digest / Excessive Violence line the marker becomes
// the fused hook's inner effect; a leading [hc:X]: stays the parser-read class gate.
// why: D-24582 — [keyword:reveal:cost-zero:discard+attack-fixed-<n>] is the parameterized reveal
// for vnom See Future Timelines ("Reveal the top card of your deck. If it costs 0, discard it and
// you get +2[icon:attack]."): the new 'discard' reveal action, then the fixed attack only when the
// discard applied. The printed [icon:attack] is subsumed by the rule's attack-fixed (no phantom grant).
...
function runApply(markerMap) {
  ...
  for (const setAbbr of Object.keys(markerMap).sort()) {
    ...
    for (const entry of entries) {
      ...
      // why: WP-667 / D-24480 — a single ability line may carry MORE THAN ONE marker
      // (Radioactive Riot's line takes both [keyword:recruit-threshold:6] and
      // [keyword:optional-ko-hand-discard]). Carry the just-appended token forward into
      // the in-memory ability so a later entry for the SAME abilityIndex resolves the
      // already-marked line — otherwise its search for the original (now-marked) text in
      // fileText fails. resolveAbility returns card.abilities[abilityIndex] by reference.
      card.abilities[abilityIndex] = markedLine;
      fileModified = true;
      totalUpdated++;
    }
